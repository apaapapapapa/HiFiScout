...
function sameIdentity(existing, product) {
  return existing?.title === product.title &&
...
function cachedClassification(existing, product) {
  ...
  if (metadata.state !== 'classified' || existing.classification_status !== 'classified') return null;
...
export async function enrichProductCategories({
  ...
  for (const product of products) {
    ...
    if (cached) {
      cacheHits += 1;
      enrichedCount += 1;
      enriched.push(applyCategoryClassification(product, cached, product.categoryEvidence, {
    ...
    if (recentUnresolvedCheck(existing, product, policy.enrichment.cacheHours, now)) {
      cacheHits += 1;
      enriched.push(withDetailCheckMetadata(product, classificationMetadata(existing)?.detailCheckedAt));
      continue;
    }
    ...
    try {
      ...
      const updated = applyCategoryClassification(product, classification, evidence, { detailCheckedAt: checkedAt });
  ...
  return {
    ...
    cacheHits,
    enrichedCount,
    unresolvedCount: enriched.filter(product => product.classificationStatus !== 'classified').length
  };
}
