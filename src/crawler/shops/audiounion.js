...
const BRAND_SUFFIXES = new Set([
  'audio', 'acoustics', 'acoustic', 'audio labs', 'labs', 'lab', 'design', 'designs',
  'electronics', 'technology', 'technologies', 'digital', 'research', 'engineering'
]);
const PRODUCT_TYPE_PREFIX = /^(?:スピーカー(?:システム)?|プリメインアンプ|プリアンプ|パワーアンプ|アンプ|cd\/?sacd(?:プレーヤー)?|sacd(?:プレーヤー)?|cd(?:プレーヤー)?|dac|d\/aコンバーター?|ネットワーク(?:プレーヤー)?|ターンテーブル|レコードプレーヤー|カートリッジ|ヘッドホン|イヤホン)\s*/i;
...
function combineKnownManufacturer(candidates) {
  for (let size = Math.min(3, candidates.length); size >= 1; size -= 1) {
    for (let start = 0; start + size <= candidates.length; start += 1) {
      ...
      if (exactKnownManufacturer(raw)) return { raw, consumed: new Set(candidates.slice(start, start + size)) };
    }
  }
  return null;
}
...
function repairIdentity(item, candidates) {
  ...
  if (known) {
    ...
    return { ...item, manufacturer: known.raw, model, title, category: inferCategory(title, item.category === 'その他' ? '' : item.category) };
  }

  const prefixPair = prefixPairIdentity(candidates);
  if (prefixPair) {
    const title = `${prefixPair.manufacturer} ${prefixPair.model}`.trim();
    return { ...item, ...prefixPair, title, category: inferCategory(title, item.category === 'その他' ? '' : item.category) };
  }

  const suffixBrand = suffixBrandIdentity(candidates);
  if (suffixBrand) {
    const title = suffixBrand.model ? `${suffixBrand.manufacturer} ${suffixBrand.model}` : suffixBrand.manufacturer;
    return { ...item, ...suffixBrand, title, category: inferCategory(title, item.category === 'その他' ? '' : item.category) };
  }
  ...
  if (cleanTitle !== item.title) {
    ...
    return { ...item, ...split, title: cleanTitle, category: inferCategory(cleanTitle, item.category === 'その他' ? '' : item.category) };
  }
  return item;
}
