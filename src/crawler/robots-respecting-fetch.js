...
export function createRobotsRespectingFetch(fetchImpl = globalThis.fetch, {
  ...
  return async function robotsRespectingFetch(input, init = {}) {
    ...
    if (!['http:', 'https:'].includes(parsed.protocol)) throw new Error('unsupported_knowledge_source_protocol');
    ...
    if (!isPathAllowed(policy.text, url, userAgent)) {
    ...
    return fetchImpl(input, mergeUserAgent(init, userAgent));
  };
}
