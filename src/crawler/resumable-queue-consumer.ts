...
export type ResumableCrawlConsumeResult =
  | { kind: "retry"; shopKey: string; runId?: string; reason: "crawl_in_progress" | "continuation_ahead" | "finalization_in_progress"; retryAfterSeconds: number }
  | { kind: "continued"; shopKey: string; runId: string; sequence: number; phase: CrawlFetchContinuationPhase; pageKey: string | null }
...
function workerVersion(env: CrawlerEnv): string | null {
  const metadata = (env as CrawlerEnv & { CF_VERSION_METADATA?: { id?: string } }).CF_VERSION_METADATA;
  return metadata?.id || null;
}

function queueForLane(env: CrawlerEnv, lane: CrawlQueueLane): Pick<Queue<CrawlQueueMessage>, "send"> | null {
  const queue = lane === "fast" ? env.CRAWL_FAST_QUEUE : lane === "heavy" ? env.CRAWL_HEAVY_QUEUE : env.CRAWL_RELAY_QUEUE;
  return queue || env.CRAWL_QUEUE || null;
}

async function releaseExecutionLease(db: QueryableDatabase, shopKey: string, crawlLeaseToken: string): Promise<void> {
  await db.prepare(`
    UPDATE shop_sync_state SET crawl_lease_token = NULL, crawl_lease_until = NULL
    WHERE shop_key = ? AND crawl_lease_token = ?
  `).bind(shopKey, crawlLeaseToken).run();
}
...
function continuationFromSession(session: CrawlFetchSessionRow): CrawlContinuationDescriptor | null {
...
async function sendContinuation(
  ...
  console.log(JSON.stringify({ event: "crawl_fetch_continuation_enqueued", shopKey: plugin.key, runId: session.run_id, sequence: continuation.sequence, phase: continuation.phase, pageKey: continuation.pageKey || null, lane, workerVersion: workerVersion(env) }));
}
...
async function ensureSession(
  ...
  const result = await ensureCrawlFetchSession(env.DB, { runId, shopKey: plugin.key, requestedAt: body.requestedAt, maxPages, pageLimit, pages: pageInputs(plugin, initial), createdAt });
  if (result.created) {
    console.log(JSON.stringify({ event: "crawl_fetch_session_started", shopKey: plugin.key, runId, requestedAt: body.requestedAt, pageLimit, initialPages: initial.length, workerVersion: workerVersion(env) }));
  }
  return result.session;
}

async function failCollection(env: RuntimeEnv, plugin: ShopPlugin, runId: string, error: unknown): Promise<ResumableCrawlConsumeResult> {
  ...
  console.warn(JSON.stringify({ event: "crawl_fetch_collection_failed", shopKey: plugin.key, runId, message, workerVersion: workerVersion(env) }));
  return { kind: "terminal", runId, result: { status: "failed", shopKey: plugin.key, crawlRunId: null, error: message, dataQuality: null } };
}

async function continued(env: RuntimeEnv, plugin: ShopPlugin, body: ResumableCrawlQueueMessage, runId: string): Promise<ResumableCrawlConsumeResult> {
  ...
  return { kind: "continued", shopKey: plugin.key, runId, sequence: session.continuation_sequence, phase: session.next_phase || "finalize", pageKey: session.next_page_key };
}
...
async function processFetch(
  ...
  try {
    let html: string;
    try {
      html = await transport.fetchHtmlPage(pageKey, { baseUrl: plugin.baseUrl, userAgent: settings.userAgent, requestDelayMs, fetchFn: globalThis.fetch, robotsCache });
    } catch (error) {
      if (/HTTP 404/.test(errorMessage(error)) && shouldContinueAfterEmpty(plugin)) {
        ...
        await recordCrawlFetchPageIgnored(env.DB, { runId: session.run_id, pageKey, ignoredAt: new Date().toISOString(), currentSequence: session.continuation_sequence, nextPageKey: nextPendingPageKey(pages, pageKey) });
    ...
    await recordCrawlFetchPageFetched(env.DB, { runId: session.run_id, pageKey, html, htmlBytes, fetchedAt, currentSequence: session.continuation_sequence });
    console.log(JSON.stringify({ event: "crawl_fetch_page_fetched", shopKey: plugin.key, runId: session.run_id, page: row.ordinal, pageKey, htmlBytes, durationMs: Date.now() - startedAtMs, workerVersion: workerVersion(env) }));
  } finally {
    await transport.close?.().catch((error: unknown) => console.warn(JSON.stringify({ event: "crawl_fetch_transport_close_failed", shopKey: plugin.key, runId: session.run_id, message: errorMessage(error) })));
  }
  return continued(env, plugin, body, session.run_id);
}
...
async function processParse(
  ...
  const row = await getCrawlFetchPage(env.DB, session.run_id, pageKey);
  if (!row) throw new Error(`crawl frontier page not found: ${pageKey}`);
  if (row.state !== "fetched" || row.html_text == null) return continued(env, plugin, body, session.run_id);
  ...
  const reachedEnd = products.length === 0 && previousItems > 0 && !shouldContinueAfterEmpty(plugin);
  ...
  console.log(JSON.stringify({ event: "crawl_fetch_page_parsed", shopKey: plugin.key, runId: session.run_id, page: row.ordinal, pageKey, htmlBytes: row.html_bytes, itemCount: products.length, parseMs, discoveredCount: accepted.length, coverageIncomplete, reachedEnd, workerVersion: workerVersion(env) }));
  return continued(env, plugin, body, session.run_id);
}
...
function stagedFetchFunction(
  ...
  return (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url === syntheticUrl) {
      return new Response("<!doctype html><title>staged crawl</title>", { status: 200, headers: { "content-type": "text/html; charset=utf-8" } });
    }
...
async function processFinalize(
  ...
  if (!claimed) {
    ...
    if (refreshed?.status === "completed") return { kind: "terminal", runId: session.run_id, result: { status: "skipped", reason: "stale_dispatch", shopKey: plugin.key } };
    if (refreshed?.status === "failed") {
      return { kind: "terminal", runId: session.run_id, result: { status: "failed", shopKey: plugin.key, crawlRunId: refreshed.final_crawl_run_id, error: refreshed.error_message || "crawl collection failed", dataQuality: null } };
    }
    return { kind: "retry", shopKey: plugin.key, runId: session.run_id, reason: "finalization_in_progress", retryAfterSeconds: 60 };
  }
  ...
  const { transport: ignoredTransport, diagnostics: ignoredDiagnostics, ...safeCapabilities } = plugin.capabilities;
  ...
  const originalTransport = createTransport(env, plugin.capabilities.transport?.kind, globalThis.fetch);
  try {
    const result = await crawlShop(env, publishAdapter, { force: true, fetchFn: stagedFetchFunction(env, plugin, syntheticUrl, originalTransport) });
    ...
    if (result.status === "success") {
      ...
      await completeCrawlFetchSession(env.DB, { runId: session.run_id, finalizedAt: finishedAt, crawlRunId: result.crawlRunId });
      console.log(JSON.stringify({ event: "crawl_fetch_session_published", shopKey: plugin.key, runId: session.run_id, crawlRunId: result.crawlRunId, pageCount: claimedSession.pages_parsed, itemCount: result.itemCount, coverageIncomplete: Boolean(claimedSession.coverage_incomplete), lastCompletedPage: claimedSession.last_completed_page, workerVersion: workerVersion(env) }));
      return { kind: "terminal", runId: session.run_id, result: finalResult };
    }

    if (result.status === "failed") {
      if (result.crawlRunId != null) await setPublishedCrawlPageCount(env.DB, result.crawlRunId, claimedSession.pages_parsed);
      await failCrawlFetchSession(env.DB, { runId: session.run_id, failedAt: finishedAt, message: result.error, crawlRunId: result.crawlRunId });
    }
    return { kind: "terminal", runId: session.run_id, result };
  } finally {
    await originalTransport.close?.().catch((error: unknown) => console.warn(JSON.stringify({ event: "crawl_finalize_transport_close_failed", shopKey: plugin.key, runId: session.run_id, message: errorMessage(error) })));
  }
}
...
async function executeContinuation(
  ...
  if (session.status === "completed") return { kind: "terminal", runId, result: { status: "skipped", reason: "stale_dispatch", shopKey: plugin.key } };
  if (session.status === "failed") return { kind: "terminal", runId, result: { status: "failed", shopKey: plugin.key, crawlRunId: session.final_crawl_run_id, error: session.error_message || "crawl collection failed", dataQuality: null } };

  const deliveredSequence = body.continuation?.sequence ?? 0;
  if (deliveredSequence > session.continuation_sequence) return { kind: "retry", shopKey: plugin.key, runId, reason: "continuation_ahead", retryAfterSeconds: 5 };
  ...
  if (body.continuation && (body.continuation.phase !== canonical.phase || (body.continuation.pageKey || null) !== (canonical.pageKey || null))) {
...
export async function consumeResumableCrawlMessage(
  ...
  if (!plugin) return { kind: "terminal", result: { status: "skipped", reason: "unknown_shop", shopKey: body.shopKey } };
  if (!body.requestedAt) return { kind: "terminal", result: { status: "skipped", reason: "not_due", shopKey: plugin.key } };
  if (!isTransportConfigured(env, plugin.capabilities.transport?.kind)) return { kind: "terminal", result: { status: "skipped", reason: "configuration_missing", shopKey: plugin.key } };

  const runId = body.collectionRunId || canonicalRunId(plugin.key, body.requestedAt);
  const claimedAtDate = new Date();
  const claimedAt = claimedAtDate.toISOString();
  const crawlLeaseToken = await tryClaimShopCrawl(env.DB, plugin.key, body.requestedAt, claimedAt, EXECUTION_LEASE_MINUTES);
  if (!crawlLeaseToken) {
    ...
    const retryAfterSeconds = matchesDispatchReservation(state, plugin.key, body.requestedAt)
      ...
      : null;
    if (retryAfterSeconds != null) return { kind: "retry", shopKey: plugin.key, runId, reason: "crawl_in_progress", retryAfterSeconds };
    return { kind: "terminal", runId, result: { status: "skipped", reason: "stale_dispatch", shopKey: plugin.key } };
  }
