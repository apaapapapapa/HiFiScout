...
const CATEGORY_RULES: readonly (readonly [label: string, pattern: RegExp])[] = [
  ...
  [
    "DAP・ヘッドホンアンプ",
    /\bdap\b|digital audio player|headphone amp|ヘッドホンアンプ|ポータブルプレーヤ/i,
  ],
  [
    "DJ機器・DTM",
    /\bdj\b|\bddj[-\s]|rekordbox|serato|turntable controller|オーディオインターフェース|midi/i,
  ],
...
export function stableSourceId(url: string, title: string = ""): string {
  try {
    ...
    const pathId = parsed.pathname.match(
      /(?:detail|goods|item|product|shopdetail)[/_-]?([A-Za-z0-9_-]+)/i,
    )?.[1];
