...
export function candidateModelVariants(candidate = {}) {
  ...
  const rawValues = [candidate.observedModel, candidate.model, candidate.normalizedModel].filter(Boolean);
...
function matchesCandidateText(text, candidate) {
  return candidateModelVariants(candidate).some(model => containsFlexibleCatalogModelIdentity(text, model));
}
...
function metaContent(html, name) {
  ...
  for (const match of String(html).matchAll(/<meta\b[^>]*>/gi)) {
    ...
    if (clean(attributes.get('name')).toLowerCase() === target) return clean(attributes.get('content'));
  }
  return '';
}
...
function matchingProductNode(products, candidate) {
  return products.find(product =>
    [product.model, product.sku, product.mpn, product.name].some(value => value && matchesCandidateText(value, candidate))
  ) || null;
}
...
function officialCategoryIds(text = '') {
  ...
  if (/\b(?:disc|disk)\s+player\b|\bsa-?cd\s+player\b|スーパーオーディオ\s*cd(?:\s*\/\s*cd)?\s*(?:プレーヤー|プレイヤー)/i.test(value)) {
    ids.add('cd_sacd_player');
  }
  if (/\bphono\s+(?:equalizer\s+)?amplifier\b|フォノ(?:イコライザー)?アンプ/i.test(value)) ids.add('phono_eq');
  return [...ids];
}
...
function categoryEvidence(value, strength = 'verified') {
  ...
  return categoryIds.length ? { categoryIds, source: 'manufacturer_official', strength, value: text } : null;
}
...
export async function verifyOfficialProductPageHtmlV2({
  ...
  const productNodes = jsonLdValues(html).flatMap(value => flattenJsonLd(value)).filter(isProductNode);
  ...
  const modelMatched = [product?.model, product?.sku, product?.mpn, product?.name, h1, title, pageText]
  ...
  if (!classification || classification.classificationStatus !== 'classified') {
    const fallbackEvidence = [product?.description, metaContent(html, 'description'), breadcrumbText(html)]
  ...
  if (!classification || classification.classificationStatus !== 'classified' || !classification.categoryIds.length) {
    return {
      status: 'ambiguous', sourceUrl, sourceType, httpStatus,
      message: classification?.classificationState === 'ambiguous'
        ? 'conflicting_official_category_evidence'
        : 'official_page_has_no_unambiguous_category'
    };
  }

  const directModel = [product?.model, product?.sku, product?.mpn].find(value => value && matchesCandidateText(value, candidate));
  const canonicalModel = clean(candidate.observedModel || candidate.model || directModel || candidate.normalizedModel);
...
function htmlTextWithLabels(value = '') {
  return stripTags(String(value).replace(/<(?:img|input)\b([^>]*)>/gi, (_, attrs) => {
    const labels = [...String(attrs).matchAll(/\b(?:alt|title|aria-label)\s*=\s*["']([^"']+)["']/gi)]
      .map(match => match[1]);
...
function isLikelyCatalogIndexEntry(entry) {
  ...
  let pathname = '';
  try { pathname = new URL(entry?.url).pathname.toLowerCase(); } catch {}
  return /(?:製品(?:一覧|情報|紹介)?|生産終了|アーカイブ|product\s*(?:archive|museum|list)?|all\s+product|discontinued|catalog)/i.test(label) ||
    /\/(?:products?|history|support\/discon|support\/catalogue|category\/archive[^/]*)\/?$/i.test(pathname);
}
...
export function enhancedKnowledgeSourceDefinitions(env = {}) {
  ...
  for (const [manufacturerId, sources] of definitions) {
    result.set(manufacturerId, sources.map(source => {
...
export function createKnowledgeSourceVerifierV2(env = {}, { fetchImpl = globalThis.fetch } = {}) {
  ...
  const maxCatalogPages = boundedNumber(env.KNOWLEDGE_CATALOG_SOURCE_MAX_CATALOG_PAGES, DEFAULT_MAX_CATALOG_PAGES, 1, 20);
  const maxSitemaps = boundedNumber(env.KNOWLEDGE_CATALOG_SOURCE_MAX_SITEMAPS, DEFAULT_MAX_SITEMAPS, 1, 30);
  const maxDiscoveredUrls = boundedNumber(env.KNOWLEDGE_CATALOG_SOURCE_MAX_URLS, DEFAULT_MAX_DISCOVERED_URLS, 100, 30_000);
  const maxProductPages = boundedNumber(env.KNOWLEDGE_CATALOG_SOURCE_MAX_PRODUCT_PAGES, DEFAULT_MAX_PRODUCT_PAGES, 1, 12);
  ...
  async function cacheForSource(source) {
    ...
    const queue = [...new Set((source.catalogUrls || []).map(url => sameOriginUrl(url, source.baseUrl)).filter(Boolean))];
    ...
    while (queue.length && visited.size < maxCatalogPages && entries.length < maxDiscoveredUrls) {
      ...
      for (const entry of pageEntries) {
        ...
        if (queue.length + visited.size < maxCatalogPages && isLikelyCatalogIndexEntry(entry) && !visited.has(entry.url)) {
  ...
  async function discoverProductUrls(source, candidate) {
    ...
    if (!matches.length && source.searchUrlTemplate) {
      ...
      if (searchUrl) {
        ...
        if (result.ok) matches = extractHtmlLinkEntries(result.text, result.url).filter(entry => entryMatchesCandidate(entry, candidate));
      }
    }

    if (!matches.length) {
      const sitemapLinks = await loadSitemapLinks(source, state);
      matches = sitemapLinks.filter(url => urlMatchesCandidate(url, candidate)).map(url => ({ url, text: '' }));
    }

    return [...new Map(matches
      .sort((left, right) => discoveryScore(right, candidate) - discoveryScore(left, candidate))
      .map(entry => [entry.url, entry.url])).values()].slice(0, maxProductPages);
  }
  ...
  async function verifyCandidate(candidate) {
    ...
    let bestFailure = {
      status: 'not_found', sourceType: sources[0].sourceType, sourceUrl: '', httpStatus: null,
  ...
  async function verifyStoredSource(product) {
    if (!product?.sourceUrl) {
      return {
        status: 'unsupported', sourceType: product?.sourceType || '', sourceUrl: '', httpStatus: null,
    ...
    if (!page.ok) {
      return {
        status: page.status === 404 || page.status === 410 ? 'not_found' : 'error',
        sourceType: product.sourceType || '', sourceUrl: product.sourceUrl, httpStatus: page.status || null,
        message: page.error || `http_${page.status}`
      };
    }
