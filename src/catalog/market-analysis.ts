...
function condition(facts: readonly string[]): MarketCondition {
  ...
  return selected.length > 1
    ? "mixed"
    : (selected[0] ?? (facts.includes("used") ? "used" : "unknown"));
}
function saleUnit(facts: readonly string[]): MarketSaleUnit {
  const units = (["single", "pair", "set"] as const).filter((unit) =>
    facts.includes(`sale_${unit}`),
  );
...
function band(quotes: readonly Quote[]): MarketPriceBand {
  ...
  return {
    ...
    median_yen: enough
      ? Math.round((prices[middle] + prices[Math.floor((prices.length - 1) / 2)]) / 2)
      : null,
    min_yen: enough ? prices[0] : null,
    max_yen: enough ? prices.at(-1)! : null,
...
export function calculateMarketAnalysis(
  ...
  const groups = new Map<
    string,
    { condition: MarketCondition; sale_unit: MarketSaleUnit; quotes: Quote[] }
  >();
  for (const offer of offers) {
    ...
    if (
      ...
      observed < now.getTime() - 90 * DAY
    )
      continue;
    const evidence = facts.get(offer.id) ?? [];
    const c = condition(evidence),
      unit = saleUnit(evidence),
      key = `${c}:${unit}`;
    const group = groups.get(key) ?? { condition: c, sale_unit: unit, quotes: [] };
    group.quotes.push({
      id: offer.id,
      shop: offer.shop_key,
      price: offer.price_yen,
      at: new Date(observed).toISOString(),
    });
  ...
  const valid = samples.filter(
    (sample) =>
      ...
      Date.parse(sample.observed_at) <= now.getTime(),
  );
  ...
  return {
    ...
    current_conditions: [...groups.entries()]
      .sort(([a], [b]) => a.localeCompare(b))
      .map(([, group]) => ({
        condition: group.condition,
        sale_unit: group.sale_unit,
        ...band(group.quotes),
      })),
    months: months.map((month) => {
      ...
      for (const sample of valid) {
        ...
        if (sample.sample_kind === "asking" && sample.price_yen !== null) {
          ...
          if (
            !before ||
            Date.parse(sample.observed_at) > Date.parse(before.observed_at) ||
            (Date.parse(sample.observed_at) === Date.parse(before.observed_at) &&
              sample.id > before.id)
          )
            asking.set(sample.listing_product_id, sample);
        }
      ...
      return {
        month,
        ...band(
          [...asking.values()].map((sample) => ({
            id: sample.listing_product_id,
            shop: sample.shop_key,
            price: sample.price_yen!,
            at: new Date(sample.observed_at).toISOString(),
          })),
        ),
        first_observed_listings: [...firstAsking.values()].filter((value) => value === month)
          .length,
        sold_out_listings: sold.size,
        deactivated_listings: removed.size,
      };
    }),
  };
}
