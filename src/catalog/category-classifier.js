...
function normalizedEvidence(evidence = []) {
  return evidence.map(item => {
    ...
    const categoryId = values.map(value => categoryIdForClassification(value)).find(Boolean) || null;
    return {
      categoryId,
...
function classified(categoryId, tierEvidence) {
  ...
  const sources = [...new Set(tierEvidence.filter(item => item.categoryId === categoryId).map(item => item.source))];
  return {
