const RULES = [
  ...
  ['other_accessory', /\baccessor(?:y|ies)\b|insulator|インシュレータ(?:ー)?|アクセサリ(?:ー)?|hdmi\s*(?:switcher|switch)|hdmiスイッチャー|dust\s*cover|ダストカバー/i],
  ['vacuum_tube', /vacuum\s+tube|真空管/i],
  ['rack', /audio\s+rack|オーディオラック/i],
  ['soundbar', /\bsound\s*bars?\b|サウンドバー/i],
  ['av_receiver', /\bav\s+(?:receiver|amplifier)\b|av(?:サラウンド)?(?:レシーバー|アンプ)|\bavr[-\s]?[a-z0-9]/i],
  ...
  ['clock_generator', /master\s+clock\s+generator|clock\s+generator|マスタークロック|クロックジェネレータ(?:ー)?/i],
  ['equalizer', /voicing\s+equalizer|graphic\s+equalizer|\bequalizer\b|音場補正|イコライザ(?:ー)?/i],
  ['crossover', /frequency\s+dividing\s+network|channel\s+divider|\bcrossover\b|チャンネル(?:デバイダ|ディバイダ)(?:ー)?|周波数分割/i],
  ['tuner', /(?:dds\s+)?(?:fm|am\s*\/\s*fm)\s+stereo\s+tuner|\btuner\b|チューナー/i],
