...
const CATALOG_PRIMARY = "(SELECT category_id FROM knowledge_catalog_product_categories WHERE product_id = p.id AND is_primary = 1 LIMIT 1)";
...
function valuesFor(change: AdminCsvChange): AdminCsvValues {
  return Object.fromEntries(ADMIN_CSV_FIELDS[change.original.kind].map((field) => [
    field,
    change.values[field] === change.original.values[field]
      ? change.values[field] : change.values[field].trim(),
  ]));
}
...
function result(
  change: AdminCsvChange, status: AdminCsvResult["status"], message: string,
  extra: Partial<AdminCsvResult> = {},
): AdminCsvResult {
  return { line: change.line, id: change.original.id, kind: change.original.kind, status, message, ...extra };
}

async function loadState(db: ReadableDatabase, kind: AdminCsvKind, id: number): Promise<State | null> {
...
function transactionGuard(
  db: QueryableDatabase, kind: AdminCsvKind, id: number, revision: string,
): D1PreparedStatement {
  ...
  return db.prepare(`SELECT json(CASE WHEN
    ...
    THEN 'true' ELSE 'csv_import_conflict' END)`).bind(id, revision);
}

async function pendingReceipt(
  db: ReadableDatabase, change: AdminCsvChange, values: AdminCsvValues,
): Promise<Receipt | null> {
  return firstMeasured<Receipt>(db.prepare(`SELECT * FROM admin_csv_import_changes
    WHERE target_kind = ? AND target_id = ? AND status = 'pending' AND after_json = ?
    LIMIT 1`).bind(change.original.kind, change.original.id, JSON.stringify(values)));
}
...
async function invalidReason(
  ...
  for (const field of ADMIN_CSV_FIELDS[kind]) {
    ...
    if (value.includes("[truncated]")) return "省略された値は更新できません。元情報を確認してください。";
    if (field === "manufacturer_id") {
      ...
      const manufacturer = await firstMeasured(db.prepare(`SELECT id FROM knowledge_catalog_manufacturers
        WHERE id = ? AND verification_status = 'verified'`).bind(value));
    ...
    } else if (value.length > 200 || (value && !normalizeIdentityModel(value)) ||
      (kind === "catalog" && !normalizeCatalogModel(value))) {
      return "型番は有効な文字列を200文字以内で入力してください。";
    }
  }
  if (kind === "catalog" &&
      (values.manufacturer_id !== before.manufacturer_id || values.canonical_model !== before.canonical_model)) {
    const duplicate = await firstMeasured<{ id: number }>(db.prepare(`SELECT id FROM knowledge_catalog_products
      WHERE manufacturer_id = ? AND normalized_model = ? AND id <> ? LIMIT 1`)
      .bind(values.manufacturer_id, normalizeCatalogModel(values.canonical_model), change.original.id));
    if (duplicate) return "同じメーカー・型番のカタログ #" + duplicate.id + " が存在します。統合画面で確認してください。";
  }
  return null;
}

export async function previewAdminCsvChange(
  db: ReadableDatabase, change: AdminCsvChange,
  ...
  if (same(change.original.kind, current, values)) {
    ...
    if (pending) return result(change, "pending", "データは更新済みです。関連商品の反映を再開できます。", {
      operationId: pending.operation_id, revision: pending.revision,
    });
    return result(change, "unchanged", "現在のデータと一致しています。");
  }
  if (!same(change.original.kind, current, change.original.values)) {
    return result(change, "conflict", "CSV出力後にデータが変更されています。最新のCSVで再確認してください。");
  }
  const invalid = await invalidReason(db, change, values);
  if (invalid) return result(change, "invalid", invalid);
  return result(change, "ready", "更新できます。", { revision: await revisionToken(state.revision) });
}

function receiptStatement(
  db: QueryableDatabase, input: AdminCsvApplyInput, values: AdminCsvValues, now: string,
): D1PreparedStatement {
  const { original } = input.change;
  // Preserve removed alias/source evidence with the before-image, inside the atomic write.
  const before = original.kind === "catalog"
    ...
    : "json_object('values', json(?))";
  const parameters: (string | number)[] = [input.operationId, original.kind, original.id,
  ...
  return db.prepare(`INSERT INTO admin_csv_import_changes(
    operation_id, target_kind, target_id, before_json, after_json, revision, status, created_at, updated_at
  ) VALUES (?, ?, ?, ${before}, ?, ?, 'pending', ?, ?)`).bind(...parameters);
}

async function updateCatalog(
  db: QueryableDatabase, input: AdminCsvApplyInput, values: AdminCsvValues, now: string, revision: string,
): Promise<void> {
  const { original } = input.change;
  const id = original.id;
  const before = original.values;
  const identityChanged = before.manufacturer_id !== values.manufacturer_id ||
  ...
  const statements = [
    ...
    db.prepare(`UPDATE knowledge_catalog_products
      ...
      WHERE id = ?`).bind(values.manufacturer_id, values.canonical_model,
      normalizeCatalogModel(values.canonical_model), values.canonical_name, values.lifecycle_status,
      now, now, id),
  ];
  if (before.primary_category_id !== values.primary_category_id) {
    statements.push(db.prepare("DELETE FROM knowledge_catalog_product_categories WHERE product_id = ?").bind(id));
    for (const category of catalogAdminCategoryIds(values.primary_category_id)) {
      statements.push(db.prepare(`INSERT INTO knowledge_catalog_product_categories(product_id, category_id, is_primary)
        VALUES (?, ?, ?)`).bind(id, category, category === values.primary_category_id ? 1 : 0));
    }
  }
  if (identityChanged) {
    statements.push(
      db.prepare("DELETE FROM knowledge_catalog_aliases WHERE product_id = ?").bind(id),
      db.prepare(`UPDATE knowledge_catalog_sources SET status = 'error', updated_at = ?
        WHERE product_id = ? AND status = 'active'`).bind(now, id),
      db.prepare(`UPDATE knowledge_catalog_candidates
        SET catalog_product_id = NULL, review_status = 'pending', updated_at = ?
        WHERE catalog_product_id = ?`).bind(now, id),
    );
  }
  statements.push(db.prepare(`INSERT INTO knowledge_catalog_sources(
...
async function resumeReceipt(
  db: QueryableDatabase, input: AdminCsvApplyInput, receipt: Receipt, now: string,
): Promise<AdminCsvResult> {
  const { change } = input;
  ...
  if (!state || !same(receipt.target_kind, JSON.parse(state.values_json) as AdminCsvValues, desired)) {
    return result(change, "conflict", "適用後に別の変更がありました。最新データを再確認してください。");
  }
  ...
  if (receipt.target_kind === "listing") {
    await refreshListingProjections(db, [{
      id: receipt.target_id, shop_key: state.shop_key, source_id: state.source_id,
    }], now);
    ...
    if (receipt.phase < 2) {
      ...
      if (target?.identityModels.length) {
        const rows = await db.prepare(`SELECT id, shop_key, source_id FROM products
          WHERE canonical_manufacturer_id = ? AND normalized_model IN (${target.identityModels.map(() => "?").join(",")})
            AND id > ? ORDER BY id LIMIT ?`)
          .bind(target.manufacturerId, ...target.identityModels, receipt.after_listing_id, REPLAY_PAGE_SIZE)
          .all<{ id: number; shop_key: string; source_id: string }>();
        selected = rows.results || [];
      }
    }
    const before = (JSON.parse(receipt.before_json) as { values: AdminCsvValues }).values;
    const identityChanged = before.manufacturer_id !== desired.manufacturer_id ||
      normalizeCatalogModel(before.canonical_model) !== normalizeCatalogModel(desired.canonical_model);
    if (!identityChanged && receipt.phase === 0 &&
        before.primary_category_id !== desired.primary_category_id) {
      await propagateCatalogCategoryToMatchedListings(
        db, receipt.target_id, catalogAdminCategoryIds(desired.primary_category_id), now,
        selected.map((row) => row.id),
      );
    } else if (identityChanged) {
      await replayAdminCsvListings(db, selected.map((row) => row.id), now);
    } else {
    ...
    await db.prepare(`UPDATE admin_csv_import_changes SET phase = ?, after_listing_id = ?, updated_at = ?
      WHERE operation_id = ? AND status = 'pending' AND phase = ? AND after_listing_id = ?`)
      .bind(phase, cursor, now, receipt.operation_id, receipt.phase, receipt.after_listing_id).run();
    return result(change, "pending", "関連商品と検索表示を反映しています。", { operationId: receipt.operation_id });
  }
  await db.prepare(`UPDATE admin_csv_import_changes SET status = 'applied', updated_at = ?
    WHERE operation_id = ? AND status = 'pending'`).bind(now, receipt.operation_id).run();
  return result(change, "applied", "更新と検索表示への反映が完了しました。", { operationId: receipt.operation_id });
}

export async function applyAdminCsvChange(
  db: QueryableDatabase, input: AdminCsvApplyInput,
): Promise<AdminCsvResult> {
  const { change } = input;
  const values = valuesFor(change);
  const now = new Date().toISOString();
  try {
    let receipt = await firstMeasured<Receipt>(db.prepare("SELECT * FROM admin_csv_import_changes WHERE operation_id = ?")
      .bind(input.operationId));
    if (receipt) {
      if (receipt.target_kind !== change.original.kind || receipt.target_id !== change.original.id ||
      ...
      if (preview.revision !== input.revision) {
        return result(change, "conflict", "差分確認後にデータが変更されました。再確認してください。");
      }
      const state = await loadState(db, change.original.kind, change.original.id);
      ...
      if (change.original.kind === "listing") {
        const before = change.original.values;
        await updateListingAdminProduct(db, change.original.id, {
          ...(values.manufacturer_id !== before.manufacturer_id ? { manufacturerId: values.manufacturer_id } : {}),
          ...(values.model !== before.model ? { model: values.model } : {}),
          ...(values.primary_category_id !== before.primary_category_id ? { primaryCategoryId: values.primary_category_id } : {}),
        }, now, [
        ...
        await db.prepare(`UPDATE admin_csv_import_changes SET status = 'applied', updated_at = ?
          WHERE operation_id = ? AND status = 'pending'`).bind(now, input.operationId).run();
      ...
      receipt = await firstMeasured<Receipt>(db.prepare("SELECT * FROM admin_csv_import_changes WHERE operation_id = ?")
        .bind(input.operationId));
    }
    if (!receipt) return result(change, "failed", "更新結果を確認できません。再試行してください。");
    return await resumeReceipt(db, input, receipt, now);
  } catch (error) {
    console.error(JSON.stringify({ event: "admin_csv_import_failed", operationId: input.operationId,
      message: error instanceof Error ? error.message : String(error) }));
    const receipt = await firstMeasured(db.prepare("SELECT operation_id FROM admin_csv_import_changes WHERE operation_id = ?")
      .bind(input.operationId));
    if (receipt) return result(change, "failed", "一部反映済みです。同じCSVで再試行すると続きから再開します。", {
      operationId: input.operationId,
    });
    return result(change, "failed", "更新できませんでした。データ競合または利用上限の可能性があります。再確認してください。");
  }
}
