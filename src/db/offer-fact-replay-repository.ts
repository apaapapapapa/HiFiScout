...
function readState(db: ReadableDatabase) {
  return firstMeasured<ReplayRow>(db.prepare(`SELECT rule_version, after_id, max_product_id,
    scanned_count, active_count, coverage_json, started_at, updated_at, completed_at
    FROM product_offer_fact_replays WHERE rule_version = ?`).bind(OFFER_FACT_RULE_VERSION));
}
...
export async function stepOfferFactReplay(database: QueryableDatabase, now = new Date().toISOString()) {
  const usage = accountReads(database);
  const db = usage.db;
  let state = await readState(db);
  if (!state) {
    await db.prepare(`INSERT INTO product_offer_fact_replays
      ...
      .bind(OFFER_FACT_RULE_VERSION, now, now, OFFER_FACT_RULE_VERSION).run();
  ...
  const rows = await db.prepare(`SELECT id, shop_key, source_id, substr(title,1,4000) AS title,
    ...
    .bind(state.after_id, state.max_product_id, OFFER_FACT_REPLAY_BATCH_SIZE).all<Listing>();
  ...
  if (rows.results.length && !listings.length) throw new Error("offer_fact_replay_listing_too_large");
  const complete = listings.length === rows.results.length &&
    (rows.results.length < OFFER_FACT_REPLAY_BATCH_SIZE || listings.at(-1)?.id === state.max_product_id);
  ...
  for (const listing of listings) {
    ...
    const groups = new Set(facts.map((fact) => OFFER_FACT_DEFINITIONS.find((definition) => definition.id === fact.factId)!.group));
  ...
  const claim = db.prepare(`UPDATE product_offer_fact_replays SET
    ...
    WHERE rule_version = ? AND after_id = ? AND completed_at IS NULL
      ...
      ))`).bind(afterId, listings.length, active, JSON.stringify(coverage), token, now,
        complete ? now : null, OFFER_FACT_RULE_VERSION, state.after_id, JSON.stringify(listings));
  await db.batch([claim, ...listings.flatMap((listing) => sellerOfferFactWrites(
    db, listing.shop_key, listing.source_id, listing.title, listing.condition_text,
    listing.last_seen_at, { ruleVersion: OFFER_FACT_RULE_VERSION, token },
  ))]);
  const result = await readOfferFactReplay(db);
  console.log(JSON.stringify({ event: "offer_fact_replay_step", afterId: result?.afterId,
    scannedCount: result?.scannedCount, rowsRead: usage.rowsRead(), rowsWritten: usage.rowsWritten(),
