...
export async function syncProductMetadata(db, shopKey, products, observedAt) {
  const rows = await existingMetadata(db, shopKey, products.map(product => product.sourceId));
  ...
  for (const product of products) {
    ...
    writes.push(
      db.prepare('UPDATE products SET metadata_json = ?, last_changed_at = ? WHERE id = ?')
