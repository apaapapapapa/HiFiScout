import { categoryClosureIds, categoryIdForFilter, categorySearchAliases } from '../catalog/categories.js';
...
function productRow(row) {
  if (!row) return row;
  let categoryIds = [];
  try { categoryIds = JSON.parse(row.category_ids || '[]'); }
  catch { categoryIds = row.primary_category_id ? [row.primary_category_id] : []; }
...
export async function selectProductsForHistory(db, shopKey, sourceIds, chunkSize = LOOKUP_CHUNK_SIZE) {
  ...
  for (let i = 0; i < uniqueIds.length; i += chunkSize) {
    ...
    const result = await db.prepare(`SELECT id, source_id, price_yen FROM products WHERE shop_key = ? AND source_id IN (${placeholders})`).bind(shopKey, ...chunk).all();
...
export async function selectActiveProductSourceIds(db, shopKey) {
  const result = await db.prepare('SELECT source_id FROM products WHERE shop_key = ? AND is_active = 1').bind(shopKey).all();
...
export async function deactivateProductsBySourceIds(db, shopKey, sourceIds, chunkSize = LOOKUP_CHUNK_SIZE) {
  ...
  for (let i = 0; i < uniqueIds.length; i += chunkSize) {
    ...
    if (!chunk.length) continue;
    const placeholders = chunk.map(() => '?').join(',');
    statements.push(db.prepare(`UPDATE products SET is_active = 0 WHERE shop_key = ? AND is_active = 1 AND source_id IN (${placeholders})`).bind(shopKey, ...chunk));
  }
...
function listingChanged(existing, product) {
  ...
  return existing.manufacturer !== product.manufacturer ||
    previous.rawManufacturer !== current.rawManufacturer || previous.manufacturerId !== current.manufacturerId ||
    existing.model !== product.model || existing.title !== product.title || existing.category !== product.category ||
    previous.rawCategory !== current.rawCategory || previous.primaryCategoryId !== current.primaryCategoryId ||
...
async function rowsForSources(db, shopKey, sourceIds) {
  ...
  for (let i = 0; i < sourceIds.length; i += LOOKUP_CHUNK_SIZE) {
    ...
    if (!chunk.length) continue;
    const placeholders = chunk.map(() => '?').join(',');
    const result = await db.prepare(`SELECT id, source_id FROM products WHERE shop_key = ? AND source_id IN (${placeholders})`).bind(shopKey, ...chunk).all();
    rows.push(...(result.results || []));
  }
  return rows;
}
...
async function syncProductCategories(db, shopKey, products, sourceIds) {
  ...
  for (const product of selected) {
    ...
    for (const categoryId of categoryClosureIds(fields.primaryCategoryId)) {
      statements.push(db.prepare('INSERT OR IGNORE INTO product_categories(product_id, category_id) VALUES (?, ?)').bind(productId, categoryId));
    }
  }
...
async function syncProductFeatureFacts(db, shopKey, products, sourceIds, observedAt) {
  ...
  for (const product of selected) {
    ...
    statements.push(db.prepare("DELETE FROM product_feature_facts WHERE product_id = ? AND source = 'title'").bind(productId));
    for (const fact of facts) {
      statements.push(db.prepare(`
        INSERT OR REPLACE INTO product_feature_facts(product_id, feature_id, state, source, confidence, verified_at)
        VALUES (?, ?, ?, ?, ?, ?)
      `).bind(productId, fact.featureId, fact.state, fact.source, fact.confidence, fact.verifiedAt || observedAt));
    }
  }
  await runBatches(db, statements);
}

export async function upsertProducts(db, shopKey, products, observedAt, { deactivateMissing = false, touchIntervalMinutes = 1440 } = {}) {
  ...
  const missingSourceIds = deactivateMissing ? (await selectActiveProductSourceIds(db, shopKey)).filter(sourceId => !observedSourceIds.has(sourceId)) : [];
  const newSourceIds = [];
  ...
  await syncProductFeatureFacts(db, shopKey, products, [...new Set(featureSyncSourceIds)], observedAt);
  ...
  if (historySourceIds.length) {
    ...
    const historyWrites = rows.filter(row => row.price_yen != null)
  ...
  const deactivatedCount = missingSourceIds.length ? await deactivateProductsBySourceIds(db, shopKey, missingSourceIds) : 0;
...
function ftsPhrase(value) { return `"${value.replaceAll('"', '""')}"`; }
...
function addCursorPredicate(where, binds, sort, cursor) {
  ...
  const idOp = sort.idDirection === 'DESC' ? '<' : '>';
  if (cursor.isNull) { where.push(`(p.price_yen IS NULL AND p.id ${idOp} ?)`); binds.push(cursor.id); return; }
...
function cursorFor(row, sort) {
  return encodeCursor({ sort: sort.key, id: row.id, value: row[sort.column], isNull: sort.price ? row.price_yen == null : false });
}

function requestedFeatures(params) {
  return [...new Set(params.getAll('feature').flatMap(value => value.split(',')).map(value => value.trim()).filter(Boolean))];
}
...
export function validateProductQuery(url) {
  ...
  for (const value of params.getAll('feature')) if ([...value].length > 200) return 'feature_too_long';
...
export async function listProducts(db, url) {
  ...
  if (q) {
    ...
    if (terms.length === 1 && [...q].length >= 3) {
      ...
      for (const value of terms) {
        where.push(`(p.title LIKE ? OR p.manufacturer LIKE ? OR p.raw_manufacturer LIKE ? OR p.model LIKE ? OR p.category LIKE ? OR p.raw_category LIKE ? OR p.search_aliases LIKE ?)`);
        const term = `%${value}%`;
  ...
  if (manufacturer) { where.push('(p.manufacturer_id = ? OR p.manufacturer = ?)'); binds.push(manufacturerIdForFilter(manufacturer), manufacturer); }
  ...
  if (category) {
    ...
    } else { where.push('p.category = ?'); binds.push(category); }
  }
  for (const feature of requestedFeatures(params)) {
    where.push("EXISTS (SELECT 1 FROM product_feature_facts pff WHERE pff.product_id = p.id AND pff.feature_id = ? AND pff.state = 'present')");
    binds.push(feature);
  }
  ...
  const orderBy = sort.price ? `p.price_yen ${sort.direction} NULLS LAST, p.id ${sort.idDirection}` : `p.${sort.column} ${sort.direction}, p.id ${sort.idDirection}`;

  let totalCount = null;
  if (includeTotal) {
    const countResult = await db.prepare(`SELECT COUNT(*) AS total FROM products p ${join} WHERE ${countWhere.join(' AND ')}`).bind(...countBinds).all();
  ...
  const result = await db.prepare(`SELECT p.* FROM products p ${join} WHERE ${where.join(' AND ')} ORDER BY ${orderBy} ${paginationSql}`).bind(...binds, ...paginationBinds).all();
  const rows = result.results || [];
  const hasMore = rows.length > limit;
  const items = (hasMore ? rows.slice(0, limit) : rows).map(productRow);
  const last = items.at(-1);
  return { items, hasMore, nextCursor: hasMore && last ? cursorFor(last, sort) : null, ...(includeTotal ? { totalCount, totalPages: Math.ceil(totalCount / limit) } : {}) };
}
