...
export async function readModelFactsAdmin(
  db: ReadableDatabase,
  productId: number,
): Promise<ModelFactsAdminSnapshot | null> {
  const product = await firstMeasured<{ id: number; name: string; manufacturerId: string }>(
    db
      .prepare(
  ...
  const sources = await db
    .prepare(`SELECT id,source_type AS sourceType,source_url AS url,status,retrieved_at AS retrievedAt
    FROM knowledge_catalog_sources WHERE product_id=? ORDER BY id LIMIT 40`)
    .bind(productId)
    .all<ModelFactsAdminSnapshot["sources"][number]>();
  const audits = await db
    .prepare(`SELECT id,actor,occurred_at,before_json,after_json
...
export async function saveModelFactsAdmin(
  db: QueryableDatabase,
  productId: number,
  input: ModelFactWriteInput,
  actor: string,
) {
  await saveModelFact(db, productId, input.fact, {
    id: input.id ?? undefined,
    expectedVersion: input.expectedVersion ?? undefined,
    reverify: input.reverify,
    actor,
  });
