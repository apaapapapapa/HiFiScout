...
export async function loadMarketAnalysis(
  ...
  id: number,
): Promise<ProductMarketAnalysis | null> {
  const row = await firstMeasured<{ analysis_json: string }>(
    db
      .prepare(`
  ...
  if (!row) return null;
  try {
    return JSON.parse(row.analysis_json) as ProductMarketAnalysis;
  } catch {
    return null;
  }
}
...
async function refreshOne(db: QueryableDatabase, id: number, now: Date): Promise<boolean> {
  return withinD1Budget(db, 6, async () => {
    ...
    const claim = await db
      .prepare(
        "UPDATE catalog_market_dirty SET claim_token=?,claimed_at=? WHERE catalog_product_id=? AND claim_token IS NULL",
      )
      .bind(token, now.toISOString(), id)
      .run();
    if (!claim.meta.changes) return false;
    const offers = await db
      .prepare(`SELECT p.id,p.shop_key,p.price_yen,p.stock_status,p.is_active,p.last_seen_at
      ...
      .bind(id, MAX_OFFERS + 1)
      .all<MarketOffer>();
    const samples = await db
      .prepare(`SELECT id,listing_product_id,shop_key,price_yen,sample_kind,signal_kind,observed_at
      FROM knowledge_catalog_price_index_samples WHERE catalog_product_id=? LIMIT ?`)
      .bind(id, MAX_SAMPLES + 1)
      .all<MarketSample>();
    const limited = offers.results.length > MAX_OFFERS || samples.results.length > MAX_SAMPLES;
    const evidence = limited
      ? new Map<number, OfferFact[]>()
      : await effectiveOfferFacts(
          db,
          offers.results.map((offer) => offer.id),
        );
    const facts = new Map(
      [...evidence].map(([key, values]) => [
        key,
        values.filter((fact) => fact.state === "present").map((fact) => fact.factId),
      ]),
    );
    ...
    const result = await db.batch([
      db
        .prepare(`INSERT INTO catalog_market_analysis(catalog_product_id,analysis_json,next_refresh_at)
      ...
      db
        .prepare("DELETE FROM catalog_market_dirty WHERE catalog_product_id=? AND claim_token=?")
        .bind(id, token),
    ]);
...
export async function maintainMarketAnalysis(db: QueryableDatabase, now = new Date()) {
  ...
  await work
    .prepare(`WITH due AS MATERIALIZED (
  ...
  await work
    ...
    .bind(new Date(now.getTime() - 60 * 60 * 1000).toISOString(), PAGE_SIZE)
    .run();
  const page = await work
    .prepare(`SELECT catalog_product_id FROM catalog_market_dirty
    WHERE claim_token IS NULL ORDER BY queued_at,catalog_product_id LIMIT ?`)
    .bind(PAGE_SIZE)
    .all<{ catalog_product_id: number }>();
  let refreshed = 0;
  for (const item of page.results)
    if (await refreshOne(work, item.catalog_product_id, now)) refreshed++;
  const result = { selected: page.results.length, refreshed };
  console.log(
    JSON.stringify({
      event: "condition_market_analysis_d1_usage",
      ...result,
