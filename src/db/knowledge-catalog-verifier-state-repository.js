...
export async function claimKnowledgeCatalogVerifierVersion(db, version, startedAt) {
  ...
  const result = await db.prepare(`
    ...
    VALUES (?, 'running', ?)
  `).bind(normalizedVersion, startedAt).run();
  return Number(result?.meta?.changes || 0) > 0;
}

export async function finishKnowledgeCatalogVerifierVersionSuccess(db, version, finishedAt, message = '') {
  await db.prepare(`
    UPDATE knowledge_catalog_verifier_state
    SET status = 'success', finished_at = ?, message = ?
    WHERE version = ?
  `).bind(finishedAt, String(message || '').slice(0, 1000), version).run();
}

export async function finishKnowledgeCatalogVerifierVersionFailure(db, version, finishedAt, message = '') {
  await db.prepare(`
    UPDATE knowledge_catalog_verifier_state
