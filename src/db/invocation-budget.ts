...
export const D1_FINALIZATION = Symbol("d1Finalization");

type BudgetedDatabase = QueryableDatabase & {
  [D1_WORK_UNIT]?: <T>(calls: number, work: () => Promise<T>) => Promise<T>;
  [D1_FINALIZATION]?: <T>(work: () => Promise<T>) => Promise<T>;
};
...
/** Finish bookkeeping or close an incomplete dispatch using reserved, still-metered calls. */
export function withD1Finalization<T>(db: QueryableDatabase, work: () => Promise<T>): Promise<T> {
  return (db as BudgetedDatabase)[D1_FINALIZATION]?.(work) ?? work();
}
...
export function invocationBudget(
  ...
  const checkAllowance = (requiredCalls: number) => {
    if (finalizationDepth) {
      ...
      if (requiredCalls > maxCalls - calls) throw new InvocationBudgetExceeded("d1_calls");
      return;
    }
    if (requiredCalls > workLimit - calls) yieldReason = "d1_calls";
  ...
  return {
    db: {
      ...
      async [D1_FINALIZATION]<T>(work: () => Promise<T>): Promise<T> {
        finalizationDepth += 1;
        try {
          return await work();
        } finally {
    ...
    remainingCalls: () => Math.max(0, workLimit - calls),
    exhausted: () => yieldReason !== null || calls >= workLimit || clock() - started >= maxWallMs,
    defer: () => {
