...
function proof(row: ReviewedModelFact): CatalogRelationProof {
  ...
  try {
    ...
    /* A manual audit may have no public URL. */
  }
  return {
    kind: row.evidence_kind,
    sourceUrl: row.evidence_kind === "source" ? sourceUrl : null,
...
function model(row: ReviewedModelFact, inverse = false): RelatedCatalogModel {
  return {
    key: `c-${inverse ? row.related_product_id : row.product_id}`,
    manufacturer: (inverse ? row.related_manufacturer : row.manufacturer) || "",
    model: (inverse ? row.related_product_name : row.product_name) || "",
    proof: proof(row),
  };
}
...
export async function publicModelRelations(
  db: ReadableDatabase,
  productId: number,
  now = new Date().toISOString(),
): Promise<ProductModelRelations | undefined> {
  ...
  return {
    links: verified
      .filter((fact) => fact.related_product_id !== null)
      .map((fact) => ({
        ...model(fact, fact.product_id === productId),
        kind:
          fact.relation_type === "variant"
            ? "variant"
    ...
    families: memberships.map((membership) => ({
      ...
      members: familyMembers
        .filter((member) => member.family_id === membership.family_id)
        .map((member) => ({ ...model(member), position: member.position })),
    })),
  };
}
