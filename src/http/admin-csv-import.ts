...
export function parseAdminCsvChange(value: unknown): AdminCsvChange | null {
  ...
  if (
    ...
    !Number.isSafeInteger(original.id) || Number(original.id) <= 0 ||
  ...
  for (const values of [original.values, value.values]) {
    ...
    if (!fields.every((field) =>
      typeof values[field] === "string" && values[field].length <= 4096 &&
      !/[\u0000-\u001f\u007f]/u.test(values[field])
    )) return null;
  }
...
export function parseAdminCsvPreview(value: unknown): AdminCsvChange[] | null {
  ...
  return new Set(valid.map(({ original }) => original.kind + ":" + original.id)).size === valid.length
    ? valid : null;
}
...
export function parseAdminCsvApply(value: unknown): AdminCsvApplyInput | null {
  ...
  if (
    !change || typeof value.revision !== "string" || value.revision.length > 16_384 ||
    typeof value.operationId !== "string" ||
    !/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu.test(value.operationId)
  ) return null;
