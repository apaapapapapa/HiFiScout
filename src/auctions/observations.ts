...
export function auctionInstant(value: unknown): string | null {
  ...
  if (year < 2000 || month < 1 || month > 12 || day < 1 || day > days[month - 1] || hour > 23 || minute > 59 || second > 59) return null;
  const zone = match[7];
  if (zone !== "Z") {
...
export function auctionObservationStamp(value: unknown): AuctionObservationStamp | null {
  ...
  if (typeof generation !== "number" || !Number.isSafeInteger(generation) || generation < 0 ||
      typeof sequence !== "number" || !Number.isSafeInteger(sequence) || sequence < 0 || !observedAt) return null;
  return { generation, sequence, observedAt };
}

export function emptyAuctionLiveFacts(): AuctionLiveFacts {
  return { currentPrice: null, buyNowPrice: null, bidCount: null, scheduledEndAt: null,
    startedAt: null, sourceState: null, outcome: null, shipping: null };
}
...
export type AuctionApplyResult =
  ...
  | { status: "stale" | "duplicate" | "identity_mismatch" | "reopening_unconfirmed" | "invalid"; snapshot: AuctionSnapshot | null };

/** Merge only actually observed fields, preserving their original evidence times. */
export function applyAuctionObservation(previous: AuctionSnapshot | null, next: AuctionObservation): AuctionApplyResult {
  const stamp = auctionObservationStamp(next.stamp);
  if (!stamp || Object.values(next.live).some((fact) => fact !== null && fact.observedAt !== stamp.observedAt)) {
    return { status: "invalid", snapshot: previous };
  }
  if (!previous) return { status: "applied", snapshot: { ...next, stamp, cycle: 1 } };
  if (previous.source !== next.source || previous.auctionId !== next.auctionId || previous.sourceUrl !== next.sourceUrl) {
    return { status: "identity_mismatch", snapshot: previous };
  }
  if (stamp.generation === previous.stamp.generation && stamp.sequence === previous.stamp.sequence) {
  ...
  const reopening = previous.live.sourceState?.value === "ended" && next.live.sourceState?.value === "open";
  if (reopening) {
    const start = next.live.startedAt?.value;
    const confirmedEnd = previous.live.sourceState?.observedAt;
    if (!start || !confirmedEnd || !auctionInstant(start) || start <= confirmedEnd || start > stamp.observedAt) {
  ...
  const live: AuctionLiveFacts = {
    ...
    sourceState: previous.live.sourceState?.value === "ended" ? previous.live.sourceState : next.live.sourceState ?? previous.live.sourceState,
...
export function auctionPresentation(snapshot: AuctionSnapshot, now: string, maxAgeMs: number) {
  ...
  if (!instant || !Number.isSafeInteger(maxAgeMs) || maxAgeMs < 0) throw new Error("Invalid auction freshness parameters");
  ...
  const phase = state?.value === "ended" || state?.value === "unavailable" ? state.value :
    end && end <= instant ? "end_check_pending" : state?.value ?? "unknown";
  return { phase, freshness, sourceState: state?.value ?? "unknown", outcome: snapshot.live.outcome?.value ?? "unknown" };
}
