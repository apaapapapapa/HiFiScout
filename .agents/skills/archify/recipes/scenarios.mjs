const RAW_RECIPES = [
  {
    ...
    en: {
      ...
      summary:
        "A bounded map of core components, external dependencies, primary paths, and trust boundaries.",
      useWhen:
        "Onboarding, design reviews, repository orientation, or explaining a service landscape.",
      avoidWhen:
        "The audience needs exact call order, state transitions, or row-level data lineage.",
      include: [
  ...
  },
  {
    ...
    en: {
      ...
      summary:
        "A deployment-focused map of regions, networks, clusters, workloads, stores, and cross-boundary mechanisms.",
      useWhen:
        "Cloud reviews, production readiness, multi-region planning, or infrastructure ownership handoffs.",
      avoidWhen:
        "Deployment facts are unknown or the real question is application behavior rather than placement.",
      include: [
  ...
  },
  {
    ...
    en: {
      ...
      summary:
        "A lane-based agent loop with policy gates, tool execution, exception recovery, evidence, and final response.",
      useWhen:
  ...
  },
  {
    ...
    en: {
      ...
      summary:
        "A delivery flow with build, checks, environments, approvals, smoke tests, rollback, and ownership lanes.",
      useWhen:
        "CI/CD design, release reviews, deployment governance, or onboarding developers to delivery.",
      avoidWhen:
        "The question is where infrastructure runs or what states a deployment object can occupy.",
      include: [
        "trigger and build",
        "blocking checks",
  ...
  },
  {
    ...
    en: {
      ...
      summary:
        "An operational workflow that separates signals, responders, mitigation, communications, and recovery proof.",
      useWhen: "Incident playbooks, on-call handoffs, reliability reviews, and tabletop exercises.",
      avoidWhen:
        "The audience needs live metrics or a post-incident component topology instead of response actions.",
      include: [
        "detection signal",
        "triage owner",
  ...
  },
  {
    ...
    en: {
      title: "API request chain",
      question: "Who calls whom, in what order, and what returns?",
      summary:
        "A time-ordered request path with authentication, cache fallback, persistence, return traffic, and async trace.",
      useWhen:
  ...
  },
  {
    ...
    en: {
      title: "Async roundtrip",
      question: "What happens after the initial request returns?",
      summary:
        "A sequence view of enqueue, acknowledgement, background work, callbacks, retries, timeout, and final consistency.",
      useWhen:
        "Webhooks, jobs, queues, payment callbacks, eventual consistency, or async API contracts.",
      avoidWhen:
        "The primary question is topic topology and consumer ownership rather than time order.",
      include: [
  ...
  },
  {
    ...
    en: {
      ...
      summary:
        "A governed path from sources through consent, transforms, sensitive stores, warehouse, and consumers.",
      useWhen:
        "Analytics architecture, ETL/ELT review, PII assessment, warehouse design, or model feature lineage.",
      avoidWhen:
        "The audience needs request timing or operational task ownership rather than data assets.",
      include: [
        "sources and assets",
        "transform stages",
  ...
  },
  {
    ...
    en: {
      title: "Event-stream topology",
      question: "Which events move through which topics, processors, groups, and failure paths?",
      summary:
        "A stream map of producers, topics, ordered processors, consumer groups, state, replay, and DLQ.",
      useWhen:
        "Kafka/event-platform design, stream processing reviews, ownership, replay, and failure handling.",
      avoidWhen:
        "Topic names, consumer groups, and delivery semantics are not known—use a generic workflow instead.",
      include: [
  ...
  },
  {
    ...
    en: {
      ...
      summary:
        "A state model with active work, waits, retries, cancellation, failure, and explicit terminal outcomes.",
      useWhen:
        "Tasks, orders, tickets, subscriptions, jobs, agent runs, or any durable object with status.",
      avoidWhen:
        "The object has no durable state and the real question is participant interaction over time.",
      include: [
  ...
  },
  {
    ...
    type: "lifecycle",
    proof: "deployment-lifecycle",
    presentation: { preset: "signal-flow", motion: "trace", views: "recommended" },
    signals: [
      ["deployment lifecycle", 15],
      ["release state", 10],
      ["promotion state", 9],
      ["approval status", 8],
      ["rollback state", 10],
    ...
    en: {
      ...
      summary:
        "A deployment state model covering queued, building, verifying, approval, promotion, rollback, and terminal outcomes.",
      useWhen:
        "Release controllers, GitOps reconciliation, environment promotion, or deployment status APIs.",
      avoidWhen:
        "The question is the human/CI sequence of delivery actions rather than the deployment object state.",
      include: [
...
export function recommendScenario(query, options = {}) {
  ...
  const ranked = SCENARIO_RECIPES.map((recipe) => scoreRecipe(recipe, query)).sort(
    (left, right) =>
      right.score - left.score ||
      SCENARIO_RECIPES.indexOf(left.recipe) - SCENARIO_RECIPES.indexOf(right.recipe),
  );
  const winner =
    ranked[0].score > 0 ? ranked[0] : { recipe: SCENARIO_RECIPES[0], score: 0, matched: [] };
...
export function formatScenarioList(lang = "en") {
  ...
  return [
    ...
    intro,
    "",
    ...listScenarioRecipes(lang).flatMap((recipe) => [
...
export function formatScenarioRecommendation(result) {
  ...
  const labels = isZh
    ...
    : {
        ...
        alternatives: "Other possible fits",
  ...
  if (result.alternatives.length) {
    lines.push(
      "",
      `${labels.alternatives}: ${result.alternatives.map((item) => `${item.title} [${item.type}]`).join(" · ")}`,
    );
  }
  return lines.join("\n");
}
