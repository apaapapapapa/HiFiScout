...
test("renderer-owned structure supplies truthful Semantic Passport context", () => {
  ...
  assert.match(
    architecture,
    /data-node-id="api"[^>]+data-node-kind="backend"[^>]+data-node-context="AWS Region: us-west-2 › sg-api :443\/:8000"/,
  );
  assert.match(
    workflow,
    /data-node-id="approval"[^>]+data-node-kind="security"[^>]+data-node-context="Policy Boundary › Plan \+ route"/,
  );
  assert.match(
    sequence,
    /data-node-id="redis"[^>]+data-node-kind="database"[^>]+data-node-context="Sequence participant"/,
  );
  assert.match(
    dataflow,
    /data-node-id="warehouse"[^>]+data-node-kind="database"[^>]+data-node-context="04 \/ Store"/,
  );
  assert.match(
    lifecycle,
    /data-node-id="executing"[^>]+data-node-kind="active"[^>]+data-node-context="Lifecycle phases"/,
  );
});
...
test("Relationship Lens renders one Semantic Passport and copyable stable focus link", () => {
  ...
  assert.match(
    html,
    /id="btn-focus-clear"[^>]+aria-label="Close semantic passport"[^>]+title="Close">&#215;<\/button>/,
  );
  assert.match(html, /id="btn-focus-copy"[^>]+aria-label="Copy link to focused node"/);
  assert.match(
    html,
    /id="btn-focus-relations"[^>]+aria-expanded="false"[^>]+aria-controls="relationship-lens-list"/,
  );
  ...
  assert.match(
    html,
    /compactOnMobile = mobile && chip\.getAttribute\('data-relations-expanded'\) !== 'true'/,
  );
  ...
  assert.match(
    html,
    /chip\.hidden \|\| !target \|\| typeof target\.closest !== 'function' \|\| chip\.contains\(target\)/,
  );
...
test("Node Finder searches and presents the same passport facts", () => {
  ...
  assert.match(
    html,
    /\[item\.type, item\.id, item\.sublabel, item\.tag\]\.filter\(Boolean\)\.join\(' \\u00b7 '\)/,
  );
  assert.match(
    html,
