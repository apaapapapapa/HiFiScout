...
test("package, lockfile, Skill metadata, escaped Shields badge, and public docs share one development identity", () => {
  ...
  try {
    ...
    assert.match(
      result.stderr,
      /SKILL\.md metadata version 2\.12 must map to package 2\.13\.0-dev\.0/,
    );
    assert.match(result.stderr, /README\.md must advertise development identity v2\.13\.0-dev\.0/);
    assert.match(
      result.stderr,
      /docs\/index\.html must advertise development identity v2\.13\.0-dev\.0/,
    );
    assert.match(
      result.stderr,
      /docs\/start\.html must advertise development identity v2\.13\.0-dev\.0/,
    );
  } finally {
    fs.rmSync(fixture, { recursive: true, force: true });
  }
...
test("landing proof receipt matches the current nine-check artifact contract", () => {
  ...
  try {
    writeValidDevelopmentFixture(fixture, {
      "docs/index.html":
        "<span>development · v2.13.0-dev.0 · 开发版 · 8/8 checks</span><p>Raven manual ZIP / ZIP 手动安装: extract archify.zip into ~/.raven/workspace/skills, which yields ~/.raven/workspace/skills/archify; 将 archify.zip 解压到 ~/.raven/workspace/skills，解压后会得到 ~/.raven/workspace/skills/archify; not an agent-switcher target.</p>",
    });
...
test("Raven stays a truthful manual ZIP install and never becomes a generated agent-switcher command", () => {
  ...
  try {
    ...
    assert.match(
      result.stderr,
      /Raven must remain a manual ZIP installation outside the agent switcher/,
    );
  } finally {
    fs.rmSync(fixture, { recursive: true, force: true });
  }
...
test("Raven instructions reject extracting the archive into the final Skill directory", () => {
  ...
  try {
    ...
    assert.match(
      result.stderr,
      /extract archify\.zip into ~\/\.raven\/workspace\/skills, yielding ~\/\.raven\/workspace\/skills\/archify/,
    );
  } finally {
    fs.rmSync(fixture, { recursive: true, force: true });
  }
...
test("renderer template generator carries the complete package prerelease identity", () => {
  ...
  try {
    ...
    assert.match(
      result.stderr,
      /archify\/assets\/template\.html generator must be archify 2\.13\.0-dev\.0/,
    );
  } finally {
    fs.rmSync(fixture, { recursive: true, force: true });
  }
...
test("roadmap current identity follows the package release state", () => {
  ...
  try {
    ...
    assert.match(
      result.stderr,
      /ROADMAP\.md must declare the current development line as v2\.13\.0-dev\.0/,
    );
  } finally {
    fs.rmSync(fixture, { recursive: true, force: true });
  }
...
test("generated public-page templates keep a development marker and version placeholder", () => {
  ...
  try {
    ...
    assert.match(
      result.stderr,
      /scripts\/gallery-template\.html must use \[\[ARCHIFY_VERSION\]\] with development and 开发版 labels/,
    );
  } finally {
    fs.rmSync(fixture, { recursive: true, force: true });
  }
