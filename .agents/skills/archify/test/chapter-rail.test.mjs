...
test("chapter rail delegates selection and mirrors the existing activeIndex owner", () => {
  ...
  assert.match(
    html,
    /activeIndex < 0 \? 'available' : \(current \? 'current' : \(index < activeIndex \? 'before' : 'after'\)\)/,
  );
...
test("chapter rail is keyboard-first and pauses playback on reader takeover", () => {
  ...
  assert.match(
    html,
    /chapterIndex\.addEventListener\('focusin',[\s\S]*if \(playing\) pausePlayback\(\)/,
  );
...
test("chapter rail has positional, touch, mobile, motion, embed, and print boundaries", () => {
  ...
  assert.match(
    html,
    /\.guided-view-chapter\[data-chapter-position="current"\][\s\S]*border: 2px solid/,
  );
  ...
  assert.match(
    html,
    /prefers-reduced-motion: reduce[\s\S]*\.guided-view-chapter \{ transition: none !important; \}/,
  );
