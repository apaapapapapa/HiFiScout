...
test("all five renderers inherit native Route Journey controls outside canonical SVG", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(
      html,
      /id="route-journey-controls" hidden role="group" aria-label="Route journey controls"/i,
      mode,
    );
    assert.match(html, /id="route-journey-prev"[^>]+aria-label="Previous route position"/i, mode);
    assert.match(
      html,
      /id="route-journey-play"[^>]+aria-label="Play route journey"[^>]+aria-pressed="false"/i,
      mode,
    );
    assert.match(html, /id="route-journey-next"[^>]+aria-label="Next route position"/i, mode);
    assert.match(
      html,
      /id="route-journey-overview"[^>]+aria-label="Show complete route overview"/i,
      mode,
    );
    assert.match(
      html,
      /document\.createElement\(options\.interactive === true \? 'button' : 'span'\)/,
      mode,
    );
    assert.doesNotMatch(
      canonicalSvg(html),
      /data-route-journey|route-journey-(?:flow|overlay)/,
      mode,
    );
  }
});
...
test("a position owns its exact ordered incoming edge while the full route remains authored truth", () => {
  ...
  assert.match(
    template,
    /activeEdges\.forEach\(function \(edge, step\) \{[\s\S]*?var destination = step \+ 1/,
  );
  ...
  assert.match(
    template,
    /journeyIndex > 0\) renderJourneyPulse\(activeEdges\[journeyIndex - 1\]\)/,
  );
...
test("route chips provide one roving tab stop, native activation, and manual ownership", () => {
  ...
  assert.match(
    template,
    /item\.setAttribute\('aria-label', 'Route position ' \+ \(index \+ 1\) \+ ' of ' \+ ids\.length/,
  );
  assert.match(
    template,
    /path\.addEventListener\('focusin'[\s\S]*?pauseJourney\(\{ preserveElapsed: true \}\)/,
  );
...
test("playback is explicit, finite, resumable, and never leaks position into the route URL", () => {
  ...
  assert.match(
    template,
    /journeyIndex >= activeNodeIds\.length - 1[\s\S]*?pauseJourney\(\{ complete: true/,
  );
  assert.match(
    template,
    /applyJourneyState\(journeyIndex \+ 1[\s\S]*?journeyElapsedMs = 0;[\s\S]*?scheduleJourney\(\)/,
  );
  assert.doesNotMatch(template, /journeyTimer\s*=\s*(?:window\.)?setInterval/);
  assert.match(
    template,
    /function playJourney\(\)[\s\S]*?if \(journeyIndex < 0\) applyJourneyState\(0/,
  );
  assert.match(
    template,
    /function syncFromHash\(\)[\s\S]*?choose\(parts\[1\], \{ updateUrl: false \}\)/,
  );
  assert.match(
    template,
...
test("motion, camera, layered Escape, mobile, print, and embed boundaries stay explicit", () => {
  assert.match(
    template,
    /Archify\.motionGovernor\.capable === true[\s\S]*?!Archify\.motionGovernor\.isPaused\(\)/,
  );
  assert.match(template, /Archify\.motionGovernor\.claim\('route'/);
  assert.match(
    template,
    /reason: 'route-journey',[\s\S]*?maxScale: 1\.65,[\s\S]*?padding: 64,[\s\S]*?duration: 360/,
  );
  assert.match(
    template,
  ...
  assert.match(
    template,
    /event\.target\.closest\('\.diagram-nav, \.focus-chip, \.node-finder, \.diagram-guide, \.overview-map, \.route-probe, \.semantic-lens'\)/,
  );
  assert.match(template, /reason: 'guide'/);
  assert.match(template, /window\.addEventListener\('beforeprint'[\s\S]*?pauseJourney/);
  assert.match(
    template,
    /function escapeRoute\(options\)[\s\S]*?return 'paused'[\s\S]*?return 'overview'[\s\S]*?return 'cleared'/,
  );
  ...
  assert.match(
    template,
    /@media \(max-width: 720px\)[\s\S]*?\.route-probe-node \{ min-height: 2\.75rem !important; \}/,
  );
  assert.match(template, /\.route-journey-controls button \{ min-height: 2\.75rem; \}/);
  assert.match(
    template,
    /@media print[\s\S]*?\.route-probe-overlay, \.route-journey-overlay \{ display: none !important; \}/,
  );
  assert.match(
    template,
    /svg\[data-route-active\] \[data-node-id\],[\s\S]*?filter: none !important/,
  );
  ...
  assert.match(
    template,
    /@media \(prefers-reduced-motion: reduce\)[\s\S]*?\.route-journey-overlay/,
  );
});
...
test("standalone export strips every journey attribute and transient pulse", () => {
  ...
  assert.match(
    template,
    /canonicalStateClean[\s\S]*?\[data-route-journey-overlay\][\s\S]*?\[data-route-journey-current\]/,
  );
});
