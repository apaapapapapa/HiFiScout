...
test("all typed renderers emit explicit context and fine reading-depth semantics", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.doesNotMatch(
      html,
      /<text[^>]*data-detail="(?:context|fine)"[^>]*class="t-primary"/,
      mode,
    );
...
test("semantic zoom is motion-safe and full-fidelity in print and export", () => {
  ...
  assert.match(
    html,
    /@media print \{[\s\S]+\.diagram-container svg \[data-detail\] \{[\s\S]+opacity: 1 !important/,
  );
  ...
  assert.match(
    html,
    /clone\.querySelectorAll\('[^']*\[data-detail\][^']*\[data-detail-anchor\][^']*'\)\.length === 0/,
  );
