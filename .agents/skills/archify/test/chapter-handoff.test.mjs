...
test("all five renderers inherit one viewer-only Shared Anchor Chapter Handoff", () => {
  for (const mode of Object.keys(CASES)) {
    ...
    assert.doesNotMatch(
      canonicalSvg(html),
      /data-chapter-handoff|data-chapter-role|chapter-handoff-anchor/,
      mode,
    );
  }
});
...
test("latest intent, manual takeover, Still, reduced motion, and hidden pages cleanly settle", () => {
  ...
  assert.match(
    html,
    /settleHandoff\(systemPaused \? 'reduced-motion' : \(hasSuspension\(\) \? 'hidden' : 'still'\)\)/,
  );
  assert.match(html, /settleHandoff\('hidden'\)/);
  assert.match(html, /settleHandoff\('reduced-motion'\)/);
  assert.match(
    html,
    /window\.addEventListener\('beforeprint',[\s\S]*?clearChapterPreview[\s\S]*?settleHandoff\('print'\)/,
  );
});
