...
for (const [mode, filename] of Object.entries(OFFICIAL_V1_EXAMPLES)) {
  test(`official v1 ${mode} baseline remains renderable and valid`, () => {
    const doc = JSON.parse(
      fs.readFileSync(path.join(__dirname, "fixtures/v1-baseline", filename), "utf8"),
    );
...
test("legacy v1 architecture auto viewBox accommodates all seven implicit auto legend kinds", () => {
  ...
  const viewBox = svg
    ...
    ?.slice(1)
    .map(Number);
  const baselines = [...svg.matchAll(/data-legend-baseline="([\d.]+)"/g)].map((match) =>
    Number(match[1]),
  );
  assert.deepEqual(
...
test("legacy v1 explicit narrow viewBoxes never hard-fail on an implicit auto legend", () => {
  ...
  for (const [mode, doc] of Object.entries(narrowExplicitViewBoxDocuments())) {
    ...
    assert.equal(
      (svg.match(/data-node-id=/g) || []).length,
      expectedNodeCounts[mode],
      `${mode}: topology must remain intact`,
    );
    if (mode === "lifecycle") {
      ...
      assert.doesNotMatch(
        svg,
        />Legend</,
        `${mode}: an unfit implicit legend should degrade without overlap`,
      );
    }
    const validated = validate(mode, doc);
