...
const CASES = [
  ...
  [
    "workflow: node sublabel wider than its legible minimum",
    "workflow",
    (d) => {
      d.nodes[0].sublabel = "This supporting sentence is much too long for one workflow node";
    },
    ["Sublabel", "legible", "increase node.width"],
  ],
  [
    "workflow: node tag wider than its legible minimum",
    "workflow",
    (d) => {
      d.nodes[0].tag = "This tag is far too long to sit inside one workflow node box";
    },
  ...
  [
    "workflow: phase ranges overlap",
    "workflow",
    (d) => {
      d.phases[2].fromCol = d.phases[1].toCol;
    },
    ["overlaps phase", "start at col 4 or later"],
  ],
  ...
  [
    "sequence: participant sublabel wider than its legible minimum",
    "sequence",
    (d) => {
      d.participants[0].sublabel =
        "This supporting sentence is far too long for one sequence participant";
    },
  ...
  [
    "dataflow: explicit via keeps every route segment orthogonal",
    "dataflow",
    (d) => {
      d.flows[0].via = [
        [195, 140],
  ...
  [
    "dataflow: node sublabel wider than its legible minimum",
    "dataflow",
    (d) => {
      d.nodes[0].sublabel = "This supporting sentence is far too long for one data-flow node box";
    },
    ["Sublabel", "legible", "increase node.width"],
  ],
  [
    "dataflow: node tag wider than its legible minimum",
    "dataflow",
    (d) => {
      d.nodes[0].tag = "This tag is far too long to sit inside one data-flow node box";
    },
    ["Tag", "legible", "increase node.width"],
  ],
  ...
  [
    "lifecycle: state sublabel wider than its legible minimum",
    "lifecycle",
    (d) => {
      d.states[0].sublabel = "This supporting sentence is far too long for one lifecycle state box";
    },
    ["Sublabel", "legible", "increase state.width"],
  ],
  [
    "lifecycle: state tag wider than its legible minimum",
    "lifecycle",
    (d) => {
      d.states[0].tag = "This tag is far too long to sit inside one lifecycle state box";
    },
  ...
  [
    "architecture: component sublabel wider than its legible minimum",
    "architecture",
    (d) => {
      d.components[0].sublabel =
        "This supporting sentence is far too long for one architecture component";
    },
    ["Sublabel", "legible", "widen size"],
  ],
  [
    "architecture: component tag wider than its legible minimum",
    "architecture",
    (d) => {
      d.components[0].tag = "This tag is far too long to sit inside one architecture component box";
    },
...
for (const [name, mode, mutate, expected] of CASES) {
  test(name, () => {
    ...
    assert.doesNotMatch(
      stderr,
      /TypeError|is not a function|Cannot read/,
...
const SHRINK_CASES = [
  ...
  [
    "architecture",
    (d) => {
      d.components[0].sublabel = "Browser and mobile apps";
    },
    9,
  ],
  [
  ...
  [
    "dataflow",
    (d) => {
      d.nodes[0].sublabel = "browser SDK and mobile SDK";
    },
    7,
  ],
  [
    "lifecycle",
    (d) => {
      d.states[0].sublabel = "request accepted and queued";
    },
    7,
  ],
];
...
for (const [mode, collection, tag, preferred] of TAG_SHRINK_CASES) {
  test(`${mode}: an over-long tag shrinks to fit instead of overflowing`, () => {
    ...
    const match = html.match(
      new RegExp(`<text data-detail="fine"[^>]*font-size="([\\d.]+)"[^>]*>${escapedTag}</text>`),
    );
...
test("architecture: Clean Flow Gate rejects a connection through a component", () => {
  ...
  assert.match(
    stderr,
    /\[clean-flow\/edge-through-node\] architecture connections\[0\] id "direct"/,
  );
...
test("dataflow: validator and SVG share the 27px CJK/emoji classification mask", () => {
  ...
  const group =
    html.match(
      /<g data-detail="context"[^>]*data-edge-id="web-clickstream"[^>]*>[\s\S]*?<\/g>/,
    )?.[0] || "";
  assert.match(group, /data-edge-label="写入🚀"/);
  assert.match(
    group,
    /<rect x="[^\"]+" y="[^\"]+" width="41\.4" height="27" rx="4" class="c-mask"\/>/,
  );
...
test("architecture: explicit via cannot run tangentially into an authored top port", () => {
  ...
  assert.match(
    stderr,
    /\[clean-flow\/endpoint-side-direction\] architecture connections\[0\] id "tasks-file"/,
  );
...
test("architecture: showcase rejects an unrelated shared route corridor", () => {
  ...
  assert.match(
    stderr,
    /connections\[0\] id "first" "a" -> "b" shares a 240px corridor with connections\[1\] id "second" "c" -> "d"/,
  );
...
test("architecture: route rhythm warns in standard and blocks a showcase micro segment", () => {
  ...
  assert.match(
    stderr,
    /\[composition\/micro-segment\] showcase architecture connections\[0\] id "tight"/,
  );
...
test("architecture: container border run is blocking in standard and showcase", () => {
  for (const profile of ["standard", "showcase"]) {
    ...
    d.connections.find((connection) => connection.id === "jwt-verification").via = [
      [620, 142],
      [620, 270],
      [735, 270],
    ];
    ...
    assert.match(
      stderr,
      /\[composition\/container-border-run\] architecture connections\[1\] id "jwt-verification"/,
    );
...
test("dataflow: stage border run is blocking and the inter-stage gutter passes", () => {
  ...
  bad.flows.find((flow) => flow.id === "web-clickstream").via = [
    [184, 157],
    [184, 271],
  ];
...
test("sequence: a message cannot masquerade as a time-segment border", () => {
  ...
  assert.match(
    stderr,
    /\[composition\/container-border-run\] sequence messages\[4\] id "cache-read"/,
  );
...
test("sequence: segment titles render as foreground badges above their borders", () => {
  ...
  assert.ok(
    segmentLabelsAt > activationsAt,
    "segment labels should stay above lifelines, messages, and activations",
  );
  assert.ok(
    messagesAt > activationsAt,
    "message arrows and labels should stay above activation bars",
  );
  assert.match(html, new RegExp(`data-graph-role="segment-label"[^>]*data-segment-id="0"`));
  assert.match(
    html,
    new RegExp(`<text x="62" y="${firstSegment.from - 9}"[^>]*>${firstSegment.label}</text>`),
  );
});
