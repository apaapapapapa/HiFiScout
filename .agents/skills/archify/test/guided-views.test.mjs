...
for (const [mode, config] of Object.entries(CASES)) {
  test(`${mode}: guided views preserve base SVG geometry`, () => {
    ...
    assert.match(
      guided.html,
      /html\[data-embed="true"\] svg\[data-animation="trace"\] \[data-animate\],[\s\S]*?html\[data-share-playback="true"\][\s\S]*?animation: none !important;[\s\S]*?stroke-dashoffset: 0/,
    );
    assert.match(
      guided.html,
    ...
    const oneShot = guided.html.match(
      /function startCurrentViewPlayback\(\) \{([\s\S]*?)\n      function maybeStartSharePlayback/,
    );
    ...
    assert.match(
      guided.html,
      /data-share-playback="true"\] \.diagram-container \{\s*padding-top: 4\.25rem/,
    );
