...
test("Story Follow frames the exact previous, current, and next authored stops through the shared camera", () => {
  ...
  assert.match(
    template,
    /generation !== storyFollowGeneration \|\| storyBeatIndex !== step\.index/,
  );
});
...
test("playback and deliberate beat activation follow while stable moment restoration stays calm", () => {
  ...
  assert.match(
    template,
    /requestAnimationFrame\(function \(\) \{[\s\S]*?storyBeatIndex !== deferredIndex[\s\S]*?followStoryStep\(step, options\)/,
  );
});
...
test("adaptive dwell, Still, reduced motion, hidden pages, and print keep camera motion bounded", () => {
  ...
  assert.match(
    template,
    /Math\.max\(STORY_FOLLOW_MIN_DWELL_MS, VIEW_INTERVAL_MS \/ Math\.max\(1, total\)\)/,
  );
  ...
  assert.match(
    template,
    /Archify\.motionGovernor && Archify\.motionGovernor\.capable\) return !Archify\.motionGovernor\.isPaused\(\)/,
  );
  ...
  assert.match(
    template,
    /function startPlayback\(\) \{[\s\S]*?if \(!storyAutomaticPlaybackAllowed\(\)\)/,
  );
  assert.match(
    template,
...
test("pause, settle, overview, and manual camera takeover cancel Story Follow state", () => {
  ...
  assert.match(
    template,
    /function interruptCamera\(reason\)[\s\S]*?Archify\.guidedViews\.pause\(\)/,
  );
