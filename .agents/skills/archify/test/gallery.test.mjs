...
test("generated proof gallery matches its sources, receipts, and checked-in artifacts", () => {
  ...
  assert.deepEqual(
    new Set(manifest.entries.map((entry) => entry.type)),
    new Set(["architecture", "workflow", "sequence", "dataflow", "lifecycle"]),
  );
  assert.deepEqual(
    Object.fromEntries(
      ["architecture", "workflow", "sequence", "dataflow", "lifecycle"].map((type) => [
        type,
  ...
  assert.ok(
    manifest.entries
      .filter((entry) => entry.id !== "deployment-ownership")
  ...
  for (const entry of manifest.entries) {
    ...
    assert.ok(
      entry.checks.every((check) => check.ok),
      `${entry.id}: validation receipt not green`,
    );
    assert.equal(
      entry.composition.profile,
      "showcase",
      `${entry.id}: expected showcase composition profile`,
    );
    assert.equal(
      entry.composition.status,
      "pass",
      `${entry.id}: showcase composition is not green`,
    );
    assert.equal(
      entry.composition.metrics.properCrossings,
      0,
      `${entry.id}: proper crossing debt remains`,
    );
    assert.equal(
      entry.composition.metrics.ambiguousCorridors,
      0,
      `${entry.id}: ambiguous corridor debt remains`,
    );
    assert.equal(
      entry.composition.metrics.containerBorderRuns,
      0,
      `${entry.id}: container border-run debt remains`,
    );
    assert.equal(
      entry.composition.metrics.labelRouteClearanceIssues,
      0,
      `${entry.id}: label-route clearance debt remains`,
    );
    assert.equal(
      entry.composition.metrics.shortInteriorSegmentCount,
      0,
      `${entry.id}: cramped interior turn remains`,
    );
    assert.equal(
      entry.composition.metrics.microSegmentCount,
      0,
      `${entry.id}: micro segment remains`,
    );
  ...
  assert.match(
    html,
    /Composition<\/span><span class="receipt-value ok" title="0 crossings · 0 border runs · 0 micro segments · 0 cramped turns">SHOWCASE · PASS/,
  );
  ...
  for (const type of ["architecture", "workflow", "sequence", "dataflow", "lifecycle"]) {
    assert.match(
      html,
      new RegExp(`start\\.html\\?type=${type}&amp;source=gallery`),
      `${type}: gallery-to-start link missing`,
    );
  }
  assert.match(html, /class="community-callout"/);
  assert.match(
    html,
    /href="https:\/\/github\.com\/tt-a1i\/archify\/issues\/new\?template=showcase\.yml"[^>]+rel="noopener noreferrer"/,
  );
  ...
  for (const relative of [
    ...
    assert.equal(
      ...
      normalize(fs.readFileSync(checked, "utf8")),
