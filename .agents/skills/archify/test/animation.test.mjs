...
test("webm renders an explicit time-varying canvas scene instead of replaying one cached SVG bitmap", () => {
  ...
  const recordBlock =
    html.match(/function recordWebm\(options\) \{[\s\S]*?\n      var menu =/)?.[0] || "";
...
test("all five renderers add one geometry-neutral semantic sigil per primary node", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    const sigils = (svg) =>
      [...svg.matchAll(/<g aria-hidden="true" data-semantic-sigil="[^"]+"[\s\S]*?<\/g>/g)].map(
        (match) => match[0],
      );
    ...
    assert.deepEqual(
      sigils(traceSvg),
      sigils(staticSvg),
      `${mode} trace must not change sigil geometry`,
    );
...
for (const [mode, example] of Object.entries(CASES)) {
  test(`${mode}: trace animation annotates svg, edges, and nodes`, () => {
    ...
    assert.match(
      svg,
      /id="node-[^"]+" data-node-id="[^"]+"[^>]+role="button"[^>]+aria-pressed="false"/,
    );
...
test("semantic SVG identity is deterministic for unchanged input", () => {
  ...
  const hooks = (svg) =>
    [...svg.matchAll(/(?:id="node-|data-edge-from=")[^>]+/g)].map((match) => match[0]);
