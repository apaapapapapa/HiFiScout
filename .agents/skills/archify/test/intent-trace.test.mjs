...
test("all typed renderers inherit one geometry-neutral Intent Trace", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(
      html,
      /id="intent-trace-status" role="status" aria-live="polite" aria-atomic="true"/,
      mode,
    );
...
test("Intent Trace normalizes motion, respects reduced motion, and exports cleanly", () => {
  ...
  assert.match(
    html,
    /@media \(prefers-reduced-motion: reduce\)[\s\S]+\.intent-trace-flow \{[\s\S]+animation: none !important/,
  );
  ...
  assert.match(
    html,
    /clone\.querySelectorAll\('\[data-intent-trace-match\], \[data-intent-trace-selected\]'\)/,
  );
