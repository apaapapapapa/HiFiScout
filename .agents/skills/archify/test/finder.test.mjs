...
test("all typed renderers ship the same geometry-neutral node finder", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(
      html,
      /id="btn-node-finder"[^>]+aria-label="Find a node"[^>]+aria-haspopup="dialog"/,
      mode,
    );
...
test("finder presents one focused search control and a structured result list", () => {
  ...
  assert.match(
    html,
    /\.node-finder\s*\{[\s\S]*?display:\s*flex;[\s\S]*?max-height:\s*calc\(100% - 2rem\);/,
  );
  ...
  assert.match(
    html,
    /context\.kind === 'focus'\s*\? item\.links \+ \(item\.links === 1 \? ' link' : ' links'\)/,
  );
