...
test("relationship ids stay optional and duplicate ids fail closed in the shared zero-install path", () => {
  for (const mode of Object.keys(CASES)) {
    ...
    const keyZeroTags = Array.from(
      plain.html.matchAll(/<(?:path|g)\b[^>]*data-edge-key="0"[^>]*>/g),
      (match) => match[0],
    );
    assert.ok(keyZeroTags.length > 0, `${mode} emits runtime key zero`);
    assert.ok(
      keyZeroTags.every((tag) => !tag.includes("data-edge-id=")),
      `${mode} does not invent a durable id`,
    );
...
test("the viewer restores and copies stable relation links without exposing numeric keys", () => {
  ...
  assert.match(
    html,
    /if \(html\.getAttribute\('data-embed'\) === 'true' \|\|\s*!inspectRelationshipById/,
  );
