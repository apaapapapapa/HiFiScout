...
test("all five renderers inherit one viewer-only Story Director Strip", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(
      html,
      /id="guided-story-caption" hidden aria-live="polite" aria-atomic="true"/,
      mode,
    );
...
test("captions derive only authored edge labels and existing node facts", () => {
  assert.match(
    template,
    /edgeLabels: edges\.map\(function \(edge\) \{ return edge\.getAttribute\('data-edge-label'\) \|\| ''; \}\)/,
  );
  assert.match(
    template,
  ...
  assert.match(
    template,
    /authored direction: ' \+ step\.nodeLabel \+ ' → ' \+ step\.previousLabel/,
  );
...
test("route copy preserves start, forward, reverse, multiple, and grouped semantics", () => {
  assert.match(
    template,
    /step\.relation === 'start'\) return step\.nodeLabel \+ ' · Starting point'/,
  );
  assert.match(
    template,
    /step\.relation === 'reverse'\) return step\.previousLabel \+ ' ← ' \+ step\.nodeLabel/,
  );
  assert.match(
    template,
    /step\.relation === 'multiple'\) return step\.previousLabel \+ ' ⇄ ' \+ step\.nodeLabel/,
  );
  assert.match(
    template,
    /step\.relation === 'group'\) return step\.previousLabel \+ ' · ' \+ step\.nodeLabel/,
  );
...
test("playback announcements and motion remain reader-controlled", () => {
  ...
  assert.match(
    template,
    /@media \(prefers-reduced-motion: reduce\) \{\s*\.guided-story-caption \{ animation: none !important; \}/,
  );
...
test("Presentation playback removes secondary chrome without hiding Pause or navigation", () => {
  ...
  assert.match(
    template,
    /@media \(min-width: 721px\)[\s\S]*\.guided-views\[data-playing="true"\] \.guided-view-index/,
  );
  ...
  const presentationRule =
    template.match(/@media \(min-width: 721px\) \{([\s\S]*?)\n    \}/)?.[1] || "";
  ...
  assert.match(
    template,
    /@media \(max-width: 720px\)[\s\S]*\.guided-views > #guided-view-prev,[\s\S]*height: 2\.75rem/,
  );
  assert.match(
    template,
    /@media \(max-width: 720px\)[\s\S]*\.guided-view-play,[\s\S]*\.guided-view-beat-link \{ min-height: 2\.75rem; \}/,
  );
});
...
test("embed, print, and canonical export boundaries stay clean", () => {
  ...
  assert.doesNotMatch(
    canonicalSvg(render("workflow", CASES.workflow)),
    /Story Director|guided-story-caption|data-story-caption/,
  );
});
