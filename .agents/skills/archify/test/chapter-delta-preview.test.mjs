...
test("exact stable-ID set math powers truthful counts and the existing handoff", () => {
  ...
  assert.match(
    html,
    /var delta = chapterDelta\(previous, destination\);[\s\S]*?chapterAnchor\(previous, destination, outgoingBeatIndex, delta\)/,
  );
  assert.match(
    html,
...
test("pointer and keyboard inspect while touch and native activation still commit directly", () => {
  ...
  assert.match(
    html,
    /chapterIndex\.addEventListener\('focusin',[\s\S]*?setChapterPreviewIntent\('focus'/,
  );
  assert.match(
    html,
    /event\.key === 'Escape' && activePreviewIndex >= 0[\s\S]*?event\.stopImmediatePropagation\(\)[\s\S]*?clearChapterPreview\(\{ clearIntents: true \}\)/,
  );
  ...
  assert.doesNotMatch(
    previewRuntime,
    /Archify\.view\.|updateUrl\(|Archify\.focus\.|renderStoryTrail\(/,
  );
});
...
test("mobile, Still, embed, print, and canonical exports keep the preview viewer-only", () => {
  ...
  assert.match(
    html,
    /svg\[data-chapter-preview\] \[data-node-id\],[\s\S]*?opacity: 1 !important; filter: none !important/,
  );
