...
test("all five renderers inherit one active-reach-only Reach Share Card item", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(
      html,
      /data-action="reach-share-card"[^>]*hidden disabled[^>]*>[\s\S]*?Reach Share Card[\s\S]*?1200(?:&times;|×)630 PNG/,
      mode,
    );
...
test("Reach Share Card snapshots copy the active authored closure without rerunning traversal", () => {
  ...
  assert.doesNotMatch(
    snapshotBlock,
    /computeReachability|reachabilityFor|queue\s*=|shortestDirectedPath/,
  );
...
test("Reach variant decorates only a finite canonical clone with static authored identity", () => {
  ...
  assert.match(
    applyBlock,
    /edge\.depth !== Math\.max\(snapshot\.depths\[edge\.from\], snapshot\.depths\[edge\.to\]\)/,
  );
  ...
  assert.ok(
    html.indexOf("var canonicalStateClean =") <
      html.indexOf("applyReachSnapshot(clone, opts.reachSnapshot)"),
    "canonical cleanup must precede reach decoration",
  );
});
...
test("Reach styling preserves context, direction, and Blueprint restraint without motion", () => {
  ...
  const reachStyleBlock =
    html.match(/if \(opts\.reachSnapshot\) \{[\s\S]*?\n        \}/)?.[0] || "";
