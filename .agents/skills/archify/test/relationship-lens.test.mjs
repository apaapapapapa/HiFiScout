...
test("all typed renderers expose named, stable relationships without changing geometry", () => {
  for (const [mode, config] of Object.entries(CASES)) {
    ...
    const keys = new Set(
      Array.from(diagram.matchAll(/data-edge-key="(\d+)"/g), (match) => match[1]),
    );

    assert.equal(
      keys.size,
      relationships.length,
      `${mode} keeps one stable key per source relationship`,
    );
    relationships.forEach((relationship, index) => {
      assert.match(
        diagram,
        new RegExp(
          `data-edge-from="${escapeAttr(relationship.from)}"[^>]+data-edge-to="${escapeAttr(relationship.to)}"[^>]+data-edge-key="${index}"`,
        ),
        `${mode} relationship ${index}`,
      );
      if (relationship.label) {
        assert.match(
          diagram,
          new RegExp(
            `data-edge-label="${escapeAttr(relationship.label).replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}"`,
          ),
          `${mode} named relationship ${index}`,
        );
      }
    });
...
test("relationship lens groups incoming, outgoing, and self-loop paths and follows neighbors", () => {
  ...
  assert.match(
    html,
    /id="focus-chip" hidden role="region" aria-labelledby="relationship-lens-title"/,
  );
  ...
  assert.match(
    html,
    /direction = from === id && to === id \? 'loop' : \(from === id \? 'out' : 'in'\)/,
  );
...
test("relationship preview is export-clean and visually geometry-neutral", () => {
  ...
  assert.match(
    html,
    /clone\.querySelectorAll\('\[data-relationship-preview\], \[data-relationship-preview-node\], \[data-relationship-preview-source\], \[data-relationship-preview-target\]'\)/,
  );
...
test("relationship lens is keyboard navigable, mobile-pinned, and excluded from embed and print", () => {
  ...
  assert.match(
    html,
    /previewingOnMobile = mobile && chip\.getAttribute\('data-relationship-previewing'\) === 'true'/,
  );
  assert.match(
    html,
    /nodeCenter < \(visibleTop \+ visibleBottom\) \/ 2 \? pinnedBottom : pinnedTop/,
  );
