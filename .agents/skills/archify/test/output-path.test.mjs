...
test(
  "deliver rechecks aliases immediately before committing a verified candidate",
  { timeout: 10000 },
  async () => {
    ...
    fs.writeFileSync(
      path.join(installedRenderer, "render-workflow.mjs"),
      `
import fs from 'node:fs';
...
`,
    ...
    fs.writeFileSync(
      path.join(installedScripts, "check-render-output.mjs"),
      `
...
`,
    ...
    const child = spawn(
      process.execPath,
      [path.join(installedBin, "archify.mjs"), "deliver", "workflow", input, output, "--json"],
      {
        cwd,
        encoding: "utf8",
...
test("compare rejects a receipt path that aliases either architecture input", () => {
  ...
  const result = run(
    ["compare", "architecture", base, headFixture, output, "--receipt", base, "--json"],
    cwd,
  );
...
test(
  "compare rechecks every target immediately before committing the artifact pair",
  { timeout: 10000 },
  async () => {
    ...
    fs.writeFileSync(
      path.join(installedRenderer, "render-architecture.mjs"),
      `
import fs from 'node:fs';
import path from 'node:path';
const [, output] = process.argv.slice(2);
...
`,
    ...
    fs.writeFileSync(
      path.join(installedDelta, "architecture-delta.mjs"),
      `
...
test("doctor reports a missing output-path safety runtime in an installed skill", () => {
  ...
  const result = spawnSync(
    process.execPath,
    [path.join(installedRoot, "bin/archify.mjs"), "doctor"],
    {
      cwd: installedRoot,
      encoding: "utf8",
    },
