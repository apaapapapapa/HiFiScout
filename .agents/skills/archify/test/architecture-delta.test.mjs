...
const run = (args) =>
  spawnSync(process.execPath, [cli, ...args], { cwd: skillRoot, encoding: "utf8" });
...
test("architecture compare classifies authored facts separately from geometry and presentation", () => {
  ...
  const authorization = receipt.changes.connections.find(
    (change) => change.id === "authorize-payment",
  );
  ...
  assert.deepEqual(
    receipt.changes.connections.find((change) => change.status === "added").classifications,
    ["topology"],
  );
  ...
  assert.deepEqual(
    boundaryReceipt.changes.boundaries.find((change) => change.label === "Fraud edge")
      .classifications,
    ["scope"],
  );
});
...
test("change navigator order is exact-ID based, complete, unique, and stable", () => {
  ...
  assert.equal(
    rows.length,
    receipt.changes.components.length +
      receipt.changes.connections.length +
      receipt.changes.boundaries.length,
  );
});
...
test("exact identity fails closed instead of guessing relationships or unrelated systems", () => {
  ...
  assert.throws(
    ...
    (error) =>
      error instanceof ArchitectureDeltaError && error.code === "delta/no-shared-component-id",
  );
});
...
test("mixed semantic and geometry component changes retain both exact forms", () => {
  ...
  assert.match(
    html,
    /data-change-key="component:queue"[^>]+data-change-target-signature="g:changed:geometry,semantic\|g:moved-from:geometry,semantic"/,
  );
  assert.deepEqual(validateArchitectureDeltaHtml(html, receipt), {
    ok: true,
    checksPassed: 10,
    checkCount: 10,
  });
...
test("mixed semantic and geometry relationship changes retain both exact routes", () => {
  ...
  assert.match(
    html,
    /data-change-key="relationship:publish-order"[^>]+data-change-target-signature="g:changed:geometry,semantic\|g:moved-from:geometry,semantic\|path:changed:geometry,semantic\|path:moved-from:geometry,semantic\|text:changed:\|text:moved-from:"/,
  );
  assert.deepEqual(validateArchitectureDeltaHtml(html, receipt), {
    ok: true,
    checksPassed: 10,
...
test("same-label node id changes remain one removal plus one addition", () => {
  ...
  const receipt = compareArchitecture(base, head);
  assert.equal(
    receipt.changes.components.find((change) => change.id === "cache").status,
    "removed",
  );
  assert.equal(
    receipt.changes.components.find((change) => change.id === "session-store").status,
    "added",
  );
  assert.equal(
    receipt.changes.components.filter(
      (change) => change.headLabel === "Session Cache" || change.baseLabel === "Session Cache",
    ).length,
    2,
  );
});
...
test("repository mismatch fails and verified matching revisions remain evidence-bounded", () => {
  ...
  assert.throws(
    ...
    (error) =>
      error instanceof ArchitectureDeltaError && error.code === "delta/repository-mismatch",
  );
...
test("compare CLI writes a deterministic three-state artifact and complete sidecar receipt", () => {
  ...
  assert.match(
    firstHtml,
    /window\.Archify\.deltaExport = \{ canonicalSvg: canonicalDeltaSvg, shareCard/,
  );
  ...
  assert.match(
    firstHtml,
    /structural-frame.*data-delta-state="changed".*stroke-dasharray:2 3!important/,
  );
  ...
  assert.deepEqual(validateArchitectureDeltaHtml(firstHtml, receipt), {
    ok: true,
    checksPassed: 10,
    checkCount: 10,
  });
...
test("artifact validation fails closed on missing, duplicate, or self-blessed review identity", () => {
  ...
  const receipt = JSON.parse(result.stdout);
  const html = fs.readFileSync(output, "utf8");
  const deltaSection = html.match(
    /<section class="canvas" data-view="delta">([\s\S]*?)<\/section>/,
  )?.[1];
  assert.ok(deltaSection);
  const fraudTag = deltaSection.match(/<g\s+[^>]*\bdata-node-id="fraud"[^>]*>/)?.[0];
  assert.ok(fraudTag);

  const missing = html.replace(
    fraudTag,
    fraudTag.replace('data-node-id="fraud"', 'data-node-id="tampered"'),
  );
  assert.throws(
  ...
  const relationshipGroup = deltaSection.match(
    /<g\s+[^>]*\bdata-edge-id="fraud-check"[^>]*>[\s\S]*?<\/g>/,
  )?.[0];
  assert.ok(relationshipGroup);
  const duplicateCompanion = html.replace(
    relationshipGroup,
  ...
  const duplicateRowTag = duplicateCompanion.match(
    /<button class="change-row"[^>]*data-change-key="relationship:fraud-check"[^>]*>/,
  )?.[0];
  ...
  const selfBlessed = duplicateCompanion.replace(
    duplicateRowTag,
    duplicateRowTag.replace(
      `data-change-target-signature="${storedSignature}"`,
      `data-change-target-signature="${selfBlessedSignature}"`,
    ),
  );
  assert.throws(
