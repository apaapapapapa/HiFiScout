...
test("only legends with an exact node-kind meaning publish bridge entries", () => {
  ...
  const workflow = canonicalSvg(
    render("workflow", (document) => {
...
test("preview is soft, input-aware, and yields to stronger exploration owners", () => {
  ...
  assert.match(
    html,
    /event\.pointerType === 'touch' \|\| \(finePointerQuery && !finePointerQuery\.matches\)/,
  );
  ...
  assert.match(
    html,
    /selectedKinds\.length > 0 \|\| !panel\.hidden \|\| html\.getAttribute\('data-present'\) === 'true'/,
  );
...
test("bridge state is print-safe, reduced-motion-safe, and absent from canonical export", () => {
  ...
  assert.match(
    html,
    /html:not\(\[data-embed="true"\]\) \.diagram-container \{\s*padding: 0\.75rem 0\.75rem 4\.25rem;/,
  );
  ...
  assert.match(
    html,
    /\[data-legend-preview-match\], \[data-legend-preview-selected\], \[data-legend-preview-peer\]/,
  );
  assert.match(
    html,
    /\[data-legend-bridge\], \[data-legend-kind\], \[data-legend-bridge-runtime\]/,
  );
});
