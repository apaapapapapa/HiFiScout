...
test(
  "preview: invalid candidates preserve the last verified artifact and repair automatically",
  { timeout: 30000 },
  async () => {
    ...
    try {
      const first = await waitForState(
        preview.url,
        (state) => state.status === "verified" && state.revision === 1,
        "first revision did not verify",
      );
      ...
      fs.rmSync(input);
      const missing = await waitForState(
        preview.url,
        (state) => state.status === "needs-fix" && state.generation === 2,
        "deleted source did not report failure",
      );
      ...
      const failed = await waitForState(
        preview.url,
        (state) => state.status === "needs-fix" && state.generation === 3,
        "invalid source did not report failure",
      );
      ...
      assert.equal(
        await (await fetch(new URL("/artifact.html", preview.url))).text(),
        firstArtifact,
      );

      source.components[0].unexpected = true;
      fs.writeFileSync(input, JSON.stringify(source));
      const schemaFailed = await waitForState(
        preview.url,
        (state) => state.status === "needs-fix" && state.generation === 4,
        "schema failure did not report render stage",
      );
      ...
      const repaired = await waitForState(
        ...
        "repaired source did not publish",
      );
...
test(
  "preview: content digests suppress identical writes and a burst publishes only its stable tail",
  { timeout: 30000 },
  async () => {
    ...
    try {
      await waitForState(
        preview.url,
        (state) => state.status === "verified" && state.revision === 1,
        "initial workflow did not verify",
      );
      fs.writeFileSync(input, original);
      ...
      state = await waitForState(
        preview.url,
        (candidate) => candidate.status === "verified" && candidate.generation === 2,
        "semantically identical source did not settle",
      );
      ...
      state = await waitForState(
        preview.url,
        (candidate) => candidate.status === "verified" && candidate.revision === 2,
        "stable burst tail did not verify",
      );
...
test(
  "preview: a superseded slow candidate can never become a published revision",
  { timeout: 30000 },
  async () => {
...
`,
    ...
    try {
      await waitForState(
        preview.url,
        (state) => state.status === "checking" && state.generation === 1,
        "slow generation did not start",
      );
      ...
      const state = await waitForState(
        preview.url,
        (candidate) => candidate.status === "verified" && candidate.generation === 2,
        "latest generation did not publish",
      );
...
test(
  "preview: each delivery reads the immutable bytes bound to its observed digest",
  { timeout: 30000 },
  async () => {
...
`,
    ...
    try {
      await waitForState(
        preview.url,
        (state) => state.status === "checking" && state.generation === 1,
        "snapshot generation did not start",
      );
      ...
      const state = await waitForState(
        preview.url,
        (candidate) => candidate.status === "verified" && candidate.revision === 1,
        "snapshot generation did not verify",
      );
...
test(
  "preview: commit rechecks the live digest when watcher and poll have not seen a newer save",
  { timeout: 10000 },
  async () => {
...
`,
    ...
    try {
      await waitForState(
        preview.url,
        (state) => state.status === "verified" && state.revision === 1,
        "prior good revision did not verify",
      );
      const priorArtifact = fs.readFileSync(output, "utf8");
      fs.writeFileSync(input, JSON.stringify({ title: "Intermediate A" }));
      await waitForState(
        preview.url,
        (state) => state.status === "checking" && state.generation === 2,
        "intermediate generation did not start",
      );
      ...
      assert.equal(
        ...
        priorArtifact,
        "superseded intermediate bytes replaced the prior last-good output",
      );
      const pending = await stateAt(preview.url);
      assert.equal(
        pending.revision,
        1,
        "superseded intermediate bytes advanced the browser revision",
      );

      const current = await waitForState(
        preview.url,
        (state) => state.status === "verified" && state.generation === 3,
        "current generation did not verify",
      );
...
test(
  "preview: stopping drains an active delivery without publishing it",
  { timeout: 30000 },
  async () => {
...
`,
    ...
    await waitForState(
      preview.url,
      (state) => state.status === "checking" && state.generation === 1,
      "slow stop candidate did not start",
    );
...
test(
  "preview: stopping has a bounded kill path for a delivery that never exits",
  { timeout: 5000 },
  async () => {
...
test(
  "preview: checker failures keep their actionable detail instead of a generic stage only",
  { timeout: 30000 },
  async () => {
...
`,
    ...
    try {
      const state = await waitForState(
        ...
        "checker failure did not surface",
      );
...
test(
  "preview: all five typed renderers reach a verified first revision",
  { timeout: 60000 },
  async () => {
    const cases = {
