...
function checkHtml(name, svgBody, profile = "standard") {
  ...
  fs.writeFileSync(
    htmlPath,
    `<!doctype html><html><body><svg viewBox="0 0 240 160" data-quality-profile="${profile}">${svgBody}</svg></body></html>`,
  );
  try {
...
test("render output check: relationship labels cannot hide another shared-source route", () => {
  for (const profile of ["standard", "showcase"]) {
    ...
    assert.deepEqual(issue.labelRelationship, {
      id: "approved",
      from: "dlq",
      to: "replay",
      label: "approved replay",
      collectionIndex: 0,
      artifactIndex: 1,
    });
    assert.deepEqual(issue.otherRelationship, {
      id: "sample",
      from: "dlq",
      to: "ops",
      label: "",
...
test("render output check: label-route thresholds include exact 2px and 4px boundaries", () => {
  ...
  assert.equal(
    showcaseAtTwo.result.composition.issues.find(
      (item) => item.code === "composition/label-route-clearance",
    ).threshold,
    4,
  );
...
test("render output check: container border runs fail both profiles with frame identity", () => {
  for (const profile of ["standard", "showcase"]) {
    ...
    assert.match(
      check.details[0],
      /\[composition\/container-border-run\].*relationship id "events"/,
    );
...
test("render output check: composition receipt records neutral normalized route metrics", () => {
  ...
  assert.deepEqual(result.composition.suggestedLimits, {
...
test("render output check: endpoint stubs from 8px pass while cramped interior turns are profile-aware", () => {
  ...
  assert.match(
    rhythm.details[0],
    /\[composition\/short-interior-segment\] showcase relationship id "tight"/,
  );
});
