  ...
  return (
    candidates.find((candidate) => {
      ...
      const revision = spawnSync("git", [
        ...
        "cat-file",
        "-e",
        "9f1a1cf1afdc04d7b5406782b40dfec76d9bc798^{commit}",
      ]);
      if (revision.status !== 0) return false;
      const origin = spawnSync("git", ["-C", candidate, "remote", "get-url", "origin"], {
        encoding: "utf8",
      });
      return (
        origin.status === 0 &&
        /github\.com[:/]mco-org\/mco(?:\.git)?\/?$/i.test(origin.stdout.trim())
      );
    }) || null
  );
}
...
test("MCO proof is source-backed, reproducible, and linked from every README", () => {
  ...
  assert.deepEqual(
    source.meta.views.map((view) => view.id),
    ["dispatch-path", "answer-evidence", "durable-sessions"],
  );
  ...
  const references = source.components.reduce(
    (count, component) => count + (component.sources?.length || 0),
    0,
  );
  ...
  if (mcoRoot) {
    ...
    const result = spawnSync(
      process.execPath,
      [cli, "deliver", "architecture", sourcePath, output, "--quality", "showcase", "--json"],
      { encoding: "utf8" },
    );
  ...
  for (const filename of ["README.md", "README_EN.md", "README_ZH.md"]) {
    ...
    assert.match(
      readme,
      /cases\/mco-runtime\.architecture\.html\?theme=dark&present=1#view=dispatch-path/,
    );
