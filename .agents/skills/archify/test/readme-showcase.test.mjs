...
test("README motion proof is compact, looping, and backed by current gallery artifacts", () => {
  ...
  assert.ok(
    buffer.byteLength <= 3 * 1024 * 1024,
    `README GIF is too large: ${buffer.byteLength} bytes`,
  );
  ...
  assert.deepEqual(
    receipt.scenes.map((scene) => scene.id),
  ...
  for (const scene of receipt.scenes) {
    ...
    assert.equal(
      ...
      sha256(artifact),
      `${scene.id}: source artifact drift; rebuild README showcase`,
    );
...
test("all README languages keep the product hero and retain the verified animated proof", () => {
  for (const filename of ["README.md", "README_EN.md", "README_ZH.md"]) {
    ...
    const demosIndex = Math.max(
      readme.indexOf("## See Archify in action"),
      readme.indexOf("## 看看 Archify 能做什么"),
    );
    assert.ok(
      heroIndex >= 0 && heroIndex < titleIndex,
      `${filename}: product hero is not above the title`,
    );
...
test("README demos use checked-in captures and live deep links below the existing hero", () => {
  ...
  for (const demo of demos) {
    ...
    assert.equal(
      ...
      "PNG",
      `${demo.asset}: invalid PNG signature`,
    );
  ...
  for (const filename of ["README.md", "README_EN.md", "README_ZH.md"]) {
    ...
    const quickStartIndex = Math.max(
      readme.indexOf("## Quick start"),
      readme.indexOf("## 快速开始"),
    );
    assert.ok(heroIndex >= 0 && heroIndex < demosIndex, `${filename}: existing hero proof moved`);
    assert.ok(
      demosIndex < previewIndex && previewIndex < quickStartIndex,
      `${filename}: demo section is misplaced`,
    );
    assert.ok(
      demosIndex < proofIndex && proofIndex < previewIndex,
      `${filename}: animated proof is outside the demo section`,
    );
    for (const demo of demos) {
...
test("README stays scannable without deleting the visual proof set", () => {
  ...
  for (const filename of ["README.md", "README_EN.md", "README_ZH.md"]) {
    ...
    for (const asset of commonAssets) {
      assert.ok(
        readme.includes(`docs/assets/${asset}`),
        `${filename}: visual proof ${asset} was removed`,
      );
    }
  }
