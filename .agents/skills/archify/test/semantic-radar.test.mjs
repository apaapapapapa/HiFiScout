...
test("all typed renderers inherit one viewer-only Semantic Radar", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(
      html,
      /id="overview-map" hidden role="region" aria-labelledby="overview-map-title"/,
      mode,
    );
    assert.match(html, /id="overview-map-surface" tabindex="0" role="group"/, mode);
    assert.match(
      html,
      /id="btn-overview-map"[^>]+aria-label="Open semantic radar"[^>]+aria-expanded="false"[^>]+aria-controls="overview-map"/,
      mode,
    );
...
test("Semantic Radar keeps redundant accessible navigation and clean exports", () => {
  ...
  assert.match(
    html,
    /event\.key === 'ArrowLeft'[\s\S]+event\.key === 'ArrowRight'[\s\S]+event\.key === 'ArrowUp'[\s\S]+event\.key === 'ArrowDown'/,
  );
  ...
  assert.match(
    html,
    /The radar is built at runtime so the checked artifact still contains[\s\S]+one canonical SVG block/,
  );
