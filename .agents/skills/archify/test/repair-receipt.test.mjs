...
test("repair receipt: validate and deliver share exact Clean Flow evidence while delivery preserves the trusted artifact", () => {
  ...
  const validateRepair = receipt(validated).diagnostics.find(
    (entry) => entry.code === "clean-flow/edge-through-node",
  );
  const deliverRepair = receipt(delivered).diagnostics.find(
    (entry) => entry.code === "clean-flow/edge-through-node",
  );
