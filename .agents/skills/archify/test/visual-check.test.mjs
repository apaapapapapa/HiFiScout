...
import { VISUAL_CHECK_VIEWPORTS, runVisualCheck, sidecarPaths } from "../bin/visual-check.mjs";
...
test("visual-check records four containment viewports and four endpoint theme captures", async () => {
  ...
  assert.equal(
    result.receipt.containment.viewports.every((entry) => entry.ok),
    true,
  );
  assert.deepEqual(
  ...
  assert.equal(
    outputs.screenshots.every((entry) => fs.existsSync(entry.path)),
    true,
  );
  ...
  for (const screenshot of outputs.screenshots) {
    assert.match(
      contactSheet,
      new RegExp(path.basename(screenshot.path).replace(/[.*+?^${}()|[\]\\]/g, "\\$&")),
    );
    assert.doesNotMatch(
      contactSheet,
      new RegExp(screenshot.path.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")),
    );
  }
});
...
test("visual-check returns 1 and preserves evidence when any viewport overflows", async () => {
  ...
  assert.deepEqual(
    result.receipt.containment.viewports
...
test("visual-check returns 1 and removes misleading capture sidecars on screenshot failure", async () => {
  ...
  assert.equal(
    outputs.screenshots.some((entry) => fs.existsSync(entry.path)),
    false,
  );
  assert.equal(fs.existsSync(outputs.receipt), true);
});
