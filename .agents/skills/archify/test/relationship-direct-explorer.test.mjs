...
test("all five renderers inherit one viewer-only Direct Relationship Explorer", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.doesNotMatch(
      canonicalSvg(html),
      /relationship-hit-(?:overlay|target|rail)|data-relationship-direct-active/,
      mode,
    );
  }
});
...
test("one roving target represents each exact stable edge key and authored direction", () => {
  ...
  assert.match(
    template,
    /\.relationship-hit-rail \{[\s\S]*stroke: transparent;[\s\S]*stroke-width: 24/,
  );
});
...
test("direct relationship targets support one-tap touch, yield to stronger states, and stay export-clean", () => {
  ...
  assert.match(
    template,
    /@media \(hover: none\), \(pointer: coarse\)[\s\S]*\.relationship-hit-rail \{ stroke-width: 24/,
  );
