...
test("Dataflow database node facts are interactive while flow variants stay visual-only", () => {
  ...
  const forcedWithoutFact = canonicalSvg(
    render(
      "dataflow",
      withLegend("dataflow", {
  ...
  assert.deepEqual(attrValues(forcedWithoutFact, "data-legend-semantic-kind"), [
    "database",
    "default",
  ]);
...
test("Issue #52 dataflow and lifecycle reproductions publish truthful default legends", () => {
  ...
  assert.deepEqual(attrValues(lifecycleLegend, "data-legend-semantic-kind"), [
    "start",
    "active",
    "success",
  ]);
...
test("label overrides round-trip through all five public renderers", () => {
  for (const type of Object.keys(FIXTURES)) {
    ...
    if (["architecture", "workflow", "lifecycle"].includes(type)) {
      assert.match(
        svg,
        new RegExp(`data-legend-kind="${kind}"[^>]+data-legend-label="${label}"`),
        type,
      );
    } else {
...
test("label overrides preserve stable kinds and exact Semantic Legend boundaries", () => {
  ...
  assert.deepEqual(attrValues(svg, "data-legend-semantic-kind"), [
    "frontend",
    "database",
    "external",
  ]);
  ...
  assert.match(
    svg,
    /data-legend-kind="frontend"[^>]+data-legend-label="Reader &lt;UI&gt; &amp; &quot;ops&quot;"/,
  );
...
test("strict per-renderer schemas reject malformed legend contracts with path-prefixed errors", () => {
  ...
  for (const type of Object.keys(FIXTURES)) {
    for (const [legendOrFactory, pathPrefix] of cases) {
      const legend =
        typeof legendOrFactory === "function" ? legendOrFactory(type) : legendOrFactory;
...
test("measured legends fail explicitly instead of wrapping into diagram content", () => {
  ...
  const routedEntries = Object.fromEntries(
    CATALOGS.architecture.map((kind) => [kind, { label: `Long ${kind} convention` }]),
  );
...
test("explicit Architecture viewBox rejects legend title rectangles that overlap content", () => {
  ...
  doc.components = [{ id: "ui", type: "frontend", label: "UI", pos: [40, 216], size: [120, 60] }];
...
test("measured legend rows share baselines and stay within the viewBox for localized labels", () => {
  ...
  const viewBox = attrValues(svg.match(/<svg\b[^>]*>/)?.[0] || "", "viewBox")[0]
    .split(/\s+/)
    .map(Number);
  const tags = [...svg.matchAll(/<g\b[^>]*data-legend-semantic-kind="[^"]+"[^>]*>/g)].map(
    (match) => match[0],
  );
  ...
  assert.ok(
    boxes.every(
      (box) => Number.isFinite(box.x) && Number.isFinite(box.y) && Number.isFinite(box.width),
    ),
  );
