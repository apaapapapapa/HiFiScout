...
test("architecture: a singly spread near-aligned horizontal relationship keeps one direct axis", () => {
  ...
  assert.deepEqual(connectionPoints(html, "hub-direct"), [
    [220, 123],
    [500, 123],
  ]);
  assert.notDeepEqual(
    connectionPoints(html, "hub-branch")[0],
    connectionPoints(html, "hub-direct")[0],
  );
});
...
test("architecture: a shared bottom port keeps its aligned child relationship straight", () => {
  ...
  assert.deepEqual(connectionPoints(forward, "parent-terminal"), [
    [393, 160],
    [393, 300],
  ]);
  assert.notDeepEqual(
...
test("architecture: incoming and outgoing relationships keep distinct bottom ports while the direct child stays straight", () => {
  ...
  assert.deepEqual(connectionPoints(html, "child-footer"), [
    [407, 160],
    [407, 320],
  ]);
  assert.notDeepEqual(
...
test("architecture: single and explicitly positioned relationships keep legacy anchors", () => {
  ...
  assert.deepEqual(connectionPoints(html, "single"), [
    [220, 310],
    [500, 310],
  ]);
  assert.deepEqual(connectionPoints(html, "via"), [
    [220, 310],
    [300, 310],
    [300, 130],
    [500, 130],
  ]);
  assert.deepEqual(connectionPoints(html, "fixed-route"), [
    [220, 310],
    [360, 310],
    [360, 490],
    [500, 490],
  ]);
  assert.deepEqual(connectionPoints(html, "fixed-label"), [
    [220, 310],
    [360, 310],
    [360, 130],
    [500, 130],
...
test("architecture: an unspread near-aligned connection shares one horizontal axis", () => {
  ...
  assert.deepEqual(connectionPoints(html, "console-controlplane"), [
    [430, 332],
    [500, 332],
  ]);
});
...
test("skill and READMEs describe automatic port spread as bounded default behavior", () => {
  ...
  assert.match(
    skill,
    /facing automatic ports \(`left`\/`right` or `top`\/`bottom`\).*one shared axis/,
  );
