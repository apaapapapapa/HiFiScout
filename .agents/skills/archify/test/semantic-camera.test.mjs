...
test("all typed renderers ship the same geometry-neutral semantic camera", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(
      html,
      /contentScale = Math\.min\(svgWidth \/ viewBox\.width, svgHeight \/ viewBox\.height\)/,
      mode,
    );
...
test("semantic camera follows reader intent but yields to manual navigation", () => {
  ...
  assert.match(
    html,
    /window\.innerWidth <= 720 && container\.hasAttribute\('data-wide-diagram'\) && Date\.now\(\) > autoScrollUntil/,
  );
...
test("semantic camera keeps mobile on its contained scroll model and respects reduced motion", () => {
  ...
  assert.match(
    html,
    /if \(!container\.hasAttribute\('data-wide-diagram'\)\) \{[\s\S]+cameraReceipt\(\{ scale: 1, x: 0, y: 0, mode: 'semantic' \}/,
  );
  assert.match(
    html,
    /state\.scale = 1;[\s\S]+state\.x = 0;[\s\S]+state\.y = 0;[\s\S]+state\.mode = 'semantic';[\s\S]+apply\(\)/,
  );
