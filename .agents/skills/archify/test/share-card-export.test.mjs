...
test("Share Card uses contain-only canonical geometry with fixed safe areas", () => {
  ...
  assert.match(
    html,
    /function rasterizeShareCard\(options\)[\s\S]*?if \(!options\.variant\) return renderShareCard\(\);/,
  );
  assert.match(
    html,
    /function renderShareCard\(options\)[\s\S]*?serializeSvg\(sourceScale, \{ routeSnapshot: routeSnapshot, reachSnapshot: reachSnapshot \}\)/,
  );
...
test("Copy Share Card reuses one canonical card blob and writes only PNG to the clipboard", () => {
  ...
  assert.match(
    html,
    /function runCopyShareCard\(\)[\s\S]*?var blobPromise = rasterizeShareCard\(\);/,
  );
