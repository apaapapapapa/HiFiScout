...
test("cli: doctor rejects a corrupt standalone validator", () => {
  ...
  fs.writeFileSync(
    path.join(corruptRoot, "renderers/shared/generated-validators.mjs"),
    "export const workflow = ;\n",
  );

  const result = spawnSync(
    process.execPath,
    [path.join(corruptRoot, "bin/archify.mjs"), "doctor"],
    {
      cwd: corruptRoot,
      encoding: "utf8",
    },
...
test("cli: examples renders from an installed skill", () => {
  ...
  const result = spawnSync(
    process.execPath,
    [path.join(installedRoot, "bin/archify.mjs"), "examples"],
    {
      cwd: installedRoot,
      encoding: "utf8",
    },
...
test("cli: guide works from an installed skill without node_modules", () => {
  ...
  const result = spawnSync(
    process.execPath,
    [installedCli, "guide", "incident-runbook", "--json"],
    {
      cwd: installedRoot,
      encoding: "utf8",
    },
...
test("cli: demo creates a ready-to-open diagram in a chosen directory", () => {
  ...
  assert.match(
    result.stdout,
    new RegExp(`Demo ready: ${output.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}`),
  );
...
test(
  "cli: opener failure does not invalidate a verified delivery or pollute json stdout",
  {
    skip: process.platform === "win32",
  },
  () => {
...
test(
  "cli: deliver failure never invokes the optional opener",
  {
    skip: process.platform === "win32",
  },
  () => {
...
test("cli: a missing optional opener module preserves verified delivery with a fallback receipt", () => {
  ...
  const result = spawnSync(
    process.execPath,
    [installedCli, "deliver", "workflow", input, out, "--open", "--json"],
    {
      cwd: installedRoot,
      encoding: "utf8",
    },
...
test("cli: deliver preserves the renderer default output contract", () => {
  ...
  assert.equal(
    receipt.output,
    path.join(fs.realpathSync(workingDirectory), "verified-default.html"),
  );
...
test("cli: deliver works from an installed skill without node_modules", () => {
  ...
  for (const [type, example] of Object.entries(cases)) {
    ...
    const result = spawnSync(
      process.execPath,
      [installedCli, "deliver", type, input, out, "--json"],
      {
        cwd: installedRoot,
        encoding: "utf8",
      },
...
test(
  "cli: preview runs from an installed skill without node_modules and exits cleanly",
  { timeout: 30000 },
  async () => {
    ...
    assert.equal(
      state?.status,
      "verified",
      `preview did not verify; stdout=${stdout}; stderr=${stderr}`,
    );
    ...
    assert.deepEqual(
      fs.readdirSync(path.dirname(output)).filter((name) => name.startsWith(".archify-preview-")),
      [],
    );
  },
);
...
test("cli: deliver preserves the previous artifact when the final check fails", () => {
  ...
  fs.writeFileSync(
    templatePath,
    template.replace("</body>", '<svg aria-label="accidental second svg"></svg>\n</body>'),
  );
  ...
  const result = spawnSync(
    process.execPath,
    [installedCli, "deliver", "architecture", input, out, "--json"],
    {
      cwd: installedRoot,
      encoding: "utf8",
    },
  ...
  assert.ok(
    failure.diagnostics[0].supportedFixes.some((fix) => fix.includes("exactly one diagram SVG")),
  );
