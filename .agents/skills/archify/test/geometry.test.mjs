...
test("automaticPortRhythmBridge: near parallel ports use readable outside runs", () => {
  const points = automaticPortRhythmBridge([742, 300], [735, 180], "top", "bottom");
...
test("cleanFlowProblems reports collection index, ids, segment, clearance, and fix", () => {
  ...
  assert.match(
    problems[0],
    /\[clean-flow\/edge-through-node\] architecture connections\[0\] id "checkout" "client" -> "database"/,
  );
...
test("cleanCrossingProblems reports one deterministic proper X in showcase", () => {
  ...
  assert.match(
    problems[0],
    /connections\[0\] id "first" "a" -> "b" crosses connections\[1\] id "second" "c" -> "d"/,
  );
...
test("ambiguous corridor gate reports unrelated collinear overlap with exact identities", () => {
  ...
  assert.match(
    problems[0],
    /edges\[0\] id "first" "a" -> "b" shares a 60px corridor with edges\[1\] id "second" "c" -> "d"/,
  );
...
test("cleanBorderRunProblems reports a deterministic long run on a rounded frame side", () => {
  ...
  assert.match(
    problems[0],
    /\[composition\/container-border-run\] architecture connections\[0\] id "jwt" "auth" -> "api"/,
  );
  assert.match(
    problems[0],
    /follows security-group "Private tier" top border for 112px on segment 0 \[108, 80\] -> \[220, 80\]/,
  );
...
test("route rhythm separates ordinary endpoint stubs from cramped turns and micro segments", () => {
  const issues = collectRouteRhythmIssues({
    routedRelations: [
      {
        relation: { id: "lane-hop", from: "a", to: "b" },
        points: [
          [0, 0],
          [13, 0],
          [13, 40],
      ...
      {
        relation: { id: "bad-turn", from: "c", to: "d" },
        points: [
          [0, 80],
          [24, 80],
          [24, 89],
      ...
      {
        relation: { id: "micro-stub", from: "e", to: "f" },
        points: [
          [0, 120],
          [5, 120],
          [5, 180],
  ...
  assert.deepEqual(
    issues.map((issue) => [issue.relation.id, issue.code, issue.position, issue.length]),
    [
...
test("route rhythm is a showcase-only generation gate with actionable relationship identity", () => {
  ...
  assert.match(
    problems[0],
    /\[composition\/micro-segment\] showcase architecture connections\[0\] id "events"/,
  );
