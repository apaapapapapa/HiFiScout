...
test("all typed renderers inherit one viewer-only Diagram Guide", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(
      html,
      /id="diagram-guide" hidden role="dialog" aria-modal="false" aria-labelledby="diagram-guide-title"/,
      mode,
    );
    assert.match(
      html,
      /id="btn-diagram-guide"[^>]+aria-label="Open diagram guide"[^>]+aria-haspopup="dialog"[^>]+aria-expanded="false"/,
      mode,
    );
    ...
    assert.doesNotMatch(
      canonicalSvg(html),
      /diagram-guide|Archify\.guide|Explore this system/,
      mode,
    );
  }
});
...
test("Diagram Guide delegates its task rows to existing production interactions", () => {
  ...
  assert.match(
    html,
    /event\.stopPropagation\(\);[\s\S]+execute\(button\.getAttribute\('data-guide-action'\)\)/,
  );
});
