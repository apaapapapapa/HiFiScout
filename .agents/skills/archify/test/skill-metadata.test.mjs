...
test("skill description is portable across 1024-character runtimes and remains searchable", () => {
  ...
  assert.ok(
    ...
    `description is ${description.length} characters; maximum is 1024`,
  );
  assert.ok(
    Buffer.byteLength(description, "utf8") <= 1024,
    "description must also fit a 1024-byte runtime limit",
  );
  ...
  for (const trigger of [
    ...
    assert.match(
      ...
      `description must retain the ${trigger} trigger`,
    );
  }
...
test("main skill stays a bounded authoring router with progressive references", () => {
  ...
  assert.ok(
    lines.length <= 160,
    `SKILL.md is ${lines.length} lines; keep the entrypoint at 160 or fewer`,
  );
