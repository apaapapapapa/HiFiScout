...
test("all five renderers inherit one viewer-only Story Moment Link control", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(
      html,
      /id="guided-view-beat-link"[^>]+aria-label="Select a Story Beat to copy its exact link"[^>]+disabled/,
    );
...
test("moment links use exact stable view and node ids without mutating manual selection URLs", () => {
  ...
  assert.match(
    template,
    /storySteps\.findIndex\(function \(step\) \{ return step\.nodeId === id; \}\)/,
  );
  ...
  assert.match(
    template,
    /afterHandoff\(function \(\) \{[\s\S]*restoreGeneration !== momentRestoreGeneration[\s\S]*selectStoryBeatById\(requestedBeat, \{ linked: true, follow: true, followInstant: true \}\)/,
  );
  const manualSelection =
    template.match(
      /function selectStoryBeat\(index\) \{([\s\S]*?)\n      function updateUrl/,
    )?.[1] || "";
...
test("invalid or cross-chapter beat ids fail closed while the public receipt stays read-only", () => {
  ...
  assert.match(
    template,
    /setStoryBeat\(index, \{[\s\S]*?manual: false,[\s\S]*?center: true,[\s\S]*?pulse: false,[\s\S]*?follow: options\.follow === true,[\s\S]*?linked: options\.linked === true,[\s\S]*?followInstant: options\.followInstant === true/,
  );
...
test("copy feedback, one-shot playback, and reduced motion preserve the requested moment", () => {
  ...
  assert.match(
    template,
    /if \(!storyAutomaticPlaybackAllowed\(\)\)[\s\S]*hashBeatMatchesCurrent\(\)[\s\S]*setAutoplayState\('reduced-motion'\)/,
  );
  assert.match(template, /storyPlaybackScope = 'chapter'/);
  assert.match(
    template,
    /if \(storyBeatIndex < 0\)[\s\S]*setStoryBeat\(0, \{ pulse: true, follow: true \}\)/,
  );
});
...
test("the control keeps stable desktop/mobile geometry and existing viewer boundaries", () => {
  ...
  assert.match(
    template,
    /@media \(max-width: 720px\)[\s\S]*\.guided-view-actions \{[\s\S]*grid-template-columns: repeat\(3, minmax\(0, 1fr\)\)/,
  );
  assert.match(
    template,
    /@media \(max-width: 720px\)[\s\S]*\.guided-view-beat-link \{[\s\S]*min-height: 2\.75rem/,
  );
  assert.match(
    template,
    /html\[data-embed="true"\][\s\S]*\.guided-views \{ display: none !important; \}/,
  );
  assert.match(
    template,
  ...
  assert.match(
    template,
    /clone\.querySelectorAll\('\[data-story-overlay\], \[data-story-carrier-overlay\]'\)/,
  );
});
