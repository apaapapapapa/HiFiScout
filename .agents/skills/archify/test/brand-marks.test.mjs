...
test("brand discovery resolves model names, aliases, domains, and Chinese channel aliases", () => {
  for (const [query, expected] of [
    ...
    assert.ok(
      receipt.marks.some((mark) => mark.id === expected),
      query,
    );
  }
});
...
test("all five renderers keep the semantic sigil and add one export-safe brand badge", () => {
  for (const type of Object.keys(cases)) {
    ...
    const frame = block.match(
      /<rect x="([-\d.]+)" y="([-\d.]+)" width="([-\d.]+)" height="([-\d.]+)" rx="[^"]+" class="c-mask"\/>/,
    );
    const semantic = block.match(/data-semantic-sigil[^>]+translate\(([-\d.]+) ([-\d.]+)\)/);
    const brand = block.match(
      /data-brand-mark="openai"[^>]+translate\(([-\d.]+) ([-\d.]+)\)">\s*<rect width="([-\d.]+)" height="([-\d.]+)" rx="([-\d.]+)" class="brand-mark-badge"\/>/,
    );
    assert.ok(
      frame && semantic && brand,
      `${type}: expected node frame, semantic sigil, and brand badge`,
    );
    ...
    assert.equal(
      frameX + frameWidth - (brandX + brandWidth),
      6,
      `${type}: brand badge right inset`,
    );
...
test("unknown URL strings fail closed until an exact captured digest is authored", () => {
  ...
  assert.ok(
    receipt.diagnostics.some((entry) =>
      entry.supportedFixes.some((fix) => fix.includes("brands capture")),
    ),
  );
});
...
test("capture command returns a digest-pinned brand object that renders reproducibly", async () => {
  ...
  const server = http.createServer((request, response) => {
    ...
    response.end(
      '<!doctype html><title>Example Studio</title><link rel="icon" type="image/png" href="/mark.png"><h1>Example Studio</h1>',
    );
  });
  ...
  try {
    ...
    const rendered = await renderAsync("architecture", input, "captured-link", {
      ARCHIFY_BRAND_ALLOW_PRIVATE: "1",
    });
    ...
    assert.ok(
      !rendered.html.includes("http://127.0.0.1") ||
        rendered.html.includes("data-node-brand-source="),
    );
  } finally {
...
test("a pinned brand fails closed when the remote icon digest changes", async () => {
  ...
  const server = http.createServer((request, response) => {
    ...
    response.writeHead(200, { "content-type": "text/html; charset=utf-8" });
    response.end(
      '<!doctype html><title>Changing site</title><link rel="icon" type="image/png" href="/mark.png">',
    );
  });
  await new Promise((resolve) => server.listen(0, "127.0.0.1", resolve));
  try {
    ...
    const result = await runCliAsync(["validate", "architecture", input, "--json"], {
      ARCHIFY_BRAND_ALLOW_PRIVATE: "1",
    });
    assert.equal(result.status, 1, result.stderr || result.stdout);
    const receipt = JSON.parse(result.stdout);
    assert.equal(
      receipt.diagnostics.filter((entry) => entry.code === "brand/digest-mismatch").length,
      1,
    );
  } finally {
    await new Promise((resolve) => server.close(resolve));
...
test("a pinned brand keeps identical artifact metadata when the remote page title changes", async () => {
  ...
  const server = http.createServer((request, response) => {
    ...
    response.writeHead(200, { "content-type": "text/html; charset=utf-8" });
    response.end(
      `<!doctype html><title>Title ${pageHits}</title><link rel="icon" type="image/png" href="/mark.png">`,
    );
  });
  await new Promise((resolve) => server.listen(0, "127.0.0.1", resolve));
  try {
    ...
    const first = await renderAsync("architecture", input, "stable-title-first", {
      ARCHIFY_BRAND_ALLOW_PRIVATE: "1",
    });
    const second = await renderAsync("architecture", input, "stable-title-second", {
      ARCHIFY_BRAND_ALLOW_PRIVATE: "1",
    });
...
test("rendering many pinned brands limits concurrent remote capture work", async () => {
  ...
  const server = http.createServer((request, response) => {
    ...
    setTimeout(() => {
      if (request.url.endsWith(".png")) {
        ...
        active -= 1;
        response.end(
          `<!doctype html><title>Site ${suffix}</title><link rel="icon" type="image/png" href="/mark-${suffix}.png">`,
        );
      }
    }, 40);
  });
  ...
  try {
    ...
    const rendered = await renderAsync("architecture", input, "bounded-capture", {
      ARCHIFY_BRAND_ALLOW_PRIVATE: "1",
    });
    assert.equal(rendered.status, 0, rendered.stderr || rendered.stdout);
    assert.ok(
      maximumActive <= 3,
      `expected at most 3 concurrent requests, observed ${maximumActive}`,
    );
  } finally {
    await new Promise((resolve) => server.close(resolve));
  }
  ...
  const server = http.createServer((request, response) => {
    setTimeout(() => {
      if (request.url === "/mark.png") {
        ...
        response.writeHead(200, { "content-type": "text/html; charset=utf-8" });
        response.end(
          '<!doctype html><title>Slow site</title><link rel="icon" type="image/png" href="/mark.png">',
        );
      }
    }, 100);
  });
...
test("capture rejects remote SVG even when the document appears passive", async () => {
  const server = http.createServer((request, response) => {
    ...
    response.writeHead(200, { "content-type": "text/html; charset=utf-8" });
    response.end(
      '<!doctype html><title>SVG mark</title><link rel="icon" type="image/svg+xml" href="/mark.svg">',
    );
  });
  await new Promise((resolve) => server.listen(0, "127.0.0.1", resolve));
  try {
...
test("unsupported SVG declarations cannot crowd out the favicon fallback", async () => {
  ...
  const server = http.createServer((request, response) => {
    ...
    response.writeHead(200, { "content-type": "text/html; charset=utf-8" });
    response.end(
      `<!doctype html><title>Fallback mark</title>${Array.from(
        { length: 6 },
...
test("unknown preset names fail with a repairable public CLI diagnostic", () => {
  ...
  assert.ok(
    receipt.diagnostics.some((entry) =>
      entry.supportedFixes.some((fix) => fix.includes("archify brands")),
    ),
  );
});
...
test("viewer exposes brand identity to Passport and Finder while keeping source beacons clear", () => {
  ...
  assert.match(
    template,
    /sourceSearch \+ ' ' \+ text\)\.toLowerCase\(\) \+ ' ' \+ brand\.toLowerCase\(\)/,
  );
});
