...
test("all five renderers inherit one resolved-only Route Share Card Export item", () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(
      html,
      /data-action="route-share-card"[^>]*hidden disabled[^>]*>[\s\S]*?Route Share Card[\s\S]*?1200(?:&times;|×)630 PNG/,
      mode,
    );
...
test("Route Share Card menu lifecycle excludes hidden state from keyboard navigation", () => {
  ...
  assert.match(
    html,
    /items\(\)\.filter\(function \(i\) \{ return !i\.hidden && !i\.disabled; \}\)/,
  );
  assert.match(html, /function clear\(options\)[\s\S]*?Archify\.exportMenu\.syncRouteShare\(\)/);
  assert.match(
    html,
    /function showResult\(result, options\)[\s\S]*?Archify\.exportMenu\.syncRouteShare\(\)/,
  );
...
test("Route Share Card snapshots copy exact resolved node and relationship identity without rerouting", () => {
  ...
  const geometryBlock =
    html.match(/function hasDrawableGeometry\(element\) \{[\s\S]*?\n    \}/)?.[0] || "";
  ...
  assert.doesNotMatch(
    snapshotBlock,
    /shortestDirectedPath|outgoingByNode|reachableFrom|labelAt|nearest/i,
  );
});
...
test("Route variant decorates only a finite canonical clone with dedicated static attributes", () => {
  const html = render("architecture", CASES.architecture);
  const applyBlock =
    html.match(/function applyRouteSnapshot\(clone, snapshot\) \{[\s\S]*?\n      \}/)?.[0] || "";
  ...
  assert.doesNotMatch(
    applyBlock,
    /setAttribute\('data-route-(?:match|step|start|end|active|journey)/,
  );
  assert.match(
    html,
  ...
  assert.ok(
    html.indexOf("var canonicalStateClean =") <
      html.indexOf("applyRouteSnapshot(clone, opts.routeSnapshot)"),
    "canonical cleanup must precede route decoration",
  );
});
...
test("clone-only Route styling retains context and distinguishes start, middle, and end without motion", () => {
  ...
  assert.doesNotMatch(
    html.match(/if \(opts\.routeSnapshot\) \{[\s\S]*?\n        \}/)?.[0] || "",
    /display:\s*none|animation:|filter:|transform:/,
  );
});
...
test("skill and READMEs describe the optional Export variant and show one real card without changing the hero", () => {
  ...
  assert.match(
    ...
    "concurrent validator scratch must never leak into archify.zip",
  );
});
