...
function validateDataflow() {
  ...
  if (dataflow.diagram_type !== "dataflow")
    problems.push('Data-flow files must set "diagram_type": "dataflow".');
  ...
  if (!Array.isArray(dataflow.flows))
    problems.push("Data-flow diagrams must include a flows array.");
  ...
  for (const node of nodes.values()) {
    if (typeof node.stage !== "number" || node.stage < 0 || node.stage >= stageCount) {
      problems.push(
        `Node "${node.id}" uses invalid stage ${node.stage} — valid stages are 0..${stageCount - 1}.`,
      );
    }
    if (typeof node.row !== "number" || node.row < 0 || node.row >= layout.rowYs.length) {
      problems.push(
        `Node "${node.id}" uses invalid row ${node.row} — valid rows are 0..${layout.rowYs.length - 1}.`,
      );
    }
    ...
    if (
      node.y < layout.stageY + layout.stageH + 22 ||
      node.y + node.height > viewBox[1] - layout.stageBottomPad
    ) {
      problems.push(
  ...
  for (const flow of asArray(dataflow.flows)) {
    ...
    if (!nodes.has(flow.to))
      problems.push(`Flow "${flow.label || flow.to}" references unknown target "${flow.to}".`);
    if (!flow.label)
      problems.push(`Flow "${flow.from}" -> "${flow.to}" must include a short data label.`);
    if (nodes.has(flow.from) && nodes.has(flow.to)) {
      ...
      if (Array.isArray(flow.via)) {
        for (let segmentIndex = 0; segmentIndex < routed.points.length - 1; segmentIndex += 1) {
          ...
          const isDiagonal =
            Math.abs(segmentStart[0] - segmentEnd[0]) > 0.01 &&
            Math.abs(segmentStart[1] - segmentEnd[1]) > 0.01;
          if (!isDiagonal) continue;
          const viaIndex = Math.min(segmentIndex, flow.via.length - 1);
          problems.push(
            `Flow "${flow.label}" has a diagonal segment from (${segmentStart.join(", ")}) to (${segmentEnd.join(", ")}) — align via[${viaIndex}] with its adjacent point by sharing the same x or y coordinate.`,
          );
        }
      }
    }
...
const LEGEND_CATALOG = [
  ...
  },
  {
    kind: "security",
    label: "policy / PII",
    className: "a-security",
    marker: "arrowhead-security",
    swatchWidth: 34,
    swatchGap: 9,
    interactive: false,
  },
