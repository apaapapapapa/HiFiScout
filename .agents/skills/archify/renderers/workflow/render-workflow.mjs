...
function nodeContext(node) {
  const group = asArray(workflow.groups).find(
    (candidate) =>
      candidate.lane === node.lane && node.col >= candidate.fromCol && node.col <= candidate.toCol,
  );
  const phase = asArray(workflow.phases).find(
    (candidate) => node.col >= candidate.fromCol && node.col <= candidate.toCol,
  );
  return (
    [laneLabels.get(node.lane), group?.label, phase?.label].filter(Boolean).join(" › ") ||
    "Workflow node"
  );
}
...
function lastLaneBottom() {
  return (
    layout.laneY +
    workflow.lanes.length * layout.laneH +
...
function workflowCompositionFrames() {
  ...
  for (const [index, lane] of asArray(workflow.lanes).entries()) {
    ...
    if (lane.variant === "exception") {
      frames.push({
        id: `lane-${index}-exception`,
        label: `${lane.label} exception`,
        kind: "exception-lane",
        x: layout.laneX + 6,
        y: y + 6,
        width: layout.laneW - 12,
        height: layout.laneH - 12,
        radius: 8,
      });
    }
  }
...
function nodeStep(node) {
  return (
    ...
    asArray(workflow.mainPath).length +
      asArray(workflow.nodes).findIndex((item) => item.id === node.id)
  );
}
...
function validateWorkflow() {
  ...
  for (const node of nodes.values()) {
    ...
    if (estLabelW > node.width + 6) {
      problems.push(
        `Label "${node.label}" (~${Math.round(estLabelW)}px) is wider than node "${node.id}" (${node.width}px) — shorten the label or increase node.width.`,
      );
    }
    ...
    for (const [field, value, minimum] of [
      ...
      if (minimumW > availableTextW) {
        problems.push(
          `${field} "${value}" needs ~${Math.ceil(minimumW)}px at the ${minimum}px legible minimum, but node "${node.id}" provides ${availableTextW}px — shorten the ${field.toLowerCase()} or increase node.width.`,
        );
      }
    }
    ...
    if (node.y < contentTop || node.y + node.height > top + layout.laneH) {
      problems.push(
        `Node "${node.id}" collides with the title or boundary of lane "${node.lane}".`,
      );
    }
  }
  ...
  for (let i = 0; i < phaseRanges.length; i += 1) {
    for (let j = i + 1; j < phaseRanges.length; j += 1) {
      ...
      problems.push(
        `Phase "${later.id}" (${later.fromCol}..${later.toCol}) overlaps phase "${earlier.id}" (${earlier.fromCol}..${earlier.toCol}) — start at col ${earlier.toCol + 1} or later, or end the earlier phase at col ${later.fromCol - 1}.`,
      );
    }
  }
  ...
  for (const edge of workflow.edges) {
    ...
    if (!nodes.has(edge.to))
      problems.push(`Edge "${edge.label || edge.to}" references unknown target "${edge.to}".`);
  ...
  if (legendY() + 18 > viewBox[1]) {
    problems.push(
      `Legend exceeds viewBox height ${viewBox[1]} — set meta.viewBox[1] to at least ${legendY() + 18}.`,
    );
  }
...
function renderNode(node) {
  ...
  const labelFontSize = fittedNodeFontSize(
    node.label,
    brandLabelFitWidth(node, node.width),
    nodeTextFit.labelPreferred,
    nodeTextFit.labelMinimum,
  );
