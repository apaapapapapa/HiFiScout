...
function gitValue(repoRoot, args, failure) {
  ...
  if (result.status !== 0)
    evidenceFailure("repository-evidence/git-command", failure, {
...
function verifiedSourcePath(value, where) {
  ...
  if (
    ...
    sourcePath.includes("\\") ||
    CONTROL_CHARACTER_RE.test(sourcePath)
  ) {
    evidenceFailure(
      "repository-evidence/path-invalid",
      `${where} must be a repo-relative POSIX path.`,
      {
        subject: { path: where },
  ...
  if (
    ...
    evidenceFailure(
      ...
      `${where} must stay inside the repository and may not address .git.`,
      {
        subject: { path: where },
        evidence: { authoredPath: sourcePath },
...
export function hasRepositoryEvidence(diagramType, diagram) {
  ...
  return (
    Boolean(diagram?.meta?.repository) ||
    components.some((component) => Array.isArray(component?.sources) && component.sources.length)
  );
}
...
export function verifyRepositoryEvidence(diagramType, diagram, repoRootInput) {
  ...
  if (diagramType !== "architecture")
    evidenceFailure(
      "repository-evidence/type-unsupported",
      "Repository evidence is currently supported for architecture diagrams only.",
      {
        subject: { diagramType },
  ...
  if (!authoredSlug || !String(repository.url).startsWith("https://github.com/")) {
    evidenceFailure(
      ...
      "/meta/repository/url must be a public https://github.com owner/repository URL.",
      {
  ...
  try {
    ...
    evidenceFailure(
      ...
      `Could not resolve evidence repository root "${requestedRoot}": ${error.message}`,
      {
  ...
  const gitRoot = gitValue(
    realRoot,
    ["rev-parse", "--show-toplevel"],
    `Evidence root "${realRoot}" is not a Git repository.`,
  );
  if (fs.realpathSync(gitRoot) !== realRoot) {
    evidenceFailure(
      "repository-evidence/root-not-top-level",
      `Evidence root must be the Git top-level directory: ${gitRoot}`,
      {
  ...
  const origin = gitValue(
    realRoot,
    ["remote", "get-url", "origin"],
    "Evidence repository must have an origin remote.",
  );
  if (githubSlug(origin) !== authoredSlug) {
    evidenceFailure(
      "repository-evidence/origin-mismatch",
      `Evidence repository origin ${JSON.stringify(origin)} does not match ${JSON.stringify(repository.url)}.`,
      {
        subject: { repoRoot: realRoot },
  ...
  if (commit.status !== 0) {
    evidenceFailure(
      ...
      `Evidence revision ${revision} is not available in the local repository.`,
      {
        subject: { repoRoot: realRoot },
        evidence: { revision },
    ...
    for (const [sourceIndex, authored] of component.sources.entries()) {
      ...
      if (source.endLine && !source.line) {
        evidenceFailure(
          ...
          `/components/${componentIndex}/sources/${sourceIndex}/end_line requires line.`,
          {
            subject: {
              path: `/components/${componentIndex}/sources/${sourceIndex}/end_line`,
      ...
      if (source.endLine && source.endLine < source.line) {
        evidenceFailure(
          ...
          `/components/${componentIndex}/sources/${sourceIndex}/end_line must be greater than or equal to line.`,
          {
            subject: {
              path: `/components/${componentIndex}/sources/${sourceIndex}`,
              componentId: component.id,
            },
      ...
      if (type.status !== 0 || type.stdout.trim() !== "blob") {
        evidenceFailure(
          ...
          `${where} does not identify a file at revision ${revision}.`,
          {
      ...
      if (source.line) {
        ...
        if (content.status !== 0)
          evidenceFailure(
            ...
            `${where} could not be read at revision ${revision}.`,
            {
              subject: { path: where, componentId: component.id },
              evidence: { sourcePath: source.path, revision },
        ...
        if (requestedLine > lineCount) {
          evidenceFailure(
            ...
            `/components/${componentIndex}/sources/${sourceIndex} requests line ${requestedLine}, but ${source.path} has ${lineCount} lines at revision ${revision}.`,
            {
              subject: {
                path: `/components/${componentIndex}/sources/${sourceIndex}`,
                componentId: component.id,
  ...
  if (referenceCount === 0) {
    evidenceFailure(
      ...
      "/meta/repository requires at least one component source reference.",
      {
        subject: { path: "/meta/repository" },
        supportedFixes: [
          "add at least one verified component source or remove repository metadata",
        ],
      },
    );
  }
