...
export function listBrandMarks(query = "") {
  ...
  return BRAND_MARKS.filter((mark) => {
    ...
    return [mark.id, mark.title, mark.category, ...mark.aliases, ...mark.domains].some((value) =>
...
function validateUrlShape(url, allowPrivate = process.env.ARCHIFY_BRAND_ALLOW_PRIVATE === "1") {
  if (!["https:", "http:"].includes(url.protocol))
    throw new Error("only HTTP(S) brand links are supported");
...
async function resolveRequestTarget(url, deadline) {
  ...
  if (
    !addresses.length ||
    (!allowPrivate && addresses.some(({ address }) => isPrivateBrandAddress(address)))
  ) {
    throw new Error("private brand links are not fetched");
  }
  return addresses[0];
...
function iconCandidates(html, pageUrl) {
  ...
  for (const match of html.matchAll(/<link\b[^>]*>/gi)) {
    ...
    if (
      !rel.some(
        (value) => value === "icon" || value === "apple-touch-icon" || value === "mask-icon",
      )
    )
      continue;
    ...
    try {
      ...
      const area = [...sizes.matchAll(/(\d+)x(\d+)/gi)].reduce(
        (best, size) => Math.max(best, Number(size[1]) * Number(size[2])),
        0,
      );
      const score =
        (type.includes("svg") || /\.svg(?:$|[?#])/i.test(url.href) ? 1000000 : 0) +
...
export async function captureBrandReference(value) {
  ...
  if (preset)
    return { brand: preset.id, resolved: { ...preset, kind: "preset", status: "preset" } };
...
function suggestions(value) {
  ...
  return BRAND_MARKS.map((mark) => ({
    id: mark.id,
    score: lookupForms(mark.id).some((form) => form.includes(needle) || needle.includes(form))
      ? 0
      : 1,
  }))
  ...
  await mapConcurrent(nodes, MAX_CAPTURE_CONCURRENCY, async (node, index) => {
    ...
    if (typeof node.brand === "object") {
      ...
      if (resolved.sha256 !== node.brand.sha256) {
        unknown.push(
          `/${collection}/${index}/brand digest changed: expected ${node.brand.sha256}, received ${resolved.sha256}`,
        );
        return;
      }
    ...
    if (url) {
      unknown.push(
        `/${collection}/${index}/brand ${JSON.stringify(node.brand)} is an unpinned URL; capture it first with \`archify brands capture ${url.href} --json\``,
      );
      return;
    }
    unknown.push(
      `/${collection}/${index}/brand ${JSON.stringify(node.brand)} is not a built-in brand; closest IDs: ${suggestions(node.brand).join(", ")}`,
    );
  });
  if (unknown.length) {
    throwDiagnosticError(
      ...
      unknown.map((message) => ({
        code: message.includes("is an unpinned URL")
          ? "brand/unpinned-url"
          : message.includes("digest changed")
            ? "brand/digest-mismatch"
            : message.includes("could not reproduce")
...
export function brandTopRailProblem(node, width, minimumFontSize, subject = "Node") {
  ...
  return (
    `${subject} "${node.id}" brand top rail leaves ${Math.max(0, available)}px for its label, but ` +
