...
export function relationshipLegendObstacles(relations, { pointsFor, labelRectFor } = {}) {
  ...
  for (const [index, relation] of (Array.isArray(relations) ? relations : []).entries()) {
    ...
    if (
      labelRect &&
      [labelRect.x, labelRect.y, labelRect.width, labelRect.height].every(Number.isFinite)
    ) {
...
export function legendFootprint(
  entries,
  {
    width,
    ...
    throwDiagnosticError(message, [
      {
        ...
        evidence: {
          ...
          requiredTopY: legendTopY,
          availableTopY: minTitleY,
        },
        supportedFixes: [
...
export function renderLegend({ entries, layout, renderSwatch }) {
  ...
  for (const entry of measured.entries) {
    const interactive = entry.interactive
      ...
      : "";
    parts.push(
      `          <g data-legend-semantic-kind="${esc(entry.kind)}"${interactive} data-legend-x="${entry.x}" data-legend-baseline="${entry.baseline}" data-legend-width="${entry.width}">`,
    );
    parts.push(`            ${renderSwatch(entry)}`);
    parts.push(
      `            <text x="${entry.x + (entry.swatchWidth ?? 14) + (entry.swatchGap ?? DEFAULT_SWATCH_GAP)}" y="${entry.baseline}" class="t-muted" font-size="${renderedFontSize}" font-weight="500">${esc(entry.label)}</text>`,
    );
    parts.push("          </g>");
  }
  parts.push("        </g>");
