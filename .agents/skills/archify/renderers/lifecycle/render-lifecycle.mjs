...
function measureState(state) {
  ...
  const width =
    state.width || (isPhase ? layout.phaseW : isOutcome ? layout.outcomeW : layout.eventW);
  const height =
    state.height || (isPhase ? layout.phaseH : isOutcome ? layout.outcomeH : layout.eventH);
  const xs = isPhase ? layout.phaseXs : isOutcome ? layout.outcomeXs : layout.eventXs;
  const cx = xs[state.col] ?? xs[xs.length - 1];
  const y =
    (isPhase ? layout.phaseY : isOutcome ? layout.outcomeY : layout.eventY) + (state.yOffset || 0);
  return {
    ...state,
    width,
    height,
...
function validateLifecycle() {
  ...
  if (lifecycle.schema_version !== 1)
    problems.push('Lifecycle files must set "schema_version": 1.');
  if (lifecycle.diagram_type !== "lifecycle")
    problems.push('Lifecycle files must set "diagram_type": "lifecycle".');
  ...
  if (!Array.isArray(lifecycle.transitions))
    problems.push("Lifecycle diagrams must include a transitions array.");
  ...
  for (const state of states.values()) {
    ...
    if (state.y < 64 || state.y + state.height > lifecycleAreaBottom()) {
      problems.push(
        `State "${state.id}" exceeds the vertical lifecycle area — keep y between 64 and ${lifecycleAreaBottom()} (adjust yOffset or increase meta.viewBox[1]).`,
      );
    }
    const estLabelW = textUnits(state.label) * 6.2;
    if (estLabelW > state.width + 6) {
      problems.push(
        `Label "${state.label}" (~${Math.round(estLabelW)}px) is wider than state "${state.id}" (${state.width}px) — shorten the label or increase state.width.`,
      );
    }
    ...
    for (const [field, value, minimum] of [
      ...
      if (minimumW > availableTextW) {
        problems.push(
          `${field} "${value}" needs ~${Math.ceil(minimumW)}px at the ${minimum}px legible minimum, but state "${state.id}" provides ${availableTextW}px — shorten the ${field.toLowerCase()} or increase state.width.`,
        );
      }
    }
  }
  ...
  for (const transition of asArray(lifecycle.transitions)) {
    ...
    if (!states.has(transition.to))
      problems.push(
        `Transition "${transition.label || transition.to}" references unknown target "${transition.to}".`,
      );
  ...
  problems.push(
    ...cleanCrossingProblems({
      ...
      profile: lifecycle.meta?.quality_profile,
      routeHint:
        "adjust route/via or channelX/channelY so the transitions use separate lifecycle corridors",
    }),
  );
  problems.push(
    ...cleanAmbiguousCorridorProblems({
...
function pathFor(transition) {
  ...
  const start =
    ports?.from || anchor(from, chosenSide(transition.fromSide, defaultFromSide(from, to)));
  ...
  if (
    ports &&
    !via.length &&
    Math.abs(start[0] - end[0]) >= 4 &&
    Math.abs(start[1] - end[1]) >= 4
  ) {
...
function bandTitles() {
  ...
  return [
    ...
    eventLanes.length
      ? eventLanes.map((lane) => lane.label).join(" + ")
...
function renderState(state) {
  ...
  const labelFontSize = fittedNodeFontSize(
    state.label,
    brandLabelFitWidth(state, state.width),
    10,
    8,
  );
  const passport = {
    ...
    tag: state.tag,
    context: laneLabels.get(state.lane) || "Lifecycle state",
...
function renderLegend() {
  ...
  return renderResolvedLegend({
    ...
    renderSwatch: (entry) =>
      `<rect x="${entry.x}" y="${entry.baseline - 8}" width="14" height="9" rx="2" class="${typeClass[entry.kind] || "c-external"}" stroke-width="1"/>`,
  });
}
