...
function autoViewBox() {
  ...
  return [width, Math.ceil(maxY + layout.margin + layout.legendH + footprint.extraHeight)];
}
...
function validateArchitecture() {
  ...
  if (arch.diagram_type !== "architecture")
    problems.push('Architecture files must set "diagram_type": "architecture".');
  ...
  if (arch.cards !== undefined && !Array.isArray(arch.cards))
    problems.push('Architecture "cards" must be an array.');
  if (components.size !== asArray(arch.components).length)
    problems.push("Component ids must be unique.");
  if (grid) {
  ...
  for (const c of components.values()) {
    if (!isFinitePoint(c.x, c.y, c.width, c.height)) {
      problems.push(
        `Component "${c.id}" has non-finite pos/size — pos and size must be [number, number].`,
      );
      continue;
    }
    ...
    if (estLabelW > c.width + 8) {
      problems.push(
        `Label "${c.label}" (~${Math.round(estLabelW)}px) is wider than component "${c.id}" (${c.width}px) — shorten the label or widen size.`,
      );
    }
    ...
    for (const [field, value, minimum] of [
      ...
      if (minimumW > availableTextW) {
        problems.push(
          `${field} "${value}" needs ~${Math.ceil(minimumW)}px at the ${minimum}px legible minimum, but component "${c.id}" provides ${availableTextW}px — shorten the ${field.toLowerCase()} or widen size.`,
        );
      }
    }
  }
  ...
  for (const conn of asArray(arch.connections)) {
    if (!components.has(conn.from))
      problems.push(
        `Connection "${conn.label || conn.from}" references unknown source "${conn.from}".`,
      );
    if (!components.has(conn.to))
      problems.push(
        `Connection "${conn.label || conn.to}" references unknown target "${conn.to}".`,
      );
...
function sideAwareBridgeCandidates(start, end, fromSide, toSide) {
  ...
  return rawCandidates
    ...
    .filter(
      (points) => !collinearBacktrack(points.at(-3) || points.at(-2), points.at(-2), points.at(-1)),
    )
...
function alignFacingPorts(conn, from, to, start, end, fromSide, toSide, ports) {
  ...
  for (const candidate of candidates) {
    ...
    if (
      portHasCornerClearance(from, fromSide, candidate.start) &&
...
function routeVia(conn, from, to, start, end, fromSide, toSide) {
  ...
  switch (conn.route || "auto") {
    ...
    default: {
      ...
      if ((deltaX < 4 || deltaY < 4) && routeHonorsEndpointSides([start, end], fromSide, toSide))
        return [];

      const rhythmBridge = automaticPortRhythmBridge(start, end, fromSide, toSide, {
        accept: (points) =>
          routeClearsEndpointComponents(points, from, to) && routeClearsComponents(conn, points),
      });
      ...
      for (const candidate of ordered) {
        const points = [start, ...candidate, end];
        if (routeClearsEndpointComponents(points, from, to) && routeClearsComponents(conn, points))
          return candidate;
      }
