...
自動修復のseedingは、各resolverのインデックスから候補を一定件数だけmaterializeしてから、
キュー登録済みかを照合します。登録済み候補や非アクティブなIdentity行も走査位置を進め、
同じ先頭部分の読み直しを避けます。末尾では折り返し、resolverのversion tupleが変われば
先頭へ戻ります。候補のキュー登録が完了してからチェックポイントを進めるため、予算切れ・
例外で未登録候補を飛ばしません。開始selectorを巡回させ、一つのbacklogによる他段階の
飢餓を防ぎます。状態はmigration 0100の小さなcursor表だけに持ち、商品更新用の追加indexは
作りません。`seedScannedCount`とD1の実測値を、登録件数とは別に確認してください。
...
- 「リモコン・元箱付属」のような列挙とスタンドのプレゼントは、販売対象の本体と区別します。
  本体の明示がない「純正リモコン 元箱付属」はリモコンの出品として保持します。
  メーカー名の一部分を部品名として拾わず、NOBUNAGA Labsのようにケーブル・イヤホン・部品を
  扱うメーカーはブランドだけで分類しません。公式情報で確認した完全一致の型番根拠は製品種別
  にだけ使用し、カタログ同定や限定版の統合は別の判定を維持します。
  フジヤエービックの「アナログプレーヤー」パンくずにはフォノアンプも含まれるため補助根拠とし、
  保存済みの同じラベルも再生時に同じ強さへ見直します。確認済みの型番根拠で分類できる商品は、
  古い詳細キャッシュを再利用する前に分類します。
  限定した修正のために全件の分類versionや詳細キャッシュを失効させず、確認済みIDを既存の
  管理再生経路で処理します。保存された判定根拠と現行カテゴリが食い違う商品は、古い根拠で
  過去の修正を戻さないよう個別に確認し、既存の管理overrideと複数構成品を保持します。
