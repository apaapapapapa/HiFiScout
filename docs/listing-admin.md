...
カタログ製品一覧の「写真」から、製品単位でメーカー写真を1枚登録・差し替え・掲載取り消しできます。
公式製品ページの出典URL、画像URL、任意のクレジットを登録すると、公開検索・商品詳細（直接リンクを含む）・比較に表示されます。
写真は機種の参考情報であり、販売店の実物写真・仕上げ・付属品・状態を示すものではありません。
写真未登録の製品とカタログ未確定の出品は従来どおり文字情報を表示します。

「公式ページから候補を取得」は既存のメーカー公式サイト登録を使用します。1回の操作で1ページのみ、
最大8秒・512 KB・2回の許可済みオリジン内リダイレクトに制限し、Product構造化データとOGPから最大8候補を返します。
取得先はページ移動のたびに検証します。候補を選んで型番・色・出典を確認してから「写真を登録」を押してください。
対応外のメーカーや候補がないページは、画像URLを手動で登録できます。既存製品全件の自動取得・定期巡回は行いません。

画像と出典は公開HTTPS URLのみ。写真ファイル自体は保存せず、ブラウザーが参照元へリファラーを送らずに読み込みます。
変更はリビジョンで競合を検出し、同一内容の再送はDBに書き込みません。掲載取り消しもリビジョンを保持します。
製品統合は写真を引き継ぎ、異なる写真がある場合は全体をロールバックして確認を求めます。
公開キャッシュへの反映には最大30秒かかります。「全情報ZIP」の `catalog_product_photos` に登録内容を含めます。
編集用CSVでの写真変更には未対応です。
