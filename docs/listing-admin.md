...
候補窓から最大5商品を再判定し、カーソルを永続化する。ショップ指定は既存の
`(shop_key,is_active,rowid)` 索引から稼働中・保持中の両方を読む。辞書はジョブ実行中に
...
新規メーカーはIDを指定して開き、正式名称を入力する。保存済みの別名一覧から有効／無効と
ショップ範囲を選び直せる。影響の確認後に内容を編集すると、確認結果を破棄して再確認を
求める。適用応答が不明な間は入力を固定し、同じ操作IDの再送だけを行う。
