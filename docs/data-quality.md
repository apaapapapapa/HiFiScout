...
Official page policy v4 requires model evidence from product content, matching structured identifiers or an eligible product heading. Navigation does not establish the model. Breadcrumb containers and document metadata are excluded from model-local evidence; pipe-delimited title navigation is not a product category or canonical display name. Product-local evidence precedes the remaining product-title segment. Conflicting structured/local category evidence is terminal; only insufficient evidence permits fallback. A page-wide category label cannot classify a sibling mentioned only in an index paragraph. The persisted `verified_from_official_product_page_v4` message distinguishes this policy from older verification results for targeted, budgeted review through existing verification operations. Deployments do not refetch the full verified catalog.
...
Each scheduled remediation sweep finalizes quality for the shops whose jobs it processed before resolving those durable jobs. It can reuse a snapshot only when its `source_revision` matches the shop's durable `data_quality_snapshot_state.revision` and no newer remediation event exists. Missing or legacy coverage requires recomputation. The same `saveDataQualityRun` used by crawls captures the revision before reading the aggregate and persists it with the metrics; a failed read/INSERT or a concurrent listing write cannot certify an old snapshot. SQL triggers cover active listing insertion, deletion, shop moves, inventory/manufacturer/category/model changes and Identity changes, including writes from an older Worker. They coalesce changes to one revision write per shop between snapshot captures, and ignore same-value writes, timestamps, resolver-version metadata, facets and search projections. A retry from an older writer cannot inherit a new writer's coverage on the same crawl row.

When recomputation is needed, the remediation snapshot leaves `crawl_run_id` null. Crawl-only run metrics (parser failure, evidence coverage, item-count drop) correctly report `unknown`; snapshot metrics reflect the aggregate that was actually read. An intervening change leaves its revision unmatched for the next touched-shop sweep. No synthetic crawl or scheduled full scan is added. An empty sweep writes no snapshot, and history retains the existing 180-day policy.
