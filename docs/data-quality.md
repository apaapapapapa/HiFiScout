...
The finish is the one removed annotation that is kept rather than discarded. Removing it from the model is what lets `MC Cadenza Black` and `MC Cadenza Bronze` be one product instead of two, so it must never reach `normalized_model`; but it is also what the shopper is choosing between, so deleting it made two different things look identical on one card.
