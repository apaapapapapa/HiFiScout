...
`GET /api/admin/data-quality/status` returns the overall status and latest quality record for each shop. Each shop exposes `snapshot` and `latestRun` separately, while the flattened `metrics` object supports lightweight operational UI consumers. The `details` object includes `identityResolutionMissingCount` so operators can distinguish ordinary unresolved identities from listings that have no resolution row at all. Each shop also carries `trend`: for every snapshot and crawl-run rate metric (manufacturer/category/identity/inventory/model plus parser failure and evidence coverage), `previousRate`, `previousStatus`, `absoluteDelta` (current rate minus previous rate), and `percentageDelta` (that delta relative to the previous rate) against the run immediately before it. A shop's first-ever stored run reports `null` throughout its `trend` rather than a fabricated baseline. `remediationSlo` (see below) and a top-level `remediationQueue` — the same backlog/failure-rate view as the operational metrics used internally — are both included so this one endpoint carries current value, threshold/status, previous value, delta, and remediation health together.
...
`GET /api/admin/data-quality/unresolved-models?limit=<n>` aggregates model extraction failures by canonical manufacturer, normalized model, status, and resolver method. `GET /api/admin/data-quality/unresolved-identity?limit=<n>` aggregates listings Product Identity still refuses to match, keyed by canonical manufacturer plus normalized model and carrying the current rejection reason, shop spread, and sample evidence — the same views also serve as the "top catalog candidates by affected listings and distinct shops" the candidate columns expose. `GET /api/admin/data-quality/remediation-events?limit=<n>` returns recent before/after provenance for canonical changes that remediation caused. `GET /api/admin/data-quality/identity-method-distribution` returns, per shop, how active listings are distributed across `product_identity_resolutions.match_method` (exact, catalog alias, vetoed, fuzzy candidate, and so on); a listing with no resolution row is folded into a `missing_resolution` bucket instead of disappearing, the same convention the snapshot metrics use elsewhere.

`POST /api/admin/data-quality/replay-manufacturers` and `POST /api/admin/data-quality/replay-models` reprocess one bounded page of listings whose stored result predates the current resolver version or whose downstream projection refresh is still pending. `POST /api/admin/knowledge-catalog/replay` applies one verified catalog product to the listings it explains. All accept optional `afterId`/`limit` and return `nextAfterId` when the caller must resume; none contacts a seller site. All of these require `ADMIN_TOKEN`.

Before/after comparison since a remediation rollout began is deliberately not a separate endpoint: the oldest and newest rows in a shop's `/history` page already give that comparison, bounded by the same 180-day retention as everything else, without this surface inventing a second notion of "baseline."
