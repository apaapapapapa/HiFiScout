...
`syncProductSearchEntities` is scoped strictly to the listings it is given and the identity peers they regroup. Fallback lookup and pruning use the existing unique `entity_key` index with `l-<listing ID>` keys. Catalog entity upserts resolve requested listing IDs before looking up their verified catalog rows. Inactive-offer deletion uses one integer-primary-key equality per listing, inside the existing atomic projection batch. A fixed `IN` list alone is insufficient: stale statistics can make SQLite scan the entire offer table even for an empty deletion. SQL statement count grows with the listing chunk; binding round trips do not increase, and unchanged deletions write no rows.
