...
`syncProductSearchEntities` is scoped strictly to the listings it is given and the identity peers they regroup. Single-seed peer discovery uses a direct indexed probe. Multi-seed discovery materializes the input IDs as `VALUES` and joins each by integer primary key before deduplicating manufacturer/model identities; it then evaluates category compatibility and collects peer IDs in one indexed pass per identity. Repeated members therefore do not multiply a correlated scan, eligible peers are not read a second time, and a long `IN` list cannot persuade stale statistics to scan the product table for seeds. Fallback lookup and pruning use the existing unique `entity_key` index with `l-<listing ID>` keys. Catalog entity upserts resolve requested listing IDs before looking up their verified catalog rows. Inactive-offer deletion uses one integer-primary-key equality per listing, inside the existing atomic projection batch. A fixed `IN` list alone is insufficient: stale statistics can make SQLite scan the entire offer table even for an empty deletion. SQL statement count grows with the listing chunk; binding round trips do not increase, and unchanged deletions write no rows.
...
出品のない確認済みCatalogも既存の恒久詳細経路で仕様を表示する。Catalog統合では、元Catalogの仕様を同一トランザクションで引き継いでから元の行を削除する。統合先と異なる仕様がある場合は全体をロールバックして409で確認を促し、管理者の記録を黙って上書きしない。
