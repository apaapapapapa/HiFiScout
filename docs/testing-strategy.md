...
Frontier tests cover all-empty and late-hit page sets at 100, 1,000 and 10,000 pages and require the nonempty partial index. Metadata tests prove public reads remain two small queries as listings grow, and that fresh or failed snapshot refreshes behave correctly. Search-write tests run the crawler path against real migrated SQLite and compare projection writes and FTS storage through price-only and real text changes. Invocation-budget tests include failed calls, batch accounting, admission before writes, deadline yields, lease fencing and continuation on later ticks.
