...
`test/observed-sql-read-budget.test.ts` extends real D1 accounting to 100, 1,000 and 10,000 unrelated listings, entities and verified catalog rows. It bounds unchanged projection replay, empty pending repair and in-stock date pagination, and requires unchanged replay to write zero rows. It also checks that a 40-entity stale-category prune amid 10,000 unrelated memberships uses `idx_product_search_entity_offers_entity`, stays below 300 reads with zero writes when nothing is stale, and still deletes an unsupported membership. `test/in-stock-search-order.test.ts` verifies the migration backfill, cursor pages and refreshed ordering with mixed stock states and inactive offers. The D1 migration upgrade gate also creates and updates entities with the actual previous runtime after migration and rollback, checking the new runtime's in-stock order. Extra offer filters retain their matching-subset semantics; these page budgets do not claim constant cost for every filtered aggregate or exact total.
...
[Load contracts and evidence capture](./load-regression-prevention.md) define the maintained
workloads and absolute ceilings. CI collects their source-bound samples from these same suites.
