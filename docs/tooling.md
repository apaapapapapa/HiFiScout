...
`.github/workflows/docs.yml` builds the developer site for pull requests and pushes to `main`.
Generated schema documentation may reuse its verified input-fingerprint cache; other references
are regenerated by the documentation commands. The workflow checks documented commands, generated
Mermaid/dependency views, OpenAPI/Redoc output, TypeDoc/VitePress references, database documentation,
and static report links. Its separate architecture-rule check skips Markdown-only changes.
A successful build uploads the complete site as `developer-docs`; pushes to `main` also deploy
it to GitHub Pages.
