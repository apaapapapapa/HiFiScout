    ...
    <>
      ...
      >
        <div className="filters-head">
          <strong>絞り込み</strong>
          <button id="filter-close" className="filter-close" type="button" aria-label="絞り込みを閉じる" onClick={onClose}>
            ×
          </button>
        </div>
        ...
        <label>
          <span>ショップ</span>
          <select id="shop" value={filters.shop} onChange={(event) => onValueChange("shop", event.target.value)}>
            <option value="">すべて</option>
            {shops.map((shop) => (
              <option key={shop.key} value={shop.key}>{shop.name}</option>
            ))}
          </select>
        </label>
        <label>
        ...
        <label>
          <span>最低価格</span>
          <input id="minPrice" inputMode="numeric" placeholder="0" value={filters.minPrice} onChange={(event) => onValueChange("minPrice", event.target.value, true)} />
        </label>
        <label>
          <span>最高価格</span>
          <input id="maxPrice" inputMode="numeric" placeholder="1000000" value={filters.maxPrice} onChange={(event) => onValueChange("maxPrice", event.target.value, true)} />
        </label>
        <label className="check">
          <input id="inStock" type="checkbox" checked={filters.inStock} onChange={(event) => onToggleChange("inStock", event.target.checked)} />
          <span>在庫ありのみ</span>
        </label>
        <label className="check">
          <input id="recentOnly" type="checkbox" checked={filters.recentOnly} onChange={(event) => onToggleChange("recentOnly", event.target.checked)} />
          <span>48時間以内の新着</span>
        </label>
        <label className="check">
          <input id="priceDropped" type="checkbox" checked={filters.priceDropped} onChange={(event) => onToggleChange("priceDropped", event.target.checked)} />
          <span>値下げ商品</span>
        </label>
        <label className="check">
          <input id="favoritesOnly" type="checkbox" checked={filters.favoritesOnly} onChange={(event) => onToggleChange("favoritesOnly", event.target.checked)} />
          <span>お気に入りのみ <small id="favorites-count">({favoriteCount})</small></span>
        </label>
...
function SyncStatus({ meta }: SyncStatusProps) {
  ...
  return (
    <details ref={detailsRef} id="sync-status" className={`sync-status${sync ? ` ${sync.status}` : ""}`}>
      ...
      <div id="sync-status-details" className="sync-status-details" dangerouslySetInnerHTML={{ __html: meta ? syncShopRows(meta.shops || []) : "" }} />
      <button
...
function App() {
  ...
  const [favorites, setFavorites] = useState<FavoriteStore>(() => parseFavoriteStorage(localStorage.getItem(FAVORITES_KEY), isProductSearchItem));
  ...
  const selectedCategoryLabel = useMemo(() => {
    ...
    return meta.categoryFacets?.find((facet) => facet.id === filters.category)?.name
  ...
  const loadProducts = useCallback(async (nextFilters: ProductFilters, { page = 1, reset = false }: { page?: number; reset?: boolean } = {}) => {
  ...
  const clearFilter = useCallback((id: string) => {
    ...
    if (id === "inStock" || id === "favoritesOnly" || id === "recentOnly" || id === "priceDropped") {
      next[id] = false;
    } else if (id === "q" || id === "shop" || id === "manufacturer" || id === "category" || id === "minPrice" || id === "maxPrice") {
      next[id] = "";
    }
    commitFilters(next);
  }, [commitFilters]);
  ...
  const clearAllFilters = useCallback(() => {
    const next: ProductFilters = {
      q: "", shop: "", manufacturer: "", category: "", minPrice: "", maxPrice: "", sort: filtersRef.current.sort || DEFAULT_SORT,
  ...
  useEffect(() => {
    if (offersContent !== null && offersDialogRef.current && !offersDialogRef.current.open) offersDialogRef.current.showModal();
  }, [offersContent]);
  useEffect(() => {
    if (historyContent !== null && historyDialogRef.current && !historyDialogRef.current.open) historyDialogRef.current.showModal();
  }, [historyContent]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (cancelled) return;
        setMeta(result);
        const nextShops = Object.fromEntries(result.shops.map((shop): [string, MetaShop] => [shop.key, shop]));
        setShops(nextShops);
  ...
  const visibleProducts = useMemo(() => favoriteMode
    ? favoriteResults(favorites, filters, selectedCategoryLabel)
    : products, [favoriteMode, favorites, filters, products, selectedCategoryLabel]);
  ...
  const productsMarkup = useMemo(() => {
    ...
    const cards = visibleProducts.map((product) => productCard(product, {
  ...
  }, [errorMessage, favoriteCount, favoriteMode, favorites.legacyIds.size, favorites.products, shopName, visibleProducts]);

  useLayoutEffect(() => {
    if (productsRef.current) decorateProducts(productsRef.current);
  }, [productsMarkup]);

  const productClick = useCallback((event: ReactMouseEvent<HTMLElement>) => {
    const target = eventElement(event);
    if (!target) return;
    const manufacturer = target.closest<HTMLElement>("[data-manufacturer-filter]")?.dataset.manufacturerFilter?.trim();
    if (manufacturer) {
  ...
  const dialogClick = useCallback((event: ReactMouseEvent<HTMLElement>) => {
    const target = eventElement(event);
    if (!target) return;
    ...
    <>
      <header className="hero">
        <div>
          ...
          <a className="brand-home" href="/" aria-label="HiFiScout トップページへ"><h1>HiFiScout</h1></a>
      ...
      <main>
        <section className="search-shell" aria-label="商品検索">
          <label className="search-primary" htmlFor="q">
            ...
            <div className="search-row">
              <input id="q" type="search" placeholder="例: TAD ME1 / LUXMAN / DAC" autoComplete="off" value={filters.q} onChange={(event) => changeValue("q", event.target.value, true)} />
              <button id="filter-toggle" className="filter-toggle" type="button" aria-controls="filter-panel" aria-expanded={filterOpen} onClick={() => { if (isMobile) setFilterOpen(true); }}>
                絞り込み <span id="filter-count" className="filter-count" hidden={detailFilterCount === 0}>{detailFilterCount}</span>
              </button>
            </div>
          </label>
        </section>
          ...
          {activeFilters.length ? (
            <>
              {activeFilters.map((entry) => (
                <button key={entry.id} type="button" className="filter-chip" data-clear-filter={entry.id} aria-label={`${entry.label}を解除`} onClick={() => clearFilter(entry.id)}>
                  {entry.label} <span aria-hidden="true">×</span>
                </button>
              ))}
              <button type="button" className="clear-all" data-clear-all onClick={clearAllFilters}>すべて解除</button>
            </>
        ...
        <div className="result-header">
          <div className="result-count">
            <strong id="count">{summary.count}</strong><span id="count-label">{summary.label}</span>
            <span id="more-available" className="more-available" hidden={summary.moreHidden}>さらに商品があります</span>
          </div>
          <span id="loading" className={`loading${loading ? " show" : ""}`} role="status" aria-live="polite">更新中</span>
          <div className="result-tools">
            <label className="sort-control"><span>並び順</span><select id="sort" value={filters.sort} onChange={(event) => changeValue("sort", event.target.value)}><option value="newest">新着・更新順</option><option value="oldest">更新が古い順</option><option value="priceAsc">価格が安い順</option><option value="priceDesc">価格が高い順</option></select></label>
            <div className="view-switch" role="group" aria-label="表示形式">
              <button type="button" data-view="list" aria-label="リスト表示" className={view === "list" ? "active" : ""} aria-pressed={view === "list"} onClick={() => changeView("list")}>リスト</button>
              <button type="button" data-view="cards" aria-label="カード表示" className={view === "cards" ? "active" : ""} aria-pressed={view === "cards"} onClick={() => changeView("cards")}>カード</button>
            </div>
          </div>
        </div>

        <p id="favorites-note" className="favorites-note" hidden={!favoriteMode}>お気に入りはこの端末にのみ保存されます。価格や在庫は最後に表示した時点の情報です。</p>
        <section ref={productsRef} id="products" className={`products view-${view}`} aria-live="polite" onClick={productClick} dangerouslySetInnerHTML={{ __html: productsMarkup }} />
        <nav id="pagination" className="pagination" aria-label="商品一覧のページ">
          {!favoriteMode && pagesRef.current.size > 0 && totalPages > 1 ? pageNumbers(currentPage, totalPages).map((page, index, numbers) => (
            <span key={page}>
              {index > 0 && page - numbers[index - 1] > 1 ? <span className="page-ellipsis" aria-hidden="true">…</span> : null}
              <button type="button" className={`page-button${page === currentPage ? " active" : ""}`} data-page={page} aria-label={`${page}ページ目`} aria-current={page === currentPage ? "page" : undefined} disabled={loading} onClick={() => gotoPage(page)}>{page}</button>
            </span>
          )) : null}
        </nav>
      </main>

      <dialog ref={offersDialogRef} id="offers-dialog" aria-labelledby="offers-title" onClick={dialogClick}>
      ...
      <footer>
        <p>HiFiScout は各販売店とは関係のない非公式の横断検索ツールです。価格・在庫・商品状態は必ず販売店の商品ページで確認してください。</p>
