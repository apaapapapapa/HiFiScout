...
export function SpecificationFilterControls({
  values = {},
  disabled,
  onChange,
}: {
      ...
      {SPECIFICATION_FILTER_DEFINITIONS.map(({ id, name, unit, integer }) => (
        <label key={id}>
          ...
          <input
            ...
            aria-describedby={`specification-filter-help spec-error-${id}`}
            onChange={(event) => onChange(id, event.currentTarget.value)}
          />
          <span id={`spec-error-${id}`} className="field-error" role="status">
            {errors[id]}
          </span>
        </label>
      ))}
      <p className="filter-note">
        XLRはバランス、RCAはラインの系統数です。フォノ・同軸デジタルは含めません。
      </p>
      {disabled ? (
        <p className="filter-note">お気に入り表示中は詳細仕様で絞り込めません。</p>
      ) : null}
    </fieldset>
  );
}
