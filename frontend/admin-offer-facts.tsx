...
export function AdminOfferFacts({
  listingId,
  onClose,
}: {
  listingId: number;
  ...
  const manual = new Map(
    (snapshot?.facts ?? [])
      ...
      .map((fact) => [fact.factId, fact.state]),
  );
  const changes = Object.fromEntries(
    OFFER_FACT_DEFINITIONS.filter(
      (fact) =>
        draft[fact.id] !== undefined && draft[fact.id] !== (manual.get(fact.id) ?? "inherit"),
  ...
  const save = async () => {
    ...
    setStatus("保存しています…");
    try {
      const updated = await adminJson<Snapshot>(path, {
        method: "PATCH",
  ...
  return (
    <dialog
      ref={dialog}
      aria-labelledby="offer-fact-editor-heading"
      onClose={onClose}
      onCancel={(event) => {
