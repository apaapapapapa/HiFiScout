...
function priceRange(product: DisplayProduct): string {
  ...
  return low == null ? "—" : high == null || low === high ? yen.format(low) : `${yen.format(low)}〜${yen.format(high)}`;
}

function activity(product: DisplayProduct): string {
  const value = product.latest_activity_at;
  return value && Number.isFinite(Date.parse(value)) ? new Date(value).toLocaleString("ja-JP") : "—";
}
...
const rows: { label: string; cell: (product: DisplayProduct) => ReactNode }[] = [
  ...
  { label: "過去の出品価格の指標", cell: (p) => productPriceIndex(p) ? <ProductPriceIndexSummary product={p} /> : "—" },
];

export function ProductComparison({ keys, api, onRemove, onClear }: {
  keys: string[];
  api: ApiClient;
  ...
  const [result, setResult] = useState<{ selection: string; columns: ComparisonColumn[] } | null>(null);
  useEffect(() => {
    const controller = new AbortController();
    setResult(null);
    void loadComparisonProducts(api, selection ? selection.split(",") : [], controller.signal, attempt > 0)
      .then((columns) => { if (!controller.signal.aborted) setResult({ selection, columns }); })
  ...
  return <section className="product-comparison" aria-labelledby="product-comparison-title">
    <div className="comparison-toolbar">
      ...
      <button type="button" onClick={onClear}>比較を解除</button>
    </div>
    <p>確認済みの製品を2〜4件選べます。価格・在庫は検索条件にかかわらず、各製品の掲載中の出品を表示します。</p>
    <ul className="comparison-selection">{keys.map((key) => <li key={key}>
      {columns?.find((column) => column.key === key)?.product?.model || key}
      <button type="button" aria-label={`${key}を比較から外す`} onClick={() => onRemove(key)}>外す</button>
    </li>)}</ul>
    {keys.length < 2 ? <p role="status">もう1件選ぶと比較できます。</p> : !columns ? <p role="status">比較情報を読み込んでいます…</p> : <>
      {columns.some((column) => !column.product) ? <p role="status">取得できない製品があります。<button type="button" onClick={() => setAttempt((value) => value + 1)}>比較情報を再読み込み</button></p> : null}
      <div className="comparison-table-wrap" tabIndex={0} role="region" aria-label="製品の比較表（横にスクロールできます）">
        <table className="comparison-table">
          <caption>価格は出品価格です。記載のない情報は「—」で表示します。</caption>
          <thead><tr><th scope="col">比較項目</th>{columns.map(({ key, product }) => <th scope="col" key={key}>
            <a href={productPermalinkPath(key) || "/"}>{product?.model || key}</a>
            {!product ? <p>取得できませんでした</p> : null}
          </th>)}</tr></thead>
          <tbody>{rows.map((row) => <tr key={row.label}><th scope="row">{row.label}</th>{columns.map(({ key, product }) => <td key={key}>{product ? row.cell(product) : "—"}</td>)}</tr>)}</tbody>
        </table>
      </div>
    </>}
  </section>;
