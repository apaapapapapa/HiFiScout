...
export function validTargetPrice(value: unknown): value is number | null {
  return value === null || (typeof value === "number" && Number.isSafeInteger(value) && value >= 0 && value <= 999_999_999_999);
}
...
export function parseWatchPreferences(raw: string | null): WatchPreference[] {
  ...
  try {
    ...
    for (const item of data) {
      ...
      if (typeof row.key !== "string" || !validProductKey(row.key) || !validTargetPrice(row.targetPriceYen) ||
          typeof row.note !== "string" || [...row.note].length > 1000 ||
          typeof row.updatedAt !== "string" || !Number.isFinite(Date.parse(row.updatedAt))) continue;
      entries.set(row.key, { key: row.key, targetPriceYen: row.targetPriceYen, note: row.note, updatedAt: row.updatedAt });
    }
...
export function updateWatchPreference(entries: readonly WatchPreference[], key: string, targetPriceYen: number | null, note: string, now = new Date().toISOString(), expectedUpdatedAt?: string | null): WatchPreference[] | null {
  if (!validProductKey(key) || !validTargetPrice(targetPriceYen) || [...note].length > 1000) return null;
  const current = entries.find((entry) => entry.key === key);
  if (expectedUpdatedAt !== undefined && (current?.updatedAt ?? null) !== expectedUpdatedAt) return null;
