...
export function isCatalogRelationProof(value: unknown): value is CatalogRelationProof {
  if (
    !record(value) ||
    (value.kind !== "source" && value.kind !== "manual") ||
  ...
  return (
    ...
    value.links.every(
      (link) =>
        related(link) &&
        record(link) &&
        ["predecessor", "successor", "variant"].includes(String(link.kind)),
    ) &&
    Array.isArray(value.families) &&
    value.families.length <= 40 &&
    value.families.every(
      (family) =>
        record(family) &&
        typeof family.name === "string" &&
        position(family.position) &&
        isCatalogRelationProof(family.proof) &&
        Array.isArray(family.members) &&
        family.members.length <= 40 &&
        family.members.every(
          (member) => related(member) && record(member) && position(member.position),
        ),
    )
  );
}

export const MODEL_RELATION_LABELS = {
  predecessor: "前モデル",
  successor: "後継モデル",
  variant: "別仕様モデル",
} as const;
