-- Re-converge Product Search memberships that became stale after the earlier 0045 catch-up.
-- Catalog verification/manual operations and crawl projection writes are independent bounded D1
-- operations, so a deployment can observe a verified Product Identity row whose search membership
-- still points at its former unresolved fallback. The normal projection repair remains the durable
-- runtime path; this idempotent migration repairs production rows that predate that convergence.
...
-- Move every active verified Catalog match onto its authoritative Catalog entity.
...
-- Refresh FTS-backed seller evidence for entities affected by membership changes.
UPDATE product_search_entities AS e
...
-- Abandoned fallback entities must not remain searchable after their offer moved.
