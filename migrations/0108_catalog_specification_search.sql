...
-- Port names are exact XLR/RCA labels ignoring ASCII case and surrounding spaces.
...
SELECT catalog_product_id, direction, UPPER(TRIM(json_extract(port_json, '$.connector'))) AS connector,
  CASE WHEN COUNT(*) = 1
...
WHERE UPPER(TRIM(json_extract(port_json, '$.connector'))) IN ('XLR','RCA')
...
WHEN OLD.specification_json IS NOT NEW.specification_json BEGIN
  ...
  WHERE catalog_product_id = NEW.catalog_product_id AND NOT EXISTS (
    SELECT 1 FROM (SELECT catalog_product_id, direction, UPPER(TRIM(json_extract(port_json, '$.connector'))) AS connector,
  CASE WHEN COUNT(*) = 1
    AND MAX(json_type(port_json, '$.count')) IN ('integer','real')
    AND MAX(json_extract(port_json, '$.count')) BETWEEN 1 AND 128
    AND MAX(json_extract(port_json, '$.count')) = CAST(MAX(json_extract(port_json, '$.count')) AS INTEGER)
...
GROUP BY catalog_product_id, direction, connector) desired
  ...
  INSERT INTO catalog_specification_ports(catalog_product_id,direction,connector,port_count)
  SELECT * FROM (SELECT catalog_product_id, direction, UPPER(TRIM(json_extract(port_json, '$.connector'))) AS connector,
  CASE WHEN COUNT(*) = 1
    AND MAX(json_type(port_json, '$.count')) IN ('integer','real')
    AND MAX(json_extract(port_json, '$.count')) BETWEEN 1 AND 128
    AND MAX(json_extract(port_json, '$.count')) = CAST(MAX(json_extract(port_json, '$.count')) AS INTEGER)
