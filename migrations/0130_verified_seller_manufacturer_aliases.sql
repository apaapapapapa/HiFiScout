-- These exact seller spellings were observed on the bounded 2026-09-14 product audit. The saved
-- HiFiDo/Fujiya listing is structured evidence for both the canonical manufacturer and spelling.
-- Seed the verified canonical rows as well as aliases so clean databases match production, where
-- these manufacturers were originally reviewed through the admin path.
WITH manufacturers(id, canonical_name, source, provenance_json) AS (VALUES
  ('hamilex', 'HAMILEX', 'https://www.hifido.co.jp/23-39541-12359-00.html?LNG=J',
    '{"reason":"exact_seller_identity_review","listingId":11838,"model":"BRK-345"}'),
  ('sansui', 'SANSUI', 'https://www.hifido.co.jp/26-50252-14321-00.html?LNG=J',
    '{"reason":"exact_seller_identity_review","listingId":11844,"model":"B-2105 MOS vintage"}'),
  ('diatone', 'DIATONE', 'https://www.hifido.co.jp/26-51021-21166-00.html?LNG=J',
    '{"reason":"exact_seller_identity_review","listingId":11856,"model":"DK-3"}'),
  ('transparent', 'TRANSPARENT', 'https://www.hifido.co.jp/26-51058-21476-00.html?LNG=J',
    '{"reason":"exact_seller_identity_review","listingId":11858,"model":"ML1/1.0m"}'),
  ('mit', 'MIT', 'https://www.hifido.co.jp/26-51058-21475-00.html?LNG=J',
    '{"reason":"exact_seller_identity_review","listingId":11859,"model":"AVt 3 ic/1.5m"}'),
  ('belden', 'BELDEN', 'https://www.hifido.co.jp/26-50663-18168-00.html?LNG=J',
    '{"reason":"exact_seller_identity_review","listingId":11861,"model":"EC-1192A/1.0m"}'),
  ('toshiba', 'TOSHIBA', 'https://www.hifido.co.jp/26-49978-21689-00.html?LNG=J',
    '{"reason":"exact_seller_identity_review","listingId":11863,"model":"12AU7"}'),
  ('rca', 'RCA', 'https://www.hifido.co.jp/26-26133-21683-00.html?LNG=J',
    '{"reason":"exact_seller_identity_review","listingId":11868,"model":"5751"}'),
  ('grado', 'GRADO', 'https://www.fujiya-avic.co.jp/shop/g/g240004022712/',
    '{"reason":"exact_seller_identity_review","listingId":11898,"model":"GS3000-Classic Series"}')
)
...
SELECT id, canonical_name, 'verified', source, provenance_json,
  strftime('%Y-%m-%dT%H:%M:%fZ','now'), strftime('%Y-%m-%dT%H:%M:%fZ','now')
FROM manufacturers
WHERE NOT EXISTS (
  SELECT 1 FROM knowledge_catalog_manufacturers existing WHERE existing.id=manufacturers.id
);

-- Keep aliases exact-only so short brands such as MIT and RCA never become prefix matches.
