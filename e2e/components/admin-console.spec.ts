...
test("model relations verify an explicitly selected product and retain optimistic versions", async ({ page, mount }) => {
  ...
  await page.route("**/api/admin/knowledge-catalog/products?**", (route) => {
    ...
    return route.fulfill({ json: { items: query === "next-model" ? [{ ...catalogProduct, id: 12, canonicalName: "後継モデル" }] : [catalogProduct], nextAfterId: null } });
  });
  await page.route("**/api/admin/knowledge-catalog/products/*/model-facts", async (route) => {
    if (route.request().method() === "POST") {
      ...
      received.push(body);
      facts = [{ id: "00000000-0000-4000-8000-000000000001", version: received.length, productId: 11, productName: catalogProduct.canonicalName, relatedProductName: "後継モデル", input: body.fact, reviewState: "verified", sourceUrl: "", verifiedAt: "2026-09-07T00:00:00Z", reviewDueAt: "2027-03-06T00:00:00Z" }];
    }
    return route.fulfill({ json: { product: { id: 11, name: catalogProduct.canonicalName, manufacturerId: "luxman" }, facts, sources: [], audits: [] } });
  });
  await mount("frontend/admin-console/Default");
  await page.getByRole("button", { name: `${catalogProduct.canonicalName} の機種の関係`, exact: true }).click();
  ...
  await editor.getByLabel("確認内容・資料の説明").fill("公式資料で後継機種として紹介されていることを確認しました。");
