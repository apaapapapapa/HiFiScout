...
test('invalid catalog query parameters are sanitized before the product API request', async ({ page }) => {
  ...
  await page.goto(`/?q=${longQuery}&shop=${longShop}&manufacturer=${longQuery}&category=${longQuery}&minPrice=abc&maxPrice=12x&sort=invalid&inStock=maybe&newOnly=yes&priceDropped=1&favoritesOnly=true&cursor=bogus`);
...
test('new and price-drop checkboxes update both the URL and product API query', async ({ page }) => {
  await routeMeta(page);
  await page.route('**/api/products?**', route => route.fulfill({
    contentType: 'application/json',
...
test('healthy metadata renders the simple normal sync summary', async ({ page }) => {
  ...
  await routeMeta(page, catalogMeta({
    status: 'healthy',
    shops: [
      {
        key: 'shop-a',
