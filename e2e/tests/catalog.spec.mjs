...
test('sync status summarizes delayed shops and exposes per-shop details', async ({ page }) => {
  await routeMeta(page, mockMeta({
    status: 'warning',
    shops: [
      {
        key: 'shop-a', name: 'Shop A', intervalMinutes: 60,
        sync: { last_success_at: new Date(Date.now() - 10 * 60_000).toISOString() },
        health: { status: 'healthy', lastSuccessAt: new Date(Date.now() - 10 * 60_000).toISOString(), ageMinutes: 10 }
      },
      {
        key: 'shop-b', name: 'Shop B', intervalMinutes: 60,
        sync: { last_success_at: new Date(Date.now() - 150 * 60_000).toISOString() },
        health: { status: 'warning', lastSuccessAt: new Date(Date.now() - 150 * 60_000).toISOString(), ageMinutes: 150 }
      }
    ]
  }));
  await page.route('**/api/products?**', route => route.fulfill({
    contentType: 'application/json', body: JSON.stringify({ items: [product()], hasMore: false, nextCursor: null })
  }));
...
test('URL restores search state and recent/price-drop filters reach the API', async ({ page }) => {
  ...
  await page.goto('/?manufacturer=LUXMAN&sort=priceAsc&inStock=false&newOnly=true&priceDropped=true');
