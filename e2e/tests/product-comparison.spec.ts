...
test("a shared comparison survives the real public bootstrap and metadata initialization", async ({ page, catalogPage }) => {
  const details: string[] = [];
  await page.route("**/api/**", async (route) => {
    const path = new URL(route.request().url()).pathname;
    if (path === "/api/meta") return route.fulfill({ json: { status: "healthy", shops: [], manufacturers: [], categories: [], categoryFacets: [] } });
    if (path === "/api/product-search") return route.fulfill({ json: { items: [], hasMore: false, nextCursor: null } });
    const key = path.split("/").at(-1)!;
    details.push(key);
    return route.fulfill({ json: { product: product({ key, catalog_product_id: Number(key.slice(2)), model: `Model ${key}` }), offers: [] } });
  });
