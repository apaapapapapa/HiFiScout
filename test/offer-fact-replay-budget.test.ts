...
test("replay reads and writes a bounded prefix as the retained inventory grows", async () => {
  ...
  try {
    ...
    for (const size of [100, 10000]) {
      await db.prepare(`WITH RECURSIVE n(i) AS (
    ...
    assert.ok(costs.every((cost) => cost.reads < 1000 && cost.writes < 200 && cost.statements <= 60), JSON.stringify(costs));
