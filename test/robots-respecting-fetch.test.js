...
test('same-origin redirects are followed under the same robots policy', async () => {
  ...
  const baseFetch = async url => {
    requested.push(String(url));
    if (String(url) === 'https://example.com/robots.txt') return new Response('User-agent: *\nAllow: /', { status: 200 });
    if (String(url) === 'https://example.com/start') return new Response('', { status: 302, headers: { location: '/final' } });
...
test('cross-origin redirects are blocked before the destination is fetched', async () => {
  ...
  const baseFetch = async url => {
    requested.push(String(url));
    if (String(url) === 'https://example.com/robots.txt') return new Response('User-agent: *\nAllow: /', { status: 200 });
    if (String(url) === 'https://example.com/start') return new Response('', { status: 302, headers: { location: 'https://outside.example/final' } });
    return new Response('should not fetch', { status: 200 });
  };
  const fetchImpl = createRobotsRespectingFetch(baseFetch, {
    userAgent: 'HiFiScoutBot/0.1',
