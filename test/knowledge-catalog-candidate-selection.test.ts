...
test("normal candidate scheduling prefers never-verified rows before retries", async () => {
  ...
  assert.ok(
    sql.indexOf("CASE WHEN last_verification_at IS NULL THEN 0 ELSE 1 END") <
      sql.indexOf("priority_score DESC"),
  );
...
test("verifier rollout can prefer the most recently failed candidates for a matched cohort", async () => {
  ...
  assert.ok(
    sql.indexOf("COALESCE(last_verification_at, '') DESC") < sql.indexOf("priority_score DESC"),
  );
});
