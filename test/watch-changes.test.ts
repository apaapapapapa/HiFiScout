...
import { MAX_WATCH_REFRESH, captureWatchObservation, compareWatchObservations, mergeWatchObservations, parseWatchObservations, refreshWatchedProducts } from "../frontend/watch-changes.js";
...
const before: WatchObservation = { key: "c-1", checkedAt: AT, complete: true, offers: [
...
test("revisit changes separate asking prices, sold-out signals and unconfirmed disappearance", () => {
  const after: WatchObservation = { ...before, offers: [
    { id: 1, shopKey: "one", priceYen: 90, stock: "sold_out" },
    { id: 3, shopKey: "three", priceYen: 150, stock: "in_stock" },
  ] };
  assert.deepEqual(compareWatchObservations(before, after).map((change) => change.kind), ["price", "sold_out", "new", "missing"]);
  assert.deepEqual(compareWatchObservations(undefined, after), []);
  assert.equal(compareWatchObservations(before, { ...after, complete: false }).some((change) => change.kind === "missing"), false);
  assert.equal(compareWatchObservations({ ...before, complete: false }, after).some((change) => change.kind === "new"), false);
  assert.equal(compareWatchObservations(before, { ...after, offers: [{ ...after.offers[0], priceYen: null }] }).some((change) => change.kind === "price"), false);
...
test("capped details stay partial and failed or old updates preserve prior observations", () => {
  const detail = { product: product({ offer_count: 200 }), offers: Array.from({ length: 200 }, (_, index) => offer({ listing_product_id: index + 1 })) } as unknown as ProductDetailResponse;
  ...
  assert.deepEqual(parseWatchObservations(JSON.stringify([{ ...before, offers: [before.offers[0], before.offers[0]] }])), []);
  assert.deepEqual(mergeWatchObservations([before], []), [before]);
  assert.deepEqual(mergeWatchObservations([before], [{ ...before, checkedAt: "2025-01-01", offers: [] }]), [before]);
});
...
test("watch refresh caps its scope and concurrency, preserves failed slots and validates identity", async () => {
  ...
  const api = createApiClient(async (input) => {
    ...
    return Response.json({ product: product({ key, catalog_product_id: key === "c-4" ? 99 : Number(key.slice(2)) }), offers: [offer()] });
  });
  ...
  await assert.rejects(refreshWatchedProducts(api, [...keys, "c-11"], new AbortController().signal), /invalid_watch_scope/);
  const controller = new AbortController(); controller.abort();
  await assert.rejects(refreshWatchedProducts(api, keys, controller.signal), { name: "AbortError" });
  assert.equal(calls, 10);
});
