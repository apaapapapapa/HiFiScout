...
test("taxonomy v3 has the specified product-type roots in stable order", () => {
  ...
  assert.deepEqual(
    ...
    ["PER", "SPK", "AMP", "SRC", "ANA", "PRC", "SIG", "CAB", "PWR", "ACC", "SYS", "REC", "unclassified"],
  );
  assert.ok(roots().slice(0, -1).every((category) => category.filterable && !category.classifiable));
  assert.deepEqual(
...
test("taxonomy v3 keeps product types in categories and properties out of them", () => {
  assert.deepEqual(children("PER").map((category) => category.id), [
    "PER.HEADPHONE",
    "PER.EARPHONE",
  ]);
  assert.deepEqual(children("SPK").map((category) => category.id), [
    "SPK.LOUDSPEAKER",
    "SPK.SUBWOOFER",
    "SPK.SOUNDBAR",
  ]);
  assert.deepEqual(children("CAB").map((category) => category.id), [
    "CAB.ANALOG",
    "CAB.DIGITAL",
    "CAB.SPEAKER",
    "CAB.PERSONAL",
...
test("all 59 legacy ids have explicit migration rules and none is canonical", () => {
  ...
  for (const rule of LEGACY_CATEGORY_MIGRATION_RULES) {
    ...
    assert.ok(rule.categoryIds.every((categoryId) => canonicalIds.has(categoryId)), rule.legacyId);
  }
  ...
  assert.equal(categoryIdForClassification("cable_xlr", "AES/EBU XLR digital cable"), "CAB.DIGITAL");
});
...
test("headphone and speaker properties become independent facets", () => {
  const headphone = classify(
    "Bluetooth wireless headphones with detachable wired USB cable",
  );
  assert.equal(headphone.primaryCategoryId, "PER.HEADPHONE");
  assert.ok(headphone.facetFacts.some((fact) => fact.facetId === "connectivity" && fact.value === "wireless"));
  assert.ok(headphone.facetFacts.some((fact) => fact.facetId === "connectivity" && fact.value === "wired"));
  assert.ok(headphone.facetFacts.some((fact) => fact.facetId === "protocol" && fact.value === "bluetooth"));
  ...
  assert.ok(speaker.facetFacts.some((fact) => fact.facetId === "form_factor" && fact.value === "bookshelf"));
  assert.ok(speaker.facetFacts.some((fact) => fact.facetId === "amplification_mode" && fact.value === "active"));
  assert.ok(speaker.facetFacts.some((fact) => fact.facetId === "use_case" && fact.value === "studio"));
});
...
test("XLR and transports use product type plus signal/connector facets", () => {
  ...
  assert.ok(analog.facetFacts.some((fact) => fact.facetId === "connector_a" && fact.value === "xlr"));
  assert.ok(analog.facetFacts.some((fact) => fact.facetId === "signal_type" && fact.value === "analog"));

  const digital = classify("AES/EBU XLR digital cable 1m");
  assert.equal(digital.primaryCategoryId, "CAB.DIGITAL");
  assert.ok(digital.facetFacts.some((fact) => fact.facetId === "signal_type" && fact.value === "digital"));
...
test("multifunction classification is strict and conventional products win", () => {
  ...
  assert.equal(classify("Integrated amplifier with DAC and streamer").primaryCategoryId, "AMP.INTEGRATED");
});
...
test("unknown and legacy other never become a normal catch-all category", () => {
  ...
  assert.equal(CATEGORIES.some((category) => category.id === ("other" as never)), false);
});
