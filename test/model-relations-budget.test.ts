...
test("model fact reads and guarded writes do not scan unrelated relationships", async () => {
  ...
  try {
    await db
      .prepare(`INSERT INTO knowledge_catalog_products(id,manufacturer_id,canonical_model,normalized_model,created_at,updated_at)
      VALUES(700001,'budget','A','A',?,?),(700002,'budget','B','B',?,?)`)
      .bind(AT, AT, AT, AT)
      .run();
    const input: ModelFactInput = {
      kind: "successor",
      ...
      state: "verified",
      sourceId: null,
      manualNote: "Manually verified official model succession.",
    ...
    for (const size of [100, 10000]) {
      await db
        ...
        SELECT 800000+i,'fixture','MODEL'||i,'MODEL'||i,?,? FROM n`)
      ...
      await db
        ...
        SELECT 'fixture-'||i,800000+i,800001+i,'variant','candidate','manual','{}',?,'fixture' FROM n`)
      ...
      await saveModelFact(
        ...
        700001,
        input,
        { id: first.id, expectedVersion: 1, actor: "budget-test" },
        AT,
      );
      costs.push({
        size,
    ...
    assert.ok(
      measured.rowsRead() < 200 && measured.rowsWritten() < 50,
      JSON.stringify({ reads: measured.rowsRead(), writes: measured.rowsWritten() }),
    );
    console.log(
      JSON.stringify({
        event: "model_relation_budget",
        costs,
        changedReads: measured.rowsRead(),
        changedWrites: measured.rowsWritten(),
      }),
    );
  } finally {
    await dispose();
