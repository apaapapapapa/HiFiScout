...
test('Fujiya unresolved model is classified from product-specific detail evidence', async () => {
  const product = normalizeCatalogProduct({
    sourceId: 'd10x', manufacturer: 'LUXMAN', model: 'D-10X', title: 'LUXMAN D-10X', rawCategory: 'DAP',
  ...
  const result = await enrichProductCategories({
    db: {}, adapter: fujiyaAvicAdapter, products: [product], existingRows: [],
    transport: { async fetchHtmlPage() { return '<html><head><meta name="description" content="LUXMAN D-10X フラグシップSACD/CDプレーヤーの中古商品です。"></head><body><h1>D-10X</h1></body></html>'; } },
    fetchOptions: {}, now: new Date('2026-08-11T10:00:00Z')
  });
  ...
  assert.equal(classified.metadata.categoryClassification.detailCheckedAt, '2026-08-11T10:00:00.000Z');
});
...
test('Fujiya detail extraction stops at product-specific metadata and ignores related-product mentions later in the page', () => {
  ...
  `, { model: 'MODEL10/FB' });
  assert.deepEqual(evidence.map(item => item.categoryIds), [['integrated_amp']]);
});
...
test('cached detail classification is reused for the same product identity without another request', async () => {
  ...
  const existingRows = [{
    source_id: 'd10x', manufacturer_id: product.manufacturerId, model: 'D-10X', title: 'LUXMAN D-10X',
    category: 'CD/SACDプレーヤー', primary_category_id: 'cd_sacd_player', category_ids: '["cd_sacd_player"]',
    classification_status: 'classified', search_aliases: 'CD/SACDプレーヤー cd player sacd player',
    metadata_json: JSON.stringify({ categoryClassification: { version: 3, state: 'classified', detailCheckedAt: '2026-08-10T10:00:00.000Z' } })
  }];
  const result = await enrichProductCategories({
    db: {}, adapter: fujiyaAvicAdapter, products: [product], existingRows,
    transport: { async fetchHtmlPage() { throw new Error('detail fetch must not run'); } },
    fetchOptions: {}, now: new Date('2026-08-11T10:00:00Z')
  });
...
test('successful unresolved detail checks are cached briefly, but fetch failures remain retryable', async () => {
  const product = normalizeCatalogProduct({
    sourceId: 'unknown', manufacturer: 'Example', model: 'ABC-123', title: 'Example ABC-123', rawCategory: 'DAP',
    sourceUrl: 'https://www.fujiya-avic.co.jp/shop/g/gunknown/'
  }, fujiyaAvicAdapter);
  const checked = await enrichProductCategories({
    db: {}, adapter: fujiyaAvicAdapter, products: [product], existingRows: [],
    transport: { async fetchHtmlPage() { return '<html><body><h1>ABC-123</h1><p>中古商品です。</p></body></html>'; } },
    fetchOptions: {}, now: new Date('2026-08-11T10:00:00Z')
  });
  assert.equal(checked.products[0].classificationStatus, 'unclassified');
  assert.equal(checked.products[0].metadata.categoryClassification.detailCheckedAt, '2026-08-11T10:00:00.000Z');
  const failed = await enrichProductCategories({
    db: {}, adapter: fujiyaAvicAdapter, products: [product], existingRows: [],
    transport: { async fetchHtmlPage() { throw new Error('temporary failure'); } },
    fetchOptions: {}, now: new Date('2026-08-11T11:00:00Z')
  });
