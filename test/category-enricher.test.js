  ...
  const result = await enrichProductCategories({
    ...
    transport: {
      async fetchHtmlPage() {
        return `
          <html><head>
            <meta name="description" content="LUXMAN D-10X フラグシップSACD/CDプレーヤーの中古商品です。">
          </head><body><h1>D-10X</h1></body></html>`;
      }
    },
    fetchOptions: {},
...
test('Fujiya detail extraction stops at product-specific metadata and ignores related-product mentions later in the page', () => {
  const evidence = extractFujiyaDetailCategoryEvidence(`
    <html>
      <head><meta name="description" content="MODEL10/FB Marantzのプリメインアンプです。"></head>
      <body>
        <h1>MODEL10/FB</h1>
        <p>MODEL10はプリメインアンプです。</p>
        <section>組み合わせにはSACD 10 SACDプレーヤーがおすすめです。</section>
      </body>
    </html>
  `, { model: 'MODEL10/FB' });
...
test('cached detail classification is reused for the same product identity without another request', async () => {
  ...
  const result = await enrichProductCategories({
    ...
    existingRows,
    transport: {
      async fetchHtmlPage() { throw new Error('detail fetch must not run'); }
    },
    fetchOptions: {},
    now: new Date('2026-08-11T10:00:00Z')
  });
