...
function browserHarness() {
  ...
  const page = {
    ...
    async content() { return '<html>first</html>'; },
  ...
  return {
    calls,
    launchBrowser: async () => { calls.launch += 1; return browser; }
  };
}
...
test('Browser Run transport launches once and reuses the page for same-origin fetches', async () => {
  ...
  assert.equal(await fetcher.fetchHtmlPage('https://example.com/page-1', options), '<html>first</html>');
  assert.equal(await fetcher.fetchHtmlPage('https://example.com/page-2', options), '<html>https://example.com/page-2</html>');
  await fetcher.close();
    ...
    launchBrowser: async () => ({
      async newPage() {
        return {
          ...
          url() { return 'https://example.com/page-1'; }
        };
      },
      async close() {}
    })
