...
test('unclassified products clear canonical filter categories instead of falling back to other', async () => {
  const existing = {
    id: 1,
    source_id: 'p1',
  ...
  assert.equal(categoryStatements.some(statement => /INSERT OR IGNORE INTO product_categories/.test(statement.sql)), false);

  const update = db.statements.find(statement => /UPDATE products SET/.test(statement.sql));
  assert.ok(update);
  assert.ok(update.binds.includes('[]'));
