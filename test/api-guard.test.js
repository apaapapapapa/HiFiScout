...
test('admin endpoints are not subject to the public limiter', async () => {
  let called = false;
  const env = { API_RATE_LIMITER: { async limit() { called = true; return { success: false }; } } };
  const request = new Request('https://example.test/api/admin/crawl?shop=hifido', { method: 'POST' });
  const result = await checkPublicApiRateLimit(request, env);
