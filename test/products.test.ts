...
test("product query validation rejects oversized and malformed inputs", () => {
  assert.equal(
    validateProductQuery(new URL(`https://example.test/api/products?q=${"x".repeat(101)}`)),
    "q_too_long",
  );
  assert.equal(
    validateProductQuery(new URL("https://example.test/api/products?limit=-1")),
    "limit_invalid",
  );
  assert.equal(
    validateProductQuery(new URL("https://example.test/api/products?sort=random")),
    "sort_invalid",
  );
  assert.equal(
    validateProductQuery(new URL("https://example.test/api/products?q=TAD&limit=50&sort=newest")),
    null,
  );
  assert.equal(
    validateProductQuery(new URL("https://example.test/api/products?sort=updated")),
    null,
  );
});
...
test("newest listing uses activity timestamp with keyset pagination", async () => {
  ...
  const first = await listProducts(
    db,
    new URL("https://example.test/api/products?sort=newest&limit=2"),
  );
  ...
  await listProducts(
    nextDb,
    new URL(
      `https://example.test/api/products?sort=newest&limit=2&cursor=${encodeURIComponent(first.nextCursor)}`,
    ),
  );
