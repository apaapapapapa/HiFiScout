...
import {
  specificationErrors,
  specificationFromFilterId,
} from "../frontend/specification-filters.js";
import {
  savedSearchFilters,
  savedSearchQuery,
  parseSavedSearches,
...
test("detailed specifications round-trip normalized units through URL, API, saved search and Atom", () => {
  const raw = {
    ...savedSearchFilters("category=AMP.PRE"),
  ...
  assert.deepEqual(filters.specificationFilters, {
    maxWidthMm: "450",
    maxWeightKg: "12.5",
    minXlrOutputs: "3",
  });
  ...
  for (const params of [
    productSearchParams(filters),
    new URL(savedSearchFeedPath(filters), "https://example.test").searchParams,
  ]) {
  ...
  assert.ok(
    chips.some((entry) => entry.id === "spec:maxWeightKg" && entry.label.includes("12.5kg")),
  );
  ...
  assert.deepEqual(
    filterRelaxations(filters).find((entry) => entry.id === "specifications")?.filters
      .specificationFilters,
    {},
  );
  assert.ok(
    !activeFilterEntries({ ...filters, favoritesOnly: true }, { shop: "", category: "" }).some(
      (entry) => entry.id.startsWith("spec:"),
    ),
  );
});
...
test("invalid drafts cannot silently broaden a saved or applied specification search", () => {
  ...
  assert.deepEqual(
    parseSavedSearches(
      JSON.stringify([
        { id: "one", name: "Bad", query: "minXlrOutputs=2.5", updatedAt: "2026-09-07T00:00:00Z" },
      ]),
    ),
    [],
  );
