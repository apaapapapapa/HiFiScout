...
test("server validates rows independently of the browser and bounds each request", () => {
  ...
  assert.equal(
    parseAdminCsvPreview({ changes: [{ ...change(), original: { ...original, id: "90001" } }] }),
    null,
  );
  assert.equal(
...
test("listing import is no-write on unchanged input and persists overrides without overwriting seller facts", async () => {
  ...
  try {
    ...
    const row = sqlite
      .prepare("SELECT model,raw_model,title,price_yen FROM products WHERE id=90001")
      .get();
    assert.deepEqual(
      { ...row },
      { model: "C11", raw_model: "C10", title: "LUXMAN C10", price_yen: 100000 },
    );
...
test("conflicts and invalid manufacturer/category IDs cannot mutate the database", async () => {
  ...
  try {
    ...
    for (const values of [
      ...
      assert.equal(
        (await previewAdminCsvChange(db, { line: 2, original, values })).status,
        "invalid",
      );
    }
  } finally {
...
test("race inside the write transaction rolls back overrides and the import receipt", async () => {
  ...
  try {
    ...
    assert.equal(
      sqlite.prepare("SELECT model FROM products WHERE id=90001").get()?.model,
      "Race winner",
    );
    assert.equal(sqlite.prepare("SELECT COUNT(*) n FROM admin_csv_import_changes").get()?.n, 0);
    assert.equal(
      sqlite
        .prepare("SELECT COUNT(*) n FROM product_admin_overrides WHERE listing_product_id=90001")
        .get()?.n,
      0,
    );
  } finally {
...
test("catalog import edits identity in place, rejects duplicates and retires old identity evidence", async () => {
  ...
  try {
    ...
    const item: AdminCsvChange = {
      line: 2,
      original,
      values: { ...original.values, canonical_model: "CSV-NEW", canonical_name: "Correct name" },
    };
    const result = await apply(db, item);
    assert.equal(result.status, "applied", result.message);
    assert.equal(
      sqlite.prepare("SELECT canonical_model FROM knowledge_catalog_products WHERE id=90001").get()
        ?.canonical_model,
      "CSV-NEW",
    );
    assert.equal(
      sqlite
        .prepare("SELECT COUNT(*) n FROM knowledge_catalog_aliases WHERE product_id=90001")
        .get()?.n,
      0,
    );
    assert.equal(
      sqlite
        .prepare(
          "SELECT COUNT(*) n FROM knowledge_catalog_product_categories WHERE product_id=90001",
        )
        .get()?.n,
      4,
    ...
    assert.equal(
      sqlite
        .prepare(
          "SELECT status FROM knowledge_catalog_sources WHERE source_url='https://example.test/old'",
        )
        .get()?.status,
      "error",
    );
    const again: AdminCsvChange = {
      line: 2,
      original: { ...original, values: item.values },
...
async function relatedListings(
  ...
  `);
  const rows = await db
    .prepare("SELECT id,shop_key,source_id FROM products WHERE id>=90001")
...
test("catalog category correction updates active and inactive matches but preserves explicit listing overrides", async () => {
  ...
  try {
    ...
    assert.equal(
      sqlite
        .prepare(
          "SELECT COUNT(*) n FROM product_identity_resolutions WHERE catalog_product_id=90001",
        )
        .get()?.n,
      3,
    );
    ...
    const categories = sqlite
      ...
      .all();
    assert.deepEqual(
      categories.map((row) => row.primary_category_id),
      ["PRC.DAC", "PRC.DAC", "AMP.PRE"],
    );
    assert.equal(
      sqlite
        .prepare(
          "SELECT catalog_product_id FROM product_identity_resolutions WHERE listing_product_id=90001",
        )
    ...
    assert.equal(
      sqlite
        .prepare(
          "SELECT category_id FROM product_categories WHERE product_id=90003 AND is_direct=1",
        )
        .get()?.category_id,
      "AMP.PRE",
    );
    assert.equal(
      sqlite.prepare("SELECT is_active FROM products WHERE id=90002").get()?.is_active,
      0,
    );
  } finally {
    sqlite.close();
...
test("catalog identity correction detaches old listings without rewriting their model to the new identity", async () => {
  ...
  try {
    ...
    const result = await apply(db, {
      line: 2,
      original,
      values: { ...original.values, canonical_model: "C11" },
    });
    assert.equal(result.status, "applied", result.message);
    assert.equal(
      sqlite
