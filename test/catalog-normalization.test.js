...
test('title inference suppresses component words inside accessory and amplifier names', () => {
  ...
  assert.deepEqual(normalizeCategory({ title: 'Network Transport' }).categoryIds, ['network_player']);
});
...
test('manufacturer aliases collapse Japanese and English spellings', () => {
  assert.deepEqual(normalizeManufacturer('LUXMAN'), { id: 'luxman', displayName: 'LUXMAN', matchedAlias: true });
  assert.deepEqual(normalizeManufacturer('ラックスマン'), { id: 'luxman', displayName: 'LUXMAN', matchedAlias: true });
...
test('raw seller values are preserved while UI values are canonicalized', () => {
  const product = normalizeCatalogProduct({
    sourceId: '1', rawManufacturer: 'LUXMAN ラックスマン', manufacturer: 'LUXMAN', model: 'C-10X',
    title: 'LUXMAN C-10X', rawCategory: 'コントロールアンプ', category: 'コントロールアンプ'
...
test('unknown products remain visible but are explicitly unclassified', () => {
  const product = normalizeCatalogProduct({ title: 'Mystery Device XYZ', rawCategory: '特殊機器' });
...
test('default shops keep exact seller category precedence for backward compatibility', () => {
  const product = normalizeCatalogProduct({ title: 'Example SACD Player', rawCategory: 'DAP' });
...
test('corroborative seller categories do not override explicit title evidence', () => {
  const product = normalizeCatalogProduct({ title: 'Example SACD 10 SACD Player', rawCategory: 'DAP' }, {
...
test('generic accessory seller category does not override specific title evidence', () => {
  const product = normalizeCatalogProduct({ title: 'Premium Headphone Cable 2m', rawCategory: 'アクセサリー' });
...
test('a corroborative seller category alone remains unclassified instead of becoming a false positive', () => {
  const product = normalizeCatalogProduct({ title: 'Portable Audio Model X', rawCategory: 'DAP' }, {
    categoryPolicy: { sellerCategory: { categories: { dap: 'corroborative' } } }
  });
  assert.equal(product.primaryCategoryId, 'other');
  assert.deepEqual(product.categoryIds, []);
...
test('broad seller text inferred from a product family is not authoritative by itself', () => {
  const product = normalizeCatalogProduct({ title: 'Model X', rawCategory: 'アンプ・スピーカー・プレーヤー' });
  assert.equal(product.classificationStatus, 'unclassified');
  assert.deepEqual(product.categoryIds, []);
});
...
test('Fujiya uses the generic evidence policy for broad DAP merchandising buckets', () => {
  const sacd = normalizeCatalogProduct({ title: 'MARANTZ SACD 10 SACD Player', rawCategory: 'DAP' }, fujiyaAvicAdapter);
  const cable = normalizeCatalogProduct({ title: 'Premium Headphone Cable 2m', rawCategory: 'DAP' }, fujiyaAvicAdapter);
  const unresolved = normalizeCatalogProduct({ title: 'Portable Audio Model X', rawCategory: 'DAP' }, fujiyaAvicAdapter);
