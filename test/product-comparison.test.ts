...
import {
  canonicalComparisonKeys,
  comparisonKeysFromSearch,
  comparisonPath,
  loadComparisonProducts,
} from "../frontend/product-comparison.js";
...
test("comparison URLs canonicalize catalog identities and survive catalog sanitization", () => {
  ...
  for (const keys of [
    ...
    ["c-0"],
    ["c-1", "c-2", "c-3", "c-4", "c-5"],
    ["c-1", "c-9999999999999999"],
  ])
    assert.deepEqual(canonicalComparisonKeys(keys), []);
...
test("comparison requests are bounded and validate each returned identity independently", async () => {
  ...
  assert.deepEqual(
    columns.map((column) => [column.key, column.product?.key ?? null]),
    [
      ["c-1", "c-1"],
      ["c-2", null],
      ["c-3", null],
...
test("cancelled comparison requests do not become unavailable-product results", async () => {
  ...
  await assert.rejects(loadComparisonProducts(api, ["c-1", "c-2"], controller.signal), {
    name: "AbortError",
  });
});
