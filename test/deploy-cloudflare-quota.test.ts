...
test("production smoke is deferred only after D1 code 7500 is confirmed", () => {
  ...
  assert.match(
    workflow,
    /Production Atom\/catalog smoke check deferred because D1 code 7500 was confirmed/,
  );
  assert.match(
    workflow,
    /Production runtime health check deferred because D1 code 7500 was confirmed/,
  );
});
