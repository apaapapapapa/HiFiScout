  ...
  try {
    ...
    for (const size of [100, 10000]) {
      await db
        ...
        SELECT 700000+i,'fixture','Model '||i,'MODEL'||i,?,? FROM n`)
        .bind(previous + 1, size, AT, AT)
        .run();
      await db
        .prepare(`INSERT INTO product_search_entities(id,entity_key,entity_kind,catalog_product_id,model,offer_count,in_stock_offer_count,lowest_price_yen,lowest_in_stock_price_yen)
        SELECT id,'c-'||id,'catalog',id,canonical_model,1,1,id,id FROM knowledge_catalog_products WHERE id>?`)
        .bind(700000 + previous)
        .run();
      await db
        ...
        FROM knowledge_catalog_products WHERE id>?`)
        .bind(JSON.stringify(specification), JSON.stringify(specification), AT, 700000 + previous)
        .run();
      const measured = accountReads(db);
      const result = await searchProducts(
        measured.db,
        productQuery("?maxWidthMm=450&minXlrOutputs=3&sort=priceAsc&limit=5&includeTotal=true"),
      );
    ...
    assert.ok(
      costs.every((cost) => cost.reads < 1000 && cost.statements <= 4),
    ...
    console.log(
      JSON.stringify({
        event: "specification_search_budget",
        costs,
        noopReads: noop.rowsRead(),
        noopWrites: noop.rowsWritten(),
      }),
    );
  } finally {
    await dispose();
