...
import { MAX_WATCH_PREFERENCES, parseWatchPreferences, updateWatchPreference } from "../frontend/watch-preferences.js";
...
test("watch planning preserves private notes, explicit zero prices and unchanged timestamps", () => {
  ...
  const noop = updateWatchPreference(first, "c-1", 125000, first[0].note, "2026-09-08T00:00:00Z", AT);
...
test("watch preferences enforce bounded valid keys, prices, notes and storage capacity", () => {
  for (const price of [-1, NaN, Infinity, 1.5, 1_000_000_000_000]) assert.equal(updateWatchPreference([], "c-1", price, "", AT), null);
  ...
  assert.deepEqual(parseWatchPreferences(JSON.stringify([{ key: "c-1", targetPriceYen: "100", note: "", updatedAt: AT }])), []);
  const full = Array.from({ length: MAX_WATCH_PREFERENCES }, (_, index) => ({ key: `c-${index + 1}`, targetPriceYen: 100, note: "", updatedAt: AT }));
  assert.equal(updateWatchPreference(full, "c-999", 100, "", AT), null);
  assert.equal(updateWatchPreference(full, "c-1", 200, "updated", AT)?.length, MAX_WATCH_PREFERENCES);
});
