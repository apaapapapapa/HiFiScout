...
test('Ippinkan title splitting removes listing condition markers', () => {
  ...
  assert.deepEqual(splitManufacturerModel('TAD - E2-WN/ウォルナット『展示機』（ペア）《JP-u》', 'ippinkan'), {
    manufacturer: 'TAD',
...
test('Fujiya title splitting keeps multi-word manufacturer names', () => {
  assert.deepEqual(splitManufacturerModel('Bowers & Wilkins バウワースアンドウィルキンス FS-700S3/B', 'fujiya-avic'), {
    manufacturer: 'Bowers & Wilkins',
    model: 'FS-700S3/B'
  });
  assert.deepEqual(splitManufacturerModel('iBasso Audio アイバッソオーディオ DC07PRO Black', 'fujiya-avic'), {
    manufacturer: 'iBasso Audio',
    model: 'DC07PRO Black'
  });
  assert.deepEqual(splitManufacturerModel('水月雨（MoonDrop） スイゲツアメ DISCDREAM 2', 'fujiya-avic'), {
