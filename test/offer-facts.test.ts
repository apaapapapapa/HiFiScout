...
const states = (title: string, condition = "") => Object.fromEntries(
...
test("offer facts preserve explicit positive, negative and unknown evidence", () => {
  assert.deepEqual(states("展示品", "元箱なし。リモコン付属。説明書あり。当店保証付き。動作確認済み"), {
    display: "present", operation_confirmed: "present", original_box: "absent",
    remote_control: "present", manual: "present", shop_warranty: "present",
  });
  for (const text of ["新品同様", "リモコン対応", "付属品不明", "元箱別売", "Aランク", ""]) {
    assert.deepEqual(states(text), {}, text);
  }
  assert.deepEqual(states("元箱付属なし。リモコンは付属しません"), {
    original_box: "absent", remote_control: "absent",
  });
...
test("facts carry field provenance and a bounded rule identifier without seller prose", () => {
  ...
  assert.deepEqual(fact, { factId: "original_box", state: "present", source: "seller",
    sourceField: "condition_text", ruleId: "offer.v1.original_box", confidence: 1, observedAt: AT });
});

function listing(conditionText: string) {
  return normalizeCatalogProduct({ sourceId: "one", manufacturer: "YAMAHA", model: "CD-S3000",
    title: "YAMAHA CD-S3000", conditionText, priceYen: 300000, stockStatus: "in_stock",
...
test("facts persist atomically with the listing and preserve manual decisions on refresh", async () => {
  ...
  try {
    ...
    const rows = () => sqlite.prepare("SELECT fact_id, state, source, observed_at FROM product_offer_facts ORDER BY fact_id, source").all();
    assert.equal(rows().length, 2);
    const before = rows();
    await db.batch(sellerOfferFactWrites(db, "facts", "one", "YAMAHA CD-S3000", "元箱あり。リモコン付属", LATER));
    assert.deepEqual(rows(), before, "equal facts retain their original observation time");
    sqlite.exec(`INSERT INTO product_offer_facts SELECT id, 'original_box', 'manual', 'absent', 'manual', 'admin', 1, '${AT}' FROM products`);
    await upsertProducts(db, "facts", [listing("元箱なし")], LATER);
    assert.deepEqual(rows().map((row) => [row.fact_id, row.state, row.source]), [
      ["original_box", "absent", "manual"], ["original_box", "absent", "seller"],
    ]);
...
test("a failed listing transaction cannot leave offer facts behind", async () => {
  const { db, sqlite } = migratedSqlite();
  try {
    sqlite.exec("CREATE TRIGGER fail_history BEFORE INSERT ON price_history BEGIN SELECT RAISE(ABORT,'injected'); END;");
