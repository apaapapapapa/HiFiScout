...
test("the scheduled price-index task invokes bounded backfill and expiry maintenance", async () => {
  ...
  assert.equal(
    db.calls.length,
    5,
    "empty price and condition projection maintenance stays bounded",
  );
  assert.equal(
...
test("the scheduled remediation sweep does not recount the durable backlog", async () => {
  ...
  assert.equal(
    ...
    39,
    "a scheduled replay must reserve targeted discovery and listing completion before it starts",
  );
