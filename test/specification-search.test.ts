...
import { canonicalProductQueryUrl, parseProductQuery, validateProductQuery } from "../src/api/product-query.js";
import { parseFeedQuery, validateFeedQuery } from "../src/api/feed-query.js";

const AT = "2026-09-07T00:00:00Z";
const specs = { widthMm: 440, heightMm: 150, depthMm: 400, weightKg: 12.5,
  inputs: [{ connector: "RCA", count: 2 }], outputs: [{ connector: "XLR", count: 3 }],
  main: [], sourceUrl: "https://example.test/official" };
...
test("specification query bounds and canonicalization are shared by search and Atom", () => {
  const url = new URL("https://example.test/api/product-search?maxWidthMm=00440.000&minXlrOutputs=03&maxWeightKg=12.5");
  assert.equal(validateProductQuery(url), null);
  assert.equal(validateFeedQuery(url), null);
  const query = parseProductQuery(url);
  assert.deepEqual(query.specificationFilters, { maxWidthMm: 440, maxWeightKg: 12.5, minXlrOutputs: 3 });
  ...
  for (const input of ["maxWidthMm=0", "maxWeightKg=NaN", "maxDepthMm=1e3", "minXlrOutputs=1.5", "minRcaInputs=129", "maxHeightMm=100001", "maxWidthMm=1&maxWidthMm=2"]) {
...
test("dimensions and connector counts intersect on one verified model and exclude unknowns", async () => {
  ...
  try {
    ...
    await updateCatalogSpecifications(db, 700003, { ...specs, outputs: [{ connector: "XLR", count: null }] });
    await updateCatalogSpecifications(db, 700004, { ...specs, widthMm: null });
    await updateCatalogSpecifications(db, 700005, specs);
    sqlite.exec("UPDATE knowledge_catalog_products SET verification_status='rejected' WHERE id=700005");
    await updateCatalogSpecifications(db, 700006, { ...specs, outputs: [{ connector: "XLR", count: 2 }, { connector: "xlr", count: 2 }] });
    const result = await searchProducts(db, productQuery("?maxWidthMm=450&minXlrOutputs=3&maxWeightKg=13&includeTotal=true&sort=priceAsc"));
    assert.equal(result.totalCount, 1);
    assert.deepEqual(result.items.map((item) => item.key), ["c-700001"]);
    assert.equal(sqlite.prepare("SELECT port_count FROM catalog_specification_ports WHERE catalog_product_id=700006 AND direction='output'").get()?.port_count, null);
    const plans = sqlite.prepare("EXPLAIN QUERY PLAN SELECT catalog_product_id FROM catalog_product_specifications WHERE json_type(specification_json,'$.widthMm') IN ('integer','real') AND json_extract(specification_json,'$.widthMm')>0 AND json_extract(specification_json,'$.widthMm')<=450").all();
...
test("port projection migrates retained facts and changed specification writes are atomic and minimal", async () => {
  ...
  try {
    seed(sqlite);
    await updateCatalogSpecifications(db, 700001, specs);
    sqlite.exec(migrationSources.find((migration) => migration.name === "0108_catalog_specification_search.sql")!.sql);
    assert.equal(sqlite.prepare("SELECT port_count FROM catalog_specification_ports WHERE catalog_product_id=700001 AND direction='output'").get()?.port_count, 3);
    ...
    sqlite.exec("CREATE TRIGGER fail_ports BEFORE UPDATE ON catalog_specification_ports BEGIN SELECT RAISE(ABORT,'injected'); END");
    await assert.rejects(updateCatalogSpecifications(db, 700001, { ...specs, outputs: [{ connector: "XLR", count: 4 }] }), /injected/);
    assert.equal(JSON.parse(String(sqlite.prepare("SELECT specification_json FROM catalog_product_specifications WHERE catalog_product_id=700001").get()?.specification_json)).widthMm, 430);
    sqlite.exec("DELETE FROM catalog_product_specifications WHERE catalog_product_id=700001");
    assert.equal(sqlite.prepare("SELECT COUNT(*) AS n FROM catalog_specification_ports").get()?.n, 0);
  } finally { sqlite.close(); }
});
