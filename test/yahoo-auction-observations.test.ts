...
import {
  applyAuctionObservation, auctionInstant, auctionObservationStamp, auctionPresentation,
...
const fixture = readFileSync(new URL("./fixtures/yahoo-auctions.synthetic.html", import.meta.url), "utf8");
const context = { generation: 1, sequence: 1, observedAt: "2026-09-22T01:00:00Z", categoryId: "2084037425" };
const parse = (html = fixture, extra: Partial<typeof context> = {}) => parseYahooAuctionHtml(html, { ...context, ...extra });
...
test("auction numeric readers reject ambiguity and preserve zero versus missing", () => {
  for (const value of ["", "不明", "-1円", "1.5円", "1,00円", "1e3", "01円", "100円 税込110円", "1,000円（1件）", "9007199254740993円", "100円(税込", "100円税別)"]) {
    assert.equal(yahooAuctionPrice(value), null, value);
  }
  assert.deepEqual(yahooAuctionPrice("￥１，０００円（非課税）"), { amountYen: 1000, tax: "exempt" });
  assert.deepEqual(yahooAuctionPrice("1円"), { amountYen: 1, tax: "unknown" });
  assert.equal(yahooAuctionBidCount("０件"), 0);
  for (const value of ["", "不明", "-1", "1.2", "1,00", "1e3", "1000001"]) assert.equal(yahooAuctionBidCount(value), null);
});

test("auction parser does not turn unsupported HTML or invalid contexts into empty success", () => {
  for (const html of ["", "<p>商品がありません</p>", "<p>ログインしてください</p>", "<div>1円</div>"]) assert.equal(parse(html).status, "unsupported");
  for (const value of [null, [], {}, { ...context, generation: -1 }, { ...context, sequence: 1.1 }, { ...context, observedAt: "09/22 10:00" }]) assert.equal(parseYahooAuctionHtml(fixture, value).status, "unsupported");
...
test("auction parser removes comments, scripts, style and noscript before finding cards", () => {
  ...
  for (const wrapper of [`<!--${hidden}-->`, `<script>${hidden}</script>`, `<style>${hidden}</style>`, `<noscript>${hidden}</noscript>`]) {
  ...
  const nested = fixture.replace("８８，０００円（税込）", `８８，０００円（税込）<script>1円</script><!--1円-->`);
...
test("auction parser rejects duplicate labels, bad URLs and duplicate identities", () => {
  ...
  assert.equal(parse(fixture.replaceAll("https://auctions.yahoo.co.jp", "https://evil.example")).status, "unsupported");
...
test("auction missing and malformed fields are unobserved, not zero or current", () => {
  const result = parse(card(field("現在", "不明") + field("即決", "不明") + field("入札", "不明") + field("送料", "地域による")));
...
test("auction sale unit and sold subject do not imply identity or normalized per-piece prices", () => {
  ...
  for (const [unit, subject, expectedUnit, expectedSubject, title] of cases) {
    const value = parse(card(field("販売単位", unit) + field("販売対象", subject) + field("現在", "100円") + field("商品の状態", "ジャンク"), title)).observations[0];
...
test("auction instants reject nonexistent dates, absent zones and relative countdowns", () => {
  ...
  for (const value of ["2026-02-29T00:00:00Z", "2026-09-31T00:00:00Z", "2026-09-22T24:00:00Z", "2026-09-22T00:00:00", "2026-09-22T00:00:00+14:01", "残り3分", null]) assert.equal(auctionInstant(value), null);
  assert.equal(auctionObservationStamp({ ...context, generation: Number.NaN }), null);
  assert.equal(parse(card(field("終了日時", "9/22 21:30"))).observations[0].live.scheduledEndAt, null);
});
...
test("end extension is observed, not inferred, and can restore a pending display", () => {
  ...
  const next = parse(fixture.replace("21:30:00+09:00", "21:40:00+09:00"), { sequence: 2, observedAt: "2026-09-22T12:31:00Z" }).observations[0];
...
test("partial rechecks preserve field evidence times and do not freshen an old status", () => {
  const first = applied(null, item());
  const observation = parse(card(field("即決", "なし")), { sequence: 2, observedAt: "2026-09-22T02:00:00Z" }).observations[0];
  ...
  assert.equal(auctionPresentation(second, "2026-09-22T02:01:00Z", 30 * 60 * 1000).freshness, "stale");
});

test("replayed and late observations cannot roll back price or state", () => {
  const first = applied(null, item({ generation: 2, sequence: 3, observedAt: "2026-09-22T03:00:00Z" }));
  assert.equal(applyAuctionObservation(first, item({ generation: 2, sequence: 3, observedAt: "2026-09-22T03:00:00Z" })).status, "duplicate");
  assert.equal(applyAuctionObservation(first, item({ sequence: 99, observedAt: "2026-09-22T04:00:00Z" })).status, "stale");
  assert.equal(applyAuctionObservation(first, item({ generation: 3, sequence: 4, observedAt: "2026-09-22T01:00:00Z" })).status, "stale");
...
test("confirmed end survives unknown rechecks and requires evidence for a new cycle", () => {
  const ended = parse(card(field("状態", "終了") + field("現在", "100円") + field("入札", "3件"))).observations[0];
  const first = applied(null, ended);
  assert.equal(first.live.outcome, null);
  const unknown = parse(card(field("状態", "不明")), { sequence: 2, observedAt: "2026-09-22T02:00:00Z" }).observations[0];
  assert.equal(applied(first, unknown).live.sourceState?.value, "ended");
  assert.equal(applyAuctionObservation(first, item({ sequence: 3, observedAt: "2026-09-22T03:00:00Z" })).status, "reopening_unconfirmed");
  const restarted = parse(card(field("状態", "開催中") + field("開始日時", "2026-09-22T02:00:00Z")), { sequence: 4, observedAt: "2026-09-22T03:00:00Z" }).observations[0];
...
test("winner report is explicit and remains distinct from a completed transaction", () => {
  const result = parse(card(field("状態", "終了") + field("終了結果", "落札者あり"))).observations[0];
  assert.equal(result.live.outcome?.value, "winner_reported");
  assert.equal(result.live.currentPrice, null);
  assert.equal(parse(card(field("状態", "開催中") + field("終了結果", "落札者あり"))).observations[0].live.outcome, null);
});

test("hidden cards and labelled facts cannot supply visible prices", () => {
  const visible = card(field("現在", "100円"));
  const hidden = card(field("現在", "1円"), "HIDDEN", "z1234567890").replace('class="Product"', 'class="Product" hidden');
...
test("malformed adjacent card endings never borrow another card's price", () => {
  const html = card(field("状態", "開催中")).replace("</li>", "") + card(field("現在", "900円"), "EXAMPLE B-200", "b1234567890");
