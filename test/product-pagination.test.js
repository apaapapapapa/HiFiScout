...
test('product pagination reports total pages and supports direct offset jumps', async () => {
  ...
  const result = await listProducts(db, new URL(
...
test('pagination query validation covers offset and total-count flags', () => {
  assert.equal(validateProductQuery(new URL('https://example.test/api/products?offset=-1')), 'offset_invalid');
  assert.equal(validateProductQuery(new URL('https://example.test/api/products?includeTotal=yes')), 'includeTotal_invalid');
  assert.equal(validateProductQuery(new URL('https://example.test/api/products?offset=100&includeTotal=true')), null);
});
