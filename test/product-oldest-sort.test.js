...
test('oldest sort is accepted by product query validation', () => {
  assert.equal(validateProductQuery(new URL('https://example.test/api/products?sort=oldest')), null);
});
...
test('oldest listing uses ascending activity timestamp with keyset pagination', async () => {
  ...
  const first = await listProducts(db, new URL('https://example.test/api/products?sort=oldest&limit=2'));

  assert.deepEqual(first.items.map(row => row.id), [3, 4]);
  assert.equal(first.hasMore, true);
  assert.ok(first.nextCursor);
  assert.match(db.calls[0].sql, /ORDER BY p\.last_activity_at ASC, p\.id ASC/);

  const nextDb = queryCaptureDb([]);
  await listProducts(nextDb, new URL(`https://example.test/api/products?sort=oldest&limit=2&cursor=${encodeURIComponent(first.nextCursor)}`));
