...
const quote = (id: number, price = id * 100): MarketOffer => ({ id, shop_key: id === 1 ? "a" : "b", price_yen: price, stock_status: "in_stock", is_active: 1, last_seen_at: AT });
const sample = (id: number, listing = id, price = id * 100, at = AT): MarketSample => ({ id, listing_product_id: listing, shop_key: listing === 1 ? "a" : "b", price_yen: price, sample_kind: "asking", signal_kind: "asking", observed_at: at });

test("condition bands separate selling units, conflicts, stale offers and sparse evidence", () => {
  const facts = new Map([[1, ["used", "sale_pair"]], [2, ["used", "sale_pair"]], [3, ["used", "sale_pair"]],
    [4, ["used", "sale_single"]], [5, ["used", "junk", "operation_unchecked"]]]);
  const result = calculateMarketAnalysis([quote(1), quote(2), quote(3), quote(4), quote(5),
    { ...quote(6), last_seen_at: "2026-01-01T00:00:00Z" }, { ...quote(7), stock_status: "sold_out" }], facts, [], NOW);
  ...
  assert.equal(result.current_conditions.find((band) => band.sale_unit === "single")?.median_yen, null);
  assert.ok(result.current_conditions.some((band) => band.condition === "mixed"));
  assert.equal(result.current_conditions.reduce((sum, band) => sum + band.listing_count, 0), 5);
});

test("monthly quotes give each listing one vote and first/end observations remain distinct", () => {
  const samples = [sample(1), sample(2), sample(3), sample(4, 1, 110), sample(5, 1, 120),
...
test("projections coalesce changes, honor manual unknown, reject stale claims and recover failures", async () => {
  ...
  try {
    ...
    const observed = { ...db, prepare: (sql: string) => { statements.push(sql); return db.prepare(sql); } };
    ...
    assert.ok(!statements.some((sql) => sql.includes("FROM knowledge_catalog_price_index_samples")));
    await updateOfferFactAdmin(db, 1, { used: "unknown" }, AT);
    const racing = { ...db, batch: async <T>(batch: D1PreparedStatement[]) => {
    ...
    assert.equal((await loadMarketAnalysis(db, 700001))?.current_conditions.find((band) => band.condition === "unknown")?.listing_count, 2);
    sqlite.exec(`INSERT INTO catalog_market_dirty(catalog_product_id,claim_token,claimed_at) VALUES(700001,'abandoned','2026-09-06T00:00:00Z')`);
    assert.equal((await maintainMarketAnalysis(db, NOW)).refreshed, 1);
...
test("the invocation budget yields before a claim and failed publication leaves durable recovery", async () => {
  ...
  try {
    seed(sqlite);
    await assert.rejects(maintainMarketAnalysis(invocationBudget(db, { maxCalls: 3 }).db, NOW), /budget exhausted/);
    assert.equal(sqlite.prepare("SELECT claim_token FROM catalog_market_dirty").get()?.claim_token, null);
    sqlite.exec("CREATE TRIGGER fail_market BEFORE INSERT ON catalog_market_analysis BEGIN SELECT RAISE(ABORT,'injected'); END");
