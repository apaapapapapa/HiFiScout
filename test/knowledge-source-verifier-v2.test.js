...
test('v2 candidate variants remove a redundant manufacturer prefix conservatively', () => {
  assert.deepEqual(
    candidateModelVariants({ manufacturerId: 'tad', observedManufacturer: 'TAD', observedModel: 'TAD-D1000TX' }),
    ['TAD-D1000TX', 'D1000TX']
  );
});
...
test('v2 discovers a product from model-bearing anchor text even when the URL is opaque', async () => {
  const pages = new Map([
    ...
    ['https://example.com/item/931', '<html><head><title>ABC-1 Power Amplifier</title></head><body><h1>ABC-1 Power Amplifier</h1></body></html>']
  ]);
  const requested = [];
...
test('v2 built-in definitions add historical product indexes', () => {
  ...
  assert.ok(definitions.get('accuphase')[0].catalogUrls.includes('https://www.accuphase.com/history'));
  assert.ok(definitions.get('esoteric')[0].catalogUrls.includes('https://www.esoteric.jp/jp/support/discon'));
});
